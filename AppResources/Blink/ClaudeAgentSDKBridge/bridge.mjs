...
function moduleSearchPaths() {
  const explicit = (process.env.BLINK_CLAUDE_AGENT_SDK_PATHS || "")
    .split(":")
    .filter(Boolean);
  return [
...
function textFromHistory(history) {
  ...
  for (const entry of history) {
    ...
    if (entry?.assistant) lines.push(`Blink: ${entry.assistant}`);
  }
  return lines.join("\n");
}
...
function userMessageFromCommand(command) {
  ...
  if (typeof command.systemPrompt === "string" && command.systemPrompt.trim()) {
    content.push({
      type: "text",
      text: `Blink current voice policy and runtime context for this turn:\n${command.systemPrompt}`
    });
  }
...
try {
  ...
  const options = {
    model: process.env.BLINK_CLAUDE_MODEL || "claude-sonnet-4-6",
    maxTokens: integerFromEnv("BLINK_CLAUDE_MAX_OUTPUT_TOKENS", 64000),
    cwd: process.env.BLINK_CLAUDE_CWD || process.cwd(),
    systemPrompt: process.env.BLINK_CLAUDE_SYSTEM_PROMPT || "You are Blink.",
    pathToClaudeCodeExecutable: process.env.BLINK_CLAUDE_EXECUTABLE,
    ...
    env: {
      ...process.env,
      CLAUDE_AGENT_SDK_CLIENT_APP: "blink/1.0"
    }
  };
