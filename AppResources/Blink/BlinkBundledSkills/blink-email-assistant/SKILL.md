...
6. For Gmail sends, draft first. If the user explicitly approves sending and Composio reports missing send permission, stop and tell the user Blink's Gmail connection needs send permission in Settings -> Integrations; do not run OAuth from the agent.

## Fallbacks
- If no connector is available, use pasted/visible content directly when the user supplied it. For mailbox/account work, explain that the cleaner path is Blink Settings -> Integrations, tell the user to connect/reconnect the named mail app there, and offer voice/in-app guidance through setup; do not offer to connect it yourself and do not use Computer Use to operate Blink's own Settings/Integrations flow. Offer Cua/Computer Use as a visible app/browser fallback for the original mail task, and proceed autonomously only when the user explicitly asked for visible UI or the target has no shipped connector route.
...
- Use `blink-artifacts` for exported drafts, CSVs, or tracking sheets.
