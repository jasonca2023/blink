...
## Do Not Use When
- The request is only building a visible website/app from scratch; use `blink-build-preview`.
- The request is only local dev setup, API keys, or localhost failure without repo work; use `blink-dev-setup-doctor`.
- The request is only opening a generated code artifact; use `blink-artifacts`.
