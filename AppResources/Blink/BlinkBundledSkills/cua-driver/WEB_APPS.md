...
- **URL exception — never use `set_value` on the omnibox for
  ...
  fallbacks are blocked in Blink's runtime because logs showed they
...
a selection, the page background), Blink's default runtime has no
...
Blink's managed runtime.

### Safari and Arc

Do not use Safari or Arc AppleScript JavaScript bridges in Blink's
...
(filed 2004) tracks this and remains unresolved. Blink's current
