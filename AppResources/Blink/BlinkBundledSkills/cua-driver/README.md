# cua-driver — Blink Computer Use skill

Blink's bundled Computer Use skill teaches Codex agents to drive
...
This copy is product-managed inside Blink. Do not ask users to
install `CuaDriver.app`, run the standalone `cua-driver` CLI, or
change their browser profile. Blink ships its own helper binary
inside `Blink.app`, inherits Blink's Accessibility and Screen
...
- Trajectory recording (`RECORDING.md`) — upstream reference only in
  this build. Blink's default runtime does not expose recording or
  replay tools.
- Canvas/viewport apps (Blender, Unity, GHOST, Qt, wxWidgets) —
  diagnostic guidance only in this build. Blink's default runtime blocks
...
1. **macOS 14 or newer**.
2. **Blink permissions**: Accessibility and Screen Recording granted
   to Blink.app during onboarding.
3. **Bundled helper present**:
   `Blink.app/Contents/Helpers/BlinkComputerUseRuntime`.
...
- `RECORDING.md` — upstream trajectory recording / replay reference.
  Blink's default runtime does not expose these tools.
...
The skill evolves alongside the driver. In Blink, update it through
the bundled runtime upgrade path: bump
`BlinkComputerUseRuntime/Package.swift`, sync the upstream skill docs,
then re-apply Blink's managed-runtime guardrails.
