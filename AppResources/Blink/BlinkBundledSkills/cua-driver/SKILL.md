---
name: cua-driver
description: Drive native macOS apps and browser UI through Blink's local computer-use MCP server, backed by Cua. Snapshot AX state, act by element_index, keep work backgrounded, prefer the user's default browser, and verify via re-snapshot. Use when the user asks you to operate, drive, automate, or perform a GUI task in a real macOS application on the host.
---

# cua-driver

Orchestrates macOS app automation through Cua. In Blink's managed
...
## Blink managed runtime rules

Blink ships Cua as a local `computer-use` MCP server backed by
`BlinkComputerUseRuntime`. That helper is spawned by Codex inside
Blink.app, so macOS attributes Accessibility and Screen Recording use
to Blink itself. In this product runtime:
...
- For external account-backed apps, prefer a connected structured MCP or
  ...
  yourself. Tell the user to open Blink Settings -> Integrations and
  connect/reconnect the named app, offer voice/in-app guidance through
  setup, and do not use Computer Use to operate Blink's own
...
- Blink's default runtime does not expose `move_cursor`, `drag`, or
...
reason Blink ships Computer Use. Users pay for the right to keep typing in
...
actions. Pixel clicks are blocked in Blink's default runtime. Full
...
skill means call the same-named tool on Blink's local `computer-use`
...
| Drag / drag-and-drop / marquee select | Not exposed in Blink's default runtime; stop and explain that this build lacks a safe drag/pixel mode | `cliclick dd:`, `osascript drag` |
...
Blink's default runtime has no activation tool and does not allow
...
requirement. Blink's default runtime does not expose an activation
tool.
...
2. **Does this command move the user's real cursor?** (`cliclick`,
   ...
   `page` action. Blink's default runtime blocks coordinate clicks.
...
1. Confirm the `computer-use` MCP server is available in the current
   Codex session. Blink starts it automatically from the bundled
   `BlinkComputerUseRuntime` helper.
2. Call `check_permissions({"prompt": false})`. If Accessibility or
   ...
   permissions to Blink.app in System Settings.
3. Do not start daemons, install CuaDriver.app, or invoke standalone
   shell commands. In Blink, the MCP server is already the daemon.
...
Blink's MCP helper bootstraps the AppKit runloop required by the
overlay.
...
|---|---|---|
| **`som`** (default) | tree + screenshot | `element_index` preferred; pixels are diagnostic only in Blink |
| **`ax`** | tree only (no PNG) | `element_index` only |
| **`vision`** | PNG only (no tree) | read-only visual inspection in Blink — see [SCREENSHOT.md](./SCREENSHOT.md) |
...
| state | `get_window_state` | `click`/`set_value` (AX) | `press_key` commit (Return/Space/Tab) | pixel click in Blink |
|---|---|---|---|---|
| frontmost | ✅ | ✅ | ✅ | blocked |
...
entire point of Blink Computer Use: agents drive apps in the background while
...
Blink's default runtime blocks coordinate clicks to protect the user's
...
| Left click | `click({pid, window_id, element_index})` | default `action: "press"`. Coordinate `x/y` clicks are blocked by Blink's tool policy. |
| Double-click / open | Use `click({pid, window_id, element_index, action: "open"})` when the element advertises an open action | `double_click` is not exposed in Blink's default runtime because it can fall back to pixel stamping. |
...
Blink's default `computer-use` MCP server rejects coordinate clicks.
...
Blink's default runtime. Stop and say the task requires a future
...
this Blink runtime does not expose pixel mode.
...
When both checks pass, Blink's default runtime still does not expose
...
Blink's default runtime; only use a debug-port page target if the
...
Blink's default runtime does not expose `set_recording`, `get_recording_state`, or `replay_trajectory`.
...
- **Never** translate screenshot pixels into a click in Blink's
  default runtime — the screenshot is for visual disambiguation, not
  coordinates. Use the `element_index`.
- **Prefer AX over pixels.** Blink's default runtime blocks coordinate
