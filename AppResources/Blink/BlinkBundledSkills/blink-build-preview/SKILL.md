...
## Do Not Use When
- The task is primarily GitHub/PR/CI/repo workflow; use `blink-repo-operator`.
- The task is primarily localhost/toolchain failure; use `blink-dev-setup-doctor`.
- The task is only finding/opening an existing generated file; use `blink-artifacts`.
...
6. Use `blink-artifacts` for static HTML files and saved outputs.
...
- If dependencies fail, route to `blink-dev-setup-doctor`.
...
- Use `blink-artifacts` for open/reveal requests.
