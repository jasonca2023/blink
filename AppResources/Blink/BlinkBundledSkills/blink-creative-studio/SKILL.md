---
name: blink-creative-studio
description: "Route broad creative work to available Blink capabilities: frontend polish, design critique, brand/social planning, document/PDF outputs, spreadsheet/table outputs, and existing visual artifact handling. Provider-backed image, video, motion, and new slide/deck generation are not shipped unless the runtime explicitly exposes those capabilities."
---
...
## Do Not Use When
- The task is only a website/app/dashboard implementation; use `blink-build-preview`.
- The task is only opening or finding a finished creative artifact; use `blink-artifacts`.
- The user asks for normal frontend animations inside a site/app; use `blink-build-preview` or `frontend-design` directly.

## Primary Path
1. Choose the available medium:
   - `blink-build-preview` and `frontend-design` for frontend/UI implementation, critique, and polish.
...
- Use `blink-artifacts` to open/reveal final files. For deck/image/video outputs, only do this when the artifact already exists or a real runtime created it.
