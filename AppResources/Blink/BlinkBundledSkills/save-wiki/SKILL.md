...
The wiki lives at: `~/Library/Application Support/Blink/wiki/`
...
```
~/Library/Application Support/Blink/wiki/
...
read the full URL, attempt to recover it through Blink's allowed Computer Use
...
- `preferences/` — how the user wants Blink to behave: voice, tone, writing style, formatting preferences, communication norms, taste, things they like/dislike. `personality.md` is the canonical home for voice/tone. New preferences either extend `personality.md` or land as their own file in this directory (e.g. `preferences/email-style.md`, `preferences/code-style.md`). Blink inlines `personality.md` into the agent's system prompt at startup, so anything written here directly shapes how the agent responds.
