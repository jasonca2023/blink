...
- Use default `POST /cursor` for the primary Blink pointer choreography. This uses Blink's existing smooth point-and-return animation and must not warp the real macOS pointer.
...
'''
    path = '/tmp/blink-tour-screen.swift'
