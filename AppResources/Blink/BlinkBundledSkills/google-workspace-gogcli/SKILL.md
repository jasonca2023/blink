...
description: Connect to Google Workspace through the local gogcli (`gog`) command for Gmail, Calendar, Drive, Docs, Sheets, Slides, Chat, Contacts, Tasks, Groups, Admin, Classroom, Forms, Apps Script, People, and Keep. Use when the user asks Blink to use Google Workspace, Gmail, Calendar, Drive, Docs, Sheets, Slides, Chat, Contacts, Tasks, Workspace Admin, or to set up/check gogcli auth.
version: 1.0.0
argument-hint: "[workspace task or auth setup]"
---

Use `gog` from https://github.com/steipete/gogcli as Blink's local Google Workspace connector.

This is a local CLI integration, not an Blink-hosted Google login. Do not add hosted key sync, server-side OAuth, or repository-stored credentials. `gog` stores credentials in its own OS keyring or encrypted/file keyring under the user's local account.
...
If `gog auth list` reports that the file keyring needs a passphrase, stop and tell the user gogcli needs its local keyring passphrase available to Blink as `GOG_KEYRING_PASSWORD`, usually in `~/.config/blink/secrets.env`, or they should migrate gogcli to the macOS Keychain backend. Do not keep retrying.
...
For normal Gmail/Calendar/Drive tasks, do not run OAuth setup from the agent. Google setup belongs in Blink Settings → Google unless the user explicitly asks for setup help.

If Google OAuth says the app is "Blink", that branding comes from the local gogcli OAuth client stored in `~/Library/Application Support/gogcli/credentials.json`. Blink is reusing the local gogcli store. To make OAuth say Blink, replace that gogcli credential file with an Blink-owned Desktop OAuth client and re-auth.
...
- If auth is missing, point the user to Blink Settings → Google. Only provide `gog auth credentials` / `gog auth add` commands when the user explicitly asks for manual setup.
...
- `credentials_exists: false`: check `gog auth credentials list --json`; if still empty, use Blink Settings → Google or run `gog auth credentials <client-json>` only for explicit setup tasks.
- No accounts in `gog auth list`: use Blink Settings → Google or run `gog auth add <email> --services ...` only for explicit setup tasks.
- Re-auth needed or missing scopes: use Blink Settings → Google, or run `gog auth add <email> --services ... --force-consent` only for explicit setup tasks.
