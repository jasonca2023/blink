# Blink Browser Workspace Spec

Reference mockup: `/Users/blink/Library/Application Support/Blink/AgentMode/CodexHome/generated_images/019e46fb-1755-7dd2-aab7-1e97caad1128/ig_01581f08b8b8c678016a0e135957ec8191a35586e2a2094b2a.png`

## Product intent

Blink should support a browser workspace that can show real web sites and local web pages while keeping Blink's own chat interface embedded on the right side of the browser. The user should feel like they are working inside a focused research/build browser, not jumping between a browser, a detached HUD, and separate specialist agents.
...
- Right side: Blink's own copied chat component, scoped to the current page and current specialist mode.
...
1. Let users browse real websites and local pages inside an Blink-owned workspace.
2. Keep Blink's chat interface visible and context-aware without covering the page.
3. Let specialists operate as mode chips/tabs within the side panel instead of becoming separate hidden sessions.
4. Preserve the familiar Blink composer, transcript rows, tool status, attachments, and actions.
...
- Multi-profile cookie/account management beyond the embedded WebKit session chosen for Blink.
...
- Blink-generated local previews.
...
### Right Blink chat panel

The side panel is not a generic browser sidebar. It is Blink's own chat interface component rendered inside the browser workspace.
...
5. Composer
   - Same Blink prompt composer behavior as the main panel/HUD.
...
- From a URL in chat: “Open in Blink browser”.
- From local HTML/docs: “Preview with Blink”.
- From Connect/specialist surfaces: “Open browser workspace”.
- From Agent result artifacts: open generated page in workspace.
- From current screen/browser context: “Bring this page into Blink”.

### Page-to-chat context

Blink should attach context in layers:
...
- Purple remains the main Blink accent for active specialists and send controls.
- Use small status dots rather than large banners.
- Cards should have subtle borders, not heavy panels.
- The chat panel should look native to Blink, not like an iframe from another product.
...
- `BlinkBrowserChatSidePanel`
  - Reuses the existing Blink chat component where possible.
  - Injects page-scoped context provider and specialist chip bar.

- `BlinkWebContextProvider`
...
- `BlinkBrowserSpecialistMode`
  - Defines specialist chip metadata and prompt overlays.

### Reuse existing Blink surfaces
...
### Phase 2: Real Blink chat integration

Deliver:

- Reused Blink composer and transcript.
...
- The composer behaves like the main Blink composer.
...
- Blink can summarize a real article from page text.
- Blink can explain selected text.
- Blink can inspect a local preview with URL/path context.
...
1. Should the workspace use a shared Blink WebKit process pool or a separate ephemeral session by default?
...
Start with a narrow, non-invasive prototype: one new browser workspace window, one WebView, and a clean Blink chat panel with blank transcript, clear action, page context, and basic suggestion chips. Keep ordinary page chat inside the Browser Workspace instance, with a small page-context object containing URL, title, selected text, and optional extracted body text.

## Prototype status - 2026-05-20

Implemented in `Blink/BlinkBrowserWorkspaceWindowManager.swift` and opened from the app menu with Browser Workspace / Command-Option-B after rebuild.

Current prototype covers:

- Native Blink Browser Workspace window shell.
...
- Docked right Blink chat with a blank default transcript, clear action, page context strip, and basic suggestion chips.
...
- Chrome cookie import from selectable local Chrome profiles, with active-site or all-cookie import into Blink's WebKit cookie store.
...
- Inline browser plans for prompts such as “search/click the first 4 results and summarize them here”: Blink extracts the query, searches from the Browser Workspace, opens the first result pages as tabs, and posts compact summaries back into the same side chat.
...
1. Replace the prototype chat message handler with the real Blink chat/composer pipeline.
