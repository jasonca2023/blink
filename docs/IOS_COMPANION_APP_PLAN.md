# Blink iOS Companion App Plan

## Goal

Build an iPhone and iPad companion app that lets the user talk to Blink away from the Mac keyboard, see current Blink tasks, start or steer Agent Mode work, review results, and receive task-status notifications.

The iOS app should feel like a remote control and second surface for the existing macOS Blink runtime. The Mac remains the source of truth for local files, screen context, computer-use permissions, agent processes, secrets, and Codex runtime state.

## Product Shape

### Core jobs

1. Talk to Blink from iPhone or iPad.
...
4. Carry useful context from iOS to Mac.
   ...
   - No direct iOS filesystem assumptions inside the Mac agent prompt; uploads are copied into an Blink attachment inbox on the Mac first.

## Guiding Constraints

- Keep Blink macOS as the authority. Do not run Codex, local file automation, or computer-use from iOS directly in the first version.
- Use local network pairing first. Avoid hosted Blink accounts or cloud key sync.
...
- Widgets and task summary snapshots in `BlinkWidgetStateStore`.
- Local-only bridge precedent in `BlinkExternalControlBridge`.
- SDK-style public wrapper in `BlinkSDK`.
...
- Bonjour service discovery: `_blink._tcp`.
...
- `~/Library/Application Support/Blink/PairedDevices/paired-devices.json`
...
```swift
struct BlinkCompanionSnapshot: Codable, Equatable {
...
- `/open-on-mac` should only open whitelisted local paths/deep links returned by Blink, not arbitrary device-provided paths.
...
- Large Blink orb / press-to-talk button.
...
- Mac host selector if multiple Blink Macs are discovered.
...
- Starting a new task while Blink is speaking should interrupt the old spoken response and start the new capture, matching the Mac behavior.
...
3. Mac feeds the audio into the existing Blink voice/realtime pipeline.
...
This is easier to prototype but less faithful to “talk to Blink” because it bypasses Blink’s existing voice state machine.
...
1. User opens Blink Settings → Companion Devices.
...
- Add `BlinkCompanionBridgeServer`.
- Add pairing store and Settings UI for paired devices.
- Add companion snapshot builder from `CompanionManager`, `CodexAgentSession`, and `BlinkWidgetStateStore`.
...
- Add a new iOS target or sibling Swift package app: `BlinkCompanion`.
...
- User can start, steer, stop, and archive Blink agent tasks from iOS.

### Phase 4: Voice

- Add iOS microphone capture.
- Stream audio to the Mac bridge.
- Feed streamed audio into Blink’s voice/realtime path.
- Send partial/final transcript and speaking state back to iOS.
- Add playback destination toggle: iPhone/iPad or Mac.

Exit criteria:

- User can press-to-talk on iPhone/iPad and get a real Blink response routed through the Mac runtime.

### Phase 5: Attachments and notifications

- Add camera/photo/document attachments.
- Store uploads in an Blink Mac attachment inbox and pass them into agent task context.
...
- Shortcuts integration for “Ask Blink” and “Start Blink Agent.”
...
- `Blink/BlinkCompanionBridgeServer.swift`
- `Blink/BlinkCompanionPairingStore.swift`
- `Blink/BlinkCompanionModels.swift`
- `Blink/BlinkCompanionSnapshotStore.swift`
- `Blink/BlinkCompanionSettingsView.swift`
- `BlinkCompanion/` or a new Xcode iOS target folder.
...
- `BlinkWidgetStateStore.swift` for shared stats/attention conversion.
- `BlinkSettingsWindowManager.swift` for paired-device settings.
- `BlinkApp.swift` for deep links and bridge lifecycle.
