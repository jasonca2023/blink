...
BASE_URL="${BLINK_BRIDGE_URL:-http://127.0.0.1:32123}"
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
TMP_DIR="${TMPDIR:-/tmp}/blink-bridge-tests"
mkdir -p "$TMP_DIR"
...
xcrun swiftc -enable-bare-slash-regex -typecheck \
  $(find Blink -maxdepth 1 -name '*.swift' ! -name 'BlinkApp.swift' | sort)
pass 'swiftc -typecheck app Swift sources (no xcodebuild)'

printf '\n== Bridge health ==\n'
if ! health=$(curl -sS --max-time 2 "$BASE_URL/health"); then
  warn "bridge is not reachable at $BASE_URL; launch/restart Blink to run live bridge tests"
  exit 0
fi
printf '%s\n' "$health"
...
PY
pass 'primary /cursor triggers Blink choreography without warping system pointer'
...
printf '\nAll Blink external-control bridge tests passed.\n'
