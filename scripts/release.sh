#!/usr/bin/env bash
# Build, sign, package, and notarize Blink for direct distribution
...
#   1) Have a "Developer ID Application: Blink (SW75ZJJ5R6)" cert in
...
#          BlinkNotary
#      The profile name "BlinkNotary" is what NOTARY_PROFILE below
...
#
# Output:
#   dist/Blink-<version>-<build>.dmg  (signed + stapled when notarized)
...
APP_NAME="Blink"
BUNDLE_ID="com.blink.blink"
TEAM_ID="SW75ZJJ5R6"
SIGNING_IDENTITY="Developer ID Application: Blink (${TEAM_ID})"
NOTARY_PROFILE="BlinkNotary"
...
if [[ -z "$VERSION" ]]; then
    VERSION=$(/usr/libexec/PlistBuddy -c "Print :CFBundleShortVersionString" Blink/Info.plist 2>/dev/null \
...
if [[ "${LOCAL_BUILD}" -ne 1 ]]; then
    ...
    # Blink is distributed direct (not through the App Store). Refuse to
    ...
    if [[ "${export_method}" != "developer-id" ]]; then
        ...
        echo "       Blink ships outside the App Store. Edit the plist before retrying." >&2
        exit 1
    fi
fi
