#!/usr/bin/env bash
# Single-command release pipeline for Blink (direct distribution,
# NOT App Store):
...
DMG_PATH="dist/Blink-${FINAL_VERSION}-${FINAL_BUILD}.dmg"
if [[ ! -f "$DMG_PATH" ]]; then
...
echo "         --title \"Blink ${FINAL_VERSION}\" \\"
echo "         --notes \"Release notes here\""
echo ""
echo "  2. Commit + tag + push:"
echo "       git add appcast.xml Blink.xcodeproj/project.pbxproj"
