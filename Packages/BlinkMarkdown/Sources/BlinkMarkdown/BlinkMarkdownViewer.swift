//
//  BlinkMarkdownViewer.swift
//  BlinkMarkdown
//
//  A first-party Markdown document window for Blink-owned notes,
...
@MainActor
public final class BlinkMarkdownViewerWindowManager {
    public static let shared = BlinkMarkdownViewerWindowManager()
    ...
    private func updateWindow(fileURL: URL) {
        guard let hostingView: NSHostingView<BlinkMarkdownViewerView> = BlinkLiquidGlassWindowSurface.hostingView(in: window) else {
            window?.contentView = NSHostingView(rootView: BlinkMarkdownViewerView(fileURL: fileURL))
            return
        }

        hostingView.rootView = BlinkMarkdownViewerView(fileURL: fileURL)
    }

    private func makeWindow(fileURL: URL) -> NSWindow {
        let hostingView = NSHostingView(rootView: BlinkMarkdownViewerView(fileURL: fileURL))
        let window = NSWindow(
...
private struct BlinkMarkdownViewerView: View {
    ...
    private func markdownPreview(_ markdown: String) -> some View {
        ScrollView {
            LazyVStack(alignment: .leading, spacing: 12) {
                ForEach(BlinkMarkdownBlock.blocks(from: markdown)) { block in
    ...
    @ViewBuilder
    private func markdownBlockView(_ block: BlinkMarkdownBlock) -> some View {
        switch block.kind {
    ...
    private func errorPane(_ message: String) -> some View {
        VStack(spacing: 12) {
            Image(systemName: "exclamationmark.triangle")
                ...
                .foregroundColor(.orange)
            Text("Blink couldn't open this Markdown file.")
...
private struct BlinkMarkdownBlock: Identifiable {
    enum Kind {
    ...
    static func blocks(from markdown: String) -> [BlinkMarkdownBlock] {
        var blocks: [BlinkMarkdownBlock] = []
        ...
        func appendBlock(kind: Kind, text: String) {
            blocks.append(BlinkMarkdownBlock(id: nextID, kind: kind, text: text))
            nextID += 1
        }
