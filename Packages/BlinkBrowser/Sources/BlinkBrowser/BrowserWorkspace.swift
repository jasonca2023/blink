//
//  BlinkBrowserWorkspaceWindowManager.swift
//  Blink
//
//  Prototype browser workspace: a WebKit page canvas with an Blink chat
...
@MainActor
public final class BlinkBrowserWorkspaceWindowManager {
    public static let shared = BlinkBrowserWorkspaceWindowManager()
    ...
    private func makeWindow(initialURL: URL?, delegate: BrowserWorkspaceAgentDelegate) -> NSWindow {
        let content = BlinkBrowserWorkspaceView(initialURL: initialURL, delegate: delegate)
        let hostingView = BlinkBrowserWorkspaceHostingView(rootView: content)
        ...
        let glassBackdrop = BlinkLiquidGlassBackdropView(cornerRadius: 28)
        ...
        glassBackdrop.configure(
            ...
            accentColor: BlinkAccentTheme.current.nsColor,
            strength: .expanded
        )
        ...
        window.title = "Blink Browser Workspace"
        ...
        window.center()
        window.setFrameAutosaveName("Blink.BrowserWorkspace")
        return window
    }

    private static func windowAppearanceForCurrentTheme() -> NSAppearance? {
        switch BlinkTheme.current {
        case .dark:
...
private final class BlinkBrowserWorkspaceHostingView<Content: View>: NSHostingView<Content> {
...
private struct BlinkBrowserWindowDragRegion: NSViewRepresentable {
    func makeNSView(context: Context) -> BlinkBrowserWindowDragRegionView {
        BlinkBrowserWindowDragRegionView(frame: .zero)
    }

    func updateNSView(_ nsView: BlinkBrowserWindowDragRegionView, context: Context) {}
}

private final class BlinkBrowserWindowDragRegionView: NSView {
    override var mouseDownCanMoveWindow: Bool { false }
...
private struct BlinkBrowserWorkspaceView: View {
    @StateObject private var model: BlinkBrowserWorkspaceModel
    @State private var selectedSpecialist: BlinkBrowserSpecialist = .researcher
    ...
    @AppStorage(BlinkDefaults.userThemeDefaultsKey) private var selectedThemeRawValue = BlinkTheme.system.rawValue
    @AppStorage(BlinkAccentTheme.userDefaultsKey) private var selectedAccentRawValue = BlinkAccentTheme.blue.rawValue
    @AppStorage(BlinkDefaults.userGlassOpacityDefaultsKey) private var glassOpacity = 0.75
    @AppStorage(BlinkDefaults.userGlassFrostingDefaultsKey) private var glassFrosting = 0.20

    init(initialURL: URL?, delegate: BrowserWorkspaceAgentDelegate) {
        _model = StateObject(wrappedValue: BlinkBrowserWorkspaceModel(initialURL: initialURL, delegate: delegate))
    }

    private var selectedAccentTheme: BlinkAccentTheme {
        BlinkAccentTheme(rawValue: selectedAccentRawValue) ?? .blue
    }
    ...
    private var isBrowserDarkMode: Bool {
        switch BlinkTheme(rawValue: selectedThemeRawValue) ?? .system {
        case .dark:
            return true
        case .light:
            return false
    ...
    private func browserPane(tabID: UUID, placement: BlinkBrowserPanePlacement) -> some View {
        ZStack(alignment: .topTrailing) {
            BlinkWorkspaceWebView(
    ...
    private func splitPaneControls(tabID: UUID, placement: BlinkBrowserPanePlacement) -> some View {
        HStack(spacing: 6) {
    ...
    private var browserWindowCloseButton: some View {
        Button(action: { BlinkBrowserWorkspaceWindowManager.shared.close() }) {
            Image(systemName: "xmark")
    ...
    private func tabButton(_ tab: BlinkBrowserTab) -> some View {
        HStack(spacing: 6) {
        ...
        .onDrop(of: [UTType.text.identifier], delegate: BlinkBrowserTabDropDelegate(destinationTabID: tab.id, draggingTabID: $draggingTabID, model: model))
    }
    ...
    private var chatResizeHandle: some View {
        Rectangle()
            ...
            .help("Drag to resize Blink chat")
    }
        ...
        VStack(alignment: .leading, spacing: 9) {
            ...
            HStack(spacing: 8) {
                Image(systemName: model.activeTab.currentURL?.isFileURL == true ? "doc.text" : "globe")
                    .foregroundStyle(.secondary)
                Text(model.activeTab.currentURL?.absoluteString ?? "open-blink://welcome")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                    .lineLimit(1)
    ...
    private func specialistChip(_ specialist: BlinkBrowserSpecialist) -> some View {
    ...
    private func inspectorSelectionChip(_ selection: BlinkBrowserInspectorSelection) -> some View {
        HStack(spacing: 6) {
    ...
    private var composer: some View {
        HStack(alignment: .bottom, spacing: 8) {
            VStack(alignment: .leading, spacing: 7) {
                BlinkBrowserComposerEditor(text: $composerText, onSubmit: sendPrototypeMessage)
                    .frame(minHeight: 34, maxHeight: 82)
                    .overlay(alignment: .topLeading) {
                        if composerText.isEmpty {
                            Text("Ask Blink anything about this page...")
                                .font(.system(size: 13))
                                .foregroundStyle(.secondary)
                                .padding(.top, 8)
                                .padding(.leading, 5)
...
private struct BlinkBrowserTabDropDelegate: DropDelegate {
    let destinationTabID: UUID
    @Binding var draggingTabID: UUID?
    let model: BlinkBrowserWorkspaceModel
...
private struct BlinkBrowserComposerEditor: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSScrollView {
        ...
        let textView = BlinkBrowserComposerTextView()
    ...
    func updateNSView(_ nsView: NSScrollView, context: Context) {
        guard let textView = nsView.documentView as? BlinkBrowserComposerTextView else { return }
        textView.onSubmit = onSubmit
...
private final class BlinkBrowserComposerTextView: NSTextView {
    var onSubmit: (() -> Void)?
...
@MainActor
protocol BlinkBrowserWorkspaceModelProtocol: AnyObject {
...
@MainActor
private final class BlinkBrowserWorkspaceModel: ObservableObject, BlinkBrowserWorkspaceModelProtocol {
    private weak var delegate: BrowserWorkspaceAgentDelegate?
    @Published private(set) var tabs: [BlinkBrowserTab]
    @Published var activeTabID: UUID
    @Published var splitTabID: UUID?
    @Published var addressText = ""
    @Published var messages: [BlinkBrowserChatMessage] = []
    @Published private(set) var chromeProfiles: [BlinkChromeProfile] = []
    @Published var selectedChromeProfileID: UUID?
    @Published var chromeCookieStatus = "Pick a Chrome profile, then import cookies into Blink's browser session."
    ...
    @Published private(set) var linkedAgentSummary = "Blink handles Browser Workspace chat inline and only starts Agent Mode when the prompt clearly needs longer-running work."
    @Published private(set) var attachments: [BlinkBrowserAttachment] = []
    @Published private(set) var inspectorSelections: [BlinkBrowserInspectorSelection] = []
    ...
    init(initialURL: URL?, delegate: BrowserWorkspaceAgentDelegate) {
        ...
        let firstTab = BlinkBrowserTab(initialURL: initialURL)
        self.tabs = [firstTab]
        self.activeTabID = firstTab.id
        self.addressText = firstTab.addressText
    }

    var activeTab: BlinkBrowserTab {
    ...
    func clearChat() {
        ...
        linkedAgentSummary = "Blink is handling Browser Workspace chat inline. A background agent is only used for clearly longer-running work."
    }
    ...
    func loadRequest(for tabID: UUID) -> BlinkBrowserLoadRequest {
        tab(for: tabID)?.loadRequest ?? BlinkBrowserLoadRequest(html: Self.welcomeHTML)
    }
    ...
    func addTab() {
        let tab = BlinkBrowserTab(initialURL: nil)
        tabs.append(tab)
        activateTab(tab.id)
    }
    ...
    func apply(metadata: BlinkBrowserPageMetadata, for tabID: UUID) {
        updateTab(tabID) { tab in
    ...
    func loadAddress(_ rawValue: String) -> Bool {
        ...
        if rawValue == "open-blink://welcome" {
            loadWelcomePage()
            return true
        }

        guard let url = Self.url(from: rawValue) else {
            updateActiveTab { $0.errorText = "Blink could not understand that URL or local path." }
            return false
        }
        if url.isFileURL && !FileManager.default.fileExists(atPath: url.path) {
            updateActiveTab { $0.errorText = "Blink could not find that local file." }
            return false
        }
        updateActiveTab { tab in
            ...
            tab.addressText = url.absoluteString
            tab.errorText = nil
            tab.loadRequest = BlinkBrowserLoadRequest(url: url)
        }
        addressText = url.absoluteString
        return true
    }
    ...
    func reload() {
        if let webView = webViews[activeTabID] {
            ...
            updateActiveTab { $0.loadRequest = BlinkBrowserLoadRequest(url: currentURL) }
        }
    }
    ...
    func loadWelcomePage() {
        updateActiveTab { tab in
            tab.currentURL = nil
            tab.title = "Blink Browser Workspace"
            tab.addressText = "open-blink://welcome"
            tab.errorText = nil
            tab.selectedText = ""
            tab.readableText = Self.welcomeHTML
            tab.readableTextCharacterCount = Self.welcomeHTML.count
            tab.contextStatus = "Local"
            tab.loadRequest = BlinkBrowserLoadRequest(html: Self.welcomeHTML)
        }
        addressText = "open-blink://welcome"
    }
    ...
    func recordInspectorSelection(_ payload: [String: Any]) {
        ...
        let sourceURL = payload["sourceURL"] as? String ?? activeTab.currentURL?.absoluteString ?? "open-blink://welcome"
        ...
        let selection = BlinkBrowserInspectorSelection(
            order: order,
            selector: selector,
    ...
    func removeInspectorSelection(_ selection: BlinkBrowserInspectorSelection) {
    ...
    private func removeInspectorSelectionHighlight(selector: String?) {
        ...
        (() => {
          ...
          if (!window.__openBlinkInspectorRemoveSelection) return false;
          window.__openBlinkInspectorRemoveSelection(selector);
          return true;
        })();
        """
        webView.evaluateJavaScript(script, completionHandler: nil)
    ...
    func attachActiveTabScreenshot() {
        Task {
            guard let data = await captureActiveTabScreenshot() else {
                await MainActor.run { self.messages.append(BlinkBrowserChatMessage(role: "Blink", text: "Blink could not capture the current browser viewport yet.", isUser: false)) }
                return
            }
            await MainActor.run {
                self.attachments.append(BlinkBrowserAttachment(displayName: "Viewport screenshot", detail: "\(data.count / 1024) KB captured from the active tab", systemImage: "camera.viewfinder"))
                self.messages.append(BlinkBrowserChatMessage(role: "Blink", text: "Attached the current browser viewport screenshot to this workspace chat.", isUser: false))
            }
        }
    }

    private func addAttachment(url: URL) {
        attachments.append(BlinkBrowserAttachment(displayName: url.lastPathComponent, detail: url.path, systemImage: url.isFileURL ? "doc" : "link"))
        messages.append(BlinkBrowserChatMessage(role: "Blink", text: "Attached \(url.lastPathComponent).", isUser: false))
    }

    private func addAttachment(text: String) {
        let clipped = Self.truncatedContext(text, limit: 80)
        attachments.append(BlinkBrowserAttachment(displayName: "Dropped text", detail: clipped, systemImage: "text.quote"))
        messages.append(BlinkBrowserChatMessage(role: "Blink", text: "Attached dropped text to this workspace chat.", isUser: false))
    }
    ...
    private static func inspectorModeScript(enabled: Bool, nextOrder: Int, selectedSelectors: [String]) -> String {
        ...
        return """
        (() => {
          window.__openBlinkInspectorOrder = \(nextOrder);
          const existing = window.__openBlinkInspector;
          if (existing && existing.cleanup) existing.cleanup();
          window.__openBlinkInspectorSelectedSelectors = new Set(\(selectedSelectorsLiteral));
          const clearTransientUI = () => {
            document.getElementById('open-blink-inspector-hover')?.remove();
            document.getElementById('open-blink-inspector-comment')?.remove();
          };
          if (!\(enabledLiteral)) {
            clearTransientUI();
            document.body && document.body.classList.remove('open-blink-inspecting');
            return true;
          }
          const css = document.getElementById('open-blink-inspector-style') || document.createElement('style');
          css.id = 'open-blink-inspector-style';
          css.textContent = `
            .open-blink-inspecting * { cursor: crosshair !important; }
            .open-blink-inspector-overlay { position: absolute; z-index: 2147483645; border: 2px solid #23d5ff; border-radius: 8px; background: rgba(35,213,255,.12); box-shadow: 0 0 0 1px rgba(255,255,255,.22), 0 10px 30px rgba(0,0,0,.28); pointer-events: none; box-sizing: border-box; transition: left .045s linear, top .045s linear, width .045s linear, height .045s linear; }
            .open-blink-inspector-overlay.locked { border-color: #2f7dff; background: rgba(47,125,255,.14); }
            .open-blink-inspector-badge { position: absolute; z-index: 2147483647; background: #2f7dff; color: white; border-radius: 999px; padding: 2px 7px; font: 700 12px -apple-system, BlinkMacSystemFont, sans-serif; box-shadow: 0 4px 14px rgba(0,0,0,.28); pointer-events: none; }
            .open-blink-inspector-comment { position: absolute; z-index: 2147483647; width: 260px; padding: 10px; border-radius: 14px; background: rgba(12,16,22,.96); border: 1px solid rgba(35,213,255,.45); color: white; font: 13px -apple-system, BlinkMacSystemFont, sans-serif; box-shadow: 0 18px 50px rgba(0,0,0,.34); cursor: default !important; }
            .open-blink-inspector-comment textarea { width: 100%; min-height: 66px; box-sizing: border-box; resize: vertical; border-radius: 10px; border: 1px solid rgba(255,255,255,.16); background: rgba(255,255,255,.08); color: white; padding: 8px; font: 13px -apple-system, BlinkMacSystemFont, sans-serif; outline: none; cursor: text !important; }
            .open-blink-inspector-comment .title { font-weight: 800; margin-bottom: 7px; color: #23d5ff; }
            .open-blink-inspector-comment .actions { display: flex; justify-content: flex-end; gap: 7px; margin-top: 8px; }
            .open-blink-inspector-comment button { border: 0; border-radius: 999px; padding: 5px 10px; color: white; background: rgba(255,255,255,.14); font-weight: 800; cursor: pointer !important; }
            .open-blink-inspector-comment button.primary { background: #2f7dff; }
          `;
          document.head.appendChild(css);
          document.body && document.body.classList.add('open-blink-inspecting');
          const isInspectorUI = (el) => !!(el && el.closest && el.closest('.open-blink-inspector-comment, .open-blink-inspector-badge, .open-blink-inspector-overlay'));
          ...
          const removeSelection = (selector) => {
            const nodes = document.querySelectorAll('.open-blink-inspector-overlay.locked, .open-blink-inspector-badge');
            nodes.forEach((node) => {
              if (!selector || node.dataset.openBlinkSelector === selector) node.remove();
            });
            if (selector) {
              window.__openBlinkInspectorSelectedSelectors.delete(selector);
            } else {
              window.__openBlinkInspectorSelectedSelectors.clear();
            }
            document.getElementById('open-blink-inspector-comment')?.remove();
          };
          window.__openBlinkInspectorRemoveSelection = removeSelection;
          const hoverBox = document.createElement('div');
          hoverBox.id = 'open-blink-inspector-hover';
          hoverBox.className = 'open-blink-inspector-overlay';
          ...
          const moveHover = (event) => {
            ...
            window.__openBlinkInspectorHoveredElement = el;
          };
          const showCommentBox = (el, lockedBox, order) => {
            ...
            panel.id = 'open-blink-inspector-comment';
            panel.className = 'open-blink-inspector-comment';
            ...
            const saveSelection = () => {
              ...
              badge.className = 'open-blink-inspector-badge';
              badge.textContent = '#' + order;
              badge.style.left = `${rect.left + window.scrollX}px`;
              badge.style.top = `${Math.max(0, rect.top + window.scrollY - 22)}px`;
              badge.dataset.openBlinkSelector = selector;
              document.body.appendChild(badge);
              lockedBox.dataset.openBlinkOrder = String(order);
              lockedBox.dataset.openBlinkSelector = selector;
              window.__openBlinkInspectorSelectedSelectors.add(selector);
              ...
              window.webkit?.messageHandlers?.openBlinkInspector?.postMessage(payload);
              panel.remove();
            };
          ...
          const clickHandler = (event) => {
            ...
            const el = event.target;
            const selector = selectorFor(el);
            if (window.__openBlinkInspectorSelectedSelectors.has(selector)) {
              removeSelection(selector);
              window.webkit?.messageHandlers?.openBlinkInspector?.postMessage({ action: 'remove', selector, sourceURL: location.href });
              return;
            }
            const rect = el.getBoundingClientRect();
            const order = window.__openBlinkInspectorOrder++;
            const lockedBox = document.createElement('div');
            lockedBox.className = 'open-blink-inspector-overlay locked';
            positionBox(lockedBox, rect);
            document.body.appendChild(lockedBox);
            showCommentBox(el, lockedBox, order);
          };
          const keyHandler = (event) => {
            if (event.key === 'Escape') document.getElementById('open-blink-inspector-comment')?.remove();
          };
          document.addEventListener('mouseover', moveHover, true);
          document.addEventListener('mousemove', moveHover, true);
          document.addEventListener('click', clickHandler, true);
          document.addEventListener('keydown', keyHandler, true);
          window.__openBlinkInspector = { cleanup: () => {
            ...
            delete window.__openBlinkInspectorRemoveSelection;
            document.body && document.body.classList.remove('open-blink-inspecting');
          } };
          return true;
        })();
    ...
    func discoverChromeProfiles() {
        let profiles = BlinkChromeCookieImporter.discoverProfiles()
        ...
        chromeCookieStatus = profiles.isEmpty
            ? "No readable Chrome cookie profiles found. Blink may need Full Disk Access, or Chrome may not be installed."
            : "Found \(profiles.count) Chrome profile\(profiles.count == 1 ? "" : "s"). Choose one and import site cookies or all cookies."
    }

    func importChromeCookies(scope: BlinkChromeCookieImportScope) async {
        ...
        let result = await BlinkChromeCookieImporter.importCookies(from: profile, matchingHost: host)
        isImportingChromeCookies = false
        chromeCookieStatus = result.summary
        messages.append(BlinkBrowserChatMessage(role: "Blink", text: result.summary, isUser: false))
    }
    ...
    private func inlineBrowserReply(for prompt: String) -> String {
        ...
        guard !trimmedText.isEmpty else {
            return "Blink has full access to the browser instance, but this page is not exposing readable text yet. Try selecting text, refreshing context, or asking Blink to click, type, search, or fill a page control."
        }
        ...
        if lowercasedPrompt.contains("explain") {
            return "Blink can work from the visible page text here. The most relevant excerpt is: \(Self.truncatedContext(trimmedText, limit: 900))"
        }

        return "Blink is handling this in the Browser Workspace chat, with access to the current tab. For this first inline pass, I can summarize, pull key points, explain selected text, navigate, search, click, type, and fill page controls without starting a background agent."
    }
    ...
    func sendBrowserMessage(_ prompt: String, specialist: BlinkBrowserSpecialist) {
        refreshPageContext(for: activeTabID, trigger: "Message send")
        messages.append(BlinkBrowserChatMessage(role: "You", text: prompt, isUser: true))

        if handleBrowserSlashCommand(prompt) {
            return
        }

        if let browserPlan = BlinkBrowserResearchPlan(prompt: prompt) {
            performBrowserResearchPlan(browserPlan)
            return
        }

        if let directAction = BlinkBrowserDirectPageAction(prompt: prompt) {
        ...
        if !Self.needsBackgroundAgent(prompt) {
            ...
            if usesAnthropicBrowserModel && (hasSDK || !apiKey.isEmpty) {
                ...
                Task {
                    let agent = BlinkBrowserAgentRunner(apiKey: apiKey, modelName: modelID, browserModel: self)
    ...
    private func startLinkedAgentTask(for prompt: String, specialist: BlinkBrowserSpecialist) {
        ...
        if let linkedAgentSessionID, delegate?.hasLinkedAgentSession(id: linkedAgentSessionID) == true {
            ...
            linkedAgentSummary = "Sent follow-up to the linked Blink agent task."
            messages.append(BlinkBrowserChatMessage(role: "Blink", text: "Sent that follow-up to the linked Blink Agent Mode task with the current page context attached.", isUser: false))
            return
        }

        guard let session = delegate?.submitNewAgentTaskFromUI(agentPrompt, source: "browser_workspace_chat") else {
            linkedAgentSummary = "Could not start an Blink Agent Mode task for this page."
            messages.append(BlinkBrowserChatMessage(role: "Blink", text: "I could not start an Agent Mode task for this page, so the message stayed in the workspace chat.", isUser: false))
            return
        }
        ...
        messages.append(BlinkBrowserChatMessage(role: "Blink", text: "Started a linked Blink Agent Mode task for this page. Future messages in this workspace will continue that same task with fresh page context.", isUser: false))
    }
    ...
    private func performBrowserResearchPlan(_ plan: BlinkBrowserResearchPlan) {
        guard !isRunningBrowserPlan else {
            messages.append(BlinkBrowserChatMessage(role: "Blink", text: "Blink is already running a browser plan in this workspace. Let that finish, then send the next one.", isUser: false))
            return
        }

        isRunningBrowserPlan = true
        messages.append(BlinkBrowserChatMessage(role: "Blink", text: "Plan: search for “\(plan.query)”, open the first \(plan.resultCount) result\(plan.resultCount == 1 ? "" : "s") in Browser Workspace tabs, then summarize them here.", isUser: false))

        Task { [weak self] in
            let outcome = await BlinkBrowserResearchRunner.run(plan)
            await MainActor.run {
                guard let self else { return }
                self.isRunningBrowserPlan = false
    ...
    private func applyBrowserResearchOutcome(_ outcome: BlinkBrowserResearchOutcome, plan: BlinkBrowserResearchPlan) {
        guard !outcome.items.isEmpty else {
            messages.append(BlinkBrowserChatMessage(role: "Blink", text: outcome.failureMessage ?? "Blink could not find usable web results for “\(plan.query)” yet.", isUser: false))
            return
        }

        if let searchURL = BlinkBrowserResearchRunner.searchURL(for: plan.query) {
            updateActiveTab { tab in
                tab.currentURL = searchURL
                tab.title = "Search: \(plan.query)"
                tab.addressText = searchURL.absoluteString
                tab.errorText = nil
                tab.loadRequest = BlinkBrowserLoadRequest(url: searchURL)
            }
        }

        let openedTabs = outcome.items.map { BlinkBrowserTab(researchItem: $0) }
        ...
        messages.append(BlinkBrowserChatMessage(role: "Blink", text: "Opened \(outcome.items.count) result\(outcome.items.count == 1 ? "" : "s") for “\(plan.query)” in tabs and summarized them here:\n\n\(summary)", isUser: false))
    }

    private func performDirectPageAction(_ action: BlinkBrowserDirectPageAction) {
        if action.kind == .navigate {
            let destination = action.value ?? action.target
            if loadAddress(destination) {
                messages.append(BlinkBrowserChatMessage(role: "Blink", text: "Opened \(addressText) in the active tab.", isUser: false))
            } else {
                messages.append(BlinkBrowserChatMessage(role: "Blink", text: "Blink could not understand that as a URL or local path.", isUser: false))
            }
            return
        }

        guard let webView = webViews[activeTabID] else {
            messages.append(BlinkBrowserChatMessage(role: "Blink", text: "Blink could not act on the page yet because the web view is still loading.", isUser: false))
            return
        }

        guard let script = action.javascript() else {
            messages.append(BlinkBrowserChatMessage(role: "Blink", text: "Blink could not prepare that page action.", isUser: false))
            return
        }
        ...
        webView.evaluateJavaScript(script) { [weak self] result, error in
            DispatchQueue.main.async {
                guard let self else { return }
                if let error {
                    self.messages.append(BlinkBrowserChatMessage(role: "Blink", text: "Blink could not complete that directly on the page: \(error.localizedDescription)", isUser: false))
                    return
                }
                let payload = result as? [String: Any]
                ...
                self.messages.append(
                    BlinkBrowserChatMessage(
                        role: "Blink",
                        text: summary ?? (ok ? "Done on the page." : "Blink could not find a matching page control."),
                        isUser: false
                    )
                )
    ...
    func openLinkedAgentInBlink() {
        guard let linkedAgentSessionID else { return }
        delegate?.selectCodexAgentSession(linkedAgentSessionID)
    }

    private func browserScopedAgentPrompt(userPrompt: String, specialist: BlinkBrowserSpecialist) -> String {
        let currentTab = activeTab
        let pageLabel = currentTab.title.isEmpty ? (currentTab.currentURL?.absoluteString ?? "the local preview") : currentTab.title
        let urlLine = currentTab.currentURL?.absoluteString ?? "open-blink://welcome"
        ...
        return """
        Blink Browser Workspace chat request.
        ...
        Answer as Blink. This is the bigger/background lane; use child workers only for bounded subtasks that materially help, and stay scoped to this browser workspace/page unless the user asks for broader work.
    ...
    private func tab(for tabID: UUID) -> BlinkBrowserTab? {
        tabs.first { $0.id == tabID }
    }

    private func updateActiveTab(_ update: (inout BlinkBrowserTab) -> Void) {
        updateTab(activeTabID, update)
    }

    private func updateTab(_ tabID: UUID, _ update: (inout BlinkBrowserTab) -> Void) {
    ...
    func appendAgentMessage(text: String) {
        messages.append(BlinkBrowserChatMessage(role: "Blink", text: text, isUser: false))
    }

    func updateLastAgentMessage(text: String) {
        if !messages.isEmpty, !messages.last!.isUser {
            messages[messages.count - 1] = BlinkBrowserChatMessage(role: "Blink", text: text, isUser: false)
        } else {
    ...
    <body>
      <main>
        <div class="eyebrow">Blink Local Page</div>
        <h1>Browser workspace prototype</h1>
        <p>This local page proves the left canvas can render local content while the right panel keeps Blink's own chat, specialists, context actions, and composer visible.</p>
        <section class="cards">
          ...
          <article class="card"><div class="icon">💬</div><h3>Collapsible chat</h3><p>Blink's chat copy can collapse to a rail or resize from the divider.</p></article>
...
private enum BlinkChromeCookieImportScope {
    case activeSite
    case all
}

private struct BlinkBrowserDirectPageAction {
    enum Kind: String {
        case click
        case type
        case navigate
    ...
    func javascript() -> String? {
        ...
        (() => {
          ...
          if (action.kind === 'click') {
            ...
            if (!element) return { ok: false, summary: `Blink could not find a visible page control matching “${action.target}”.` };
          ...
          if (action.kind === 'type') {
            ...
            if (!element) return { ok: false, summary: `Blink could not find a visible page field matching “${action.target}”.` };
            focusAndScroll(element);
          ...
          return { ok: false, summary: 'Blink did not recognise that page action.' };
        })();
        """
    }
    ...
    private static func looksLikeNavigableDestination(_ value: String) -> Bool {
        ...
        return lowercased == "open-blink://welcome"
...
private struct BlinkBrowserResearchPlan {
    let query: String
    let resultCount: Int
...
private struct BlinkBrowserResearchItem {
    let title: String
    let url: URL
    let summary: String
...
private struct BlinkBrowserResearchOutcome {
    let items: [BlinkBrowserResearchItem]
    let failureMessage: String?
}

private enum BlinkBrowserResearchRunner {
    ...
    static func run(_ plan: BlinkBrowserResearchPlan) async -> BlinkBrowserResearchOutcome {
        guard let searchURL = searchURL(for: plan.query) else {
            return BlinkBrowserResearchOutcome(items: [], failureMessage: "Blink could not build a search URL for “\(plan.query)”.")
        }
        ...
        do {
            ...
            guard !results.isEmpty else {
                return BlinkBrowserResearchOutcome(items: [], failureMessage: "Blink searched for “\(plan.query)”, but could not extract usable result links yet.")
            }

            var items: [BlinkBrowserResearchItem] = []
            for result in results {
                ...
                items.append(BlinkBrowserResearchItem(title: title, url: result.url, summary: summary, readableText: readable))
            }

            return BlinkBrowserResearchOutcome(items: items, failureMessage: nil)
        } catch {
            return BlinkBrowserResearchOutcome(items: [], failureMessage: "Blink could not complete that browser plan: \(error.localizedDescription)")
        }
    }
    ...
    private static func fetchText(from url: URL, limit: Int) async throws -> String {
        ...
        request.setValue("Mozilla/5.0 AppleWebKit/605.1.15 BlinkBrowserWorkspace/1.0", forHTTPHeaderField: "User-Agent")
    ...
    private static func summaryText(from text: String, fallback: String) -> String {
        ...
        if sentences.isEmpty {
            ...
            return clipped.isEmpty ? "Blink opened this result, but it did not expose readable text." : clipped
        }
...
private struct BlinkChromeProfile: Identifiable, Equatable {
    let id = UUID()
...
private struct BlinkChromeCookieImportResult {
    let imported: Int
    let skipped: Int
    let failed: Int

    var summary: String {
        "Imported \(imported) Chrome cookie\(imported == 1 ? "" : "s") into Blink. Skipped \(skipped), failed \(failed)."
    }
}

private enum BlinkChromeCookieImporter {
    static func discoverProfiles() -> [BlinkChromeProfile] {
        ...
        return profileNames.sorted().compactMap { name in
            ...
            return BlinkChromeProfile(displayName: displayName, profilePath: profilePath, cookiesPath: cookiesPath)
        }
    }

    @MainActor
    static func importCookies(from profile: BlinkChromeProfile, matchingHost host: String?) async -> BlinkChromeCookieImportResult {
        let rows = readCookieRows(from: profile.cookiesPath, matchingHost: host)
        guard !rows.isEmpty else { return BlinkChromeCookieImportResult(imported: 0, skipped: 0, failed: 0) }
        ...
        return BlinkChromeCookieImportResult(imported: imported, skipped: skipped, failed: failed)
    }

    private static func readCookieRows(from cookiesPath: URL, matchingHost host: String?) -> [BlinkChromeCookieRow] {
        let tempURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("BlinkChromeCookies-\(UUID().uuidString).sqlite")
        do {
    ...
    private static func httpCookie(from row: BlinkChromeCookieRow, value: String) -> HTTPCookie? {
    ...
    private static func cookieValue(for row: BlinkChromeCookieRow, chromeKey: [UInt8]?) -> String? {
...
private struct BlinkChromeCookieRow {
    let hostKey: String
    let name: String
    let path: String
...
private struct BlinkBrowserTab: Identifiable, Equatable {
    var id: UUID
    var title: String
    ...
    var loadRequest: BlinkBrowserLoadRequest
    ...
    init(initialURL: URL?) {
        ...
        if let initialURL {
            ...
            self.loadRequest = BlinkBrowserLoadRequest(url: initialURL)
        } else {
            self.currentURL = nil
            self.addressText = "open-blink://welcome"
            self.title = "Blink Browser Workspace"
            self.readableText = BlinkBrowserWorkspaceModel.welcomeHTMLForTabs
            self.readableTextCharacterCount = BlinkBrowserWorkspaceModel.welcomeHTMLCharacterCount
            self.loadRequest = BlinkBrowserLoadRequest(html: BlinkBrowserWorkspaceModel.welcomeHTMLForTabs)
        }
    }

    init(researchItem: BlinkBrowserResearchItem) {
        self.id = UUID()
        ...
        self.loadRequest = BlinkBrowserLoadRequest(url: researchItem.url)
    }

    func duplicateForSplit() -> BlinkBrowserTab {
        var copy = self
        copy.errorText = nil
...
private extension BlinkBrowserWorkspaceModel {
...
private enum BlinkBrowserPanePlacement {
    case single
    case primary
    case secondary
...
private struct BlinkBrowserChatMessage: Identifiable, Equatable {
    let id = UUID()
    let role: String
    let text: String
    let isUser: Bool
...
private struct BlinkBrowserAttachment: Identifiable, Equatable {
    let id = UUID()
    let displayName: String
    let detail: String
...
private struct BlinkBrowserInspectorSelection: Identifiable, Equatable {
    let id = UUID()
    let order: Int
    let selector: String
    let tagName: String
...
private struct BlinkBrowserLoadRequest: Equatable {
    let id = UUID()
    let url: URL?
    let html: String?
...
private struct BlinkBrowserPageMetadata {
    let title: String
    let url: URL?
    let canGoBack: Bool
...
private struct BlinkWorkspaceWebView: NSViewRepresentable {
    let loadRequest: BlinkBrowserLoadRequest
    let onWebViewReady: (WKWebView) -> Void
    let onMetadataChange: (BlinkBrowserPageMetadata) -> Void
    ...
    func makeNSView(context: Context) -> WKWebView {
        ...
        configuration.userContentController.add(context.coordinator, name: "openBlinkInspector")
    ...
    final class Coordinator: NSObject, WKNavigationDelegate, WKScriptMessageHandler {
        ...
        var onMetadataChange: (BlinkBrowserPageMetadata) -> Void
        var onInspectorSelection: ([String: Any]) -> Void

        init(onMetadataChange: @escaping (BlinkBrowserPageMetadata) -> Void, onInspectorSelection: @escaping ([String: Any]) -> Void) {
            self.onMetadataChange = onMetadataChange
            self.onInspectorSelection = onInspectorSelection
        }

        func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
            guard message.name == "openBlinkInspector", let payload = message.body as? [String: Any] else { return }
...
private enum BlinkBrowserSpecialist: String, CaseIterable, Identifiable {
    ...
    func sampleReply(for pageTitle: String) -> String {
        ...
        case .dev:
            return "I can inspect local pages, localhost previews, and implementation notes while keeping Blink's tools on the right."
        }
    }
}
