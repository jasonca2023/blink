...
@MainActor
struct BlinkNextStageParityTests {
    @Test func debugDevModeUsesBlinkIdentityAndDisablesSideEffects() throws {
        #expect(Bundle.main.bundleIdentifier == "com.blink.blink")
        #expect(Bundle.main.object(forInfoDictionaryKey: "CFBundleDisplayName") as? String == "Blink")
        #expect(Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String == "Blink")
        #expect(BlinkRuntimeMode.isDevelopmentBuild == true)
        #expect(BlinkAnalytics.isEnabled == false)
    }


    @Test func wikiManagerIndexesBundledWikiSeedAndSkills() throws {
        let resourcesRoot = URL(fileURLWithPath: "/Users/blink/Documents/GitHub/blink/AppResources/Blink", isDirectory: true)

        let index = try BlinkCore.WikiManager.Index.load(fromBundledResourcesRoot: resourcesRoot)

        #expect(index.articles.contains { $0.relativePath == "wiki/_index.md" && $0.title == "Index" })
        #expect(index.articles.contains { $0.relativePath == "wiki/projects/blink.md" && $0.title.localizedCaseInsensitiveContains("Blink") })
        ...
        #expect(index.article(containingTitle: "Blink")?.body.isEmpty == false)
    }
    ...
    @Test func responseCardsSanitizeAgentFinalMessagesForCursorBubble() throws {
        ...
        #expect(card.displayText.count <= BlinkResponseCard.maximumDisplayCharacters)
