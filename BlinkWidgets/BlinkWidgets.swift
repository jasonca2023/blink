...
private struct BlinkWidgetContainer<Content: View>: View {
    ...
    var body: some View {
        ZStack(alignment: .topTrailing) {
            Circle()
                .fill(BlinkWidgetTheme.accent.opacity(0.28))
            ...
            VStack(alignment: .leading, spacing: contentSpacing) {
                HStack(spacing: 8) {
                    ...
                    Text("LIVE")
                        ...
                        .tracking(0.9)
                        .foregroundStyle(BlinkWidgetTheme.accent)
                        .padding(.horizontal, 7)
                        .padding(.vertical, 4)
                        .background(
                            Capsule()
                                .fill(BlinkWidgetTheme.accent.opacity(0.17))
                                .overlay(
                                    Capsule()
                                        .strokeBorder(BlinkWidgetTheme.accent.opacity(0.34), lineWidth: 1)
                                )
                        )
                }
                ...
                .padding(.vertical, 7)
                .background(BlinkWidgetTheme.headerBackground)
        ...
        .containerBackground(for: .widget) {
            ZStack {
                ...
                RadialGradient(
                    colors: [BlinkWidgetTheme.accent.opacity(0.24), .clear],
                    center: .topTrailing,
                    startRadius: 8,
                    endRadius: 170
                )
    ...
    var body: some View {
        if family == .systemSmall {
            VStack(alignment: .leading, spacing: 8) {
                statLine(value: stats.agentTasksCreated, label: "Agents", color: BlinkWidgetTheme.accent)
            ...
            LazyVGrid(
                ...
                statTile(value: stats.agentTasksCreated, label: "Agents", color: BlinkWidgetTheme.accent, compact: true)
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], alignment: .leading, spacing: 10) {
                statTile(value: stats.agentTasksCreated, label: "Agent tasks", color: BlinkWidgetTheme.accent)
...
private struct BlinkNeedsAttentionWidgetView: View {
    ...
    private func iconName(for kind: BlinkWidgetAttentionItem.Kind) -> String {
        switch kind {
        case .failedAgent:
    ...
    private func color(for kind: BlinkWidgetAttentionItem.Kind) -> Color {
        switch kind {
        case .failedAgent:
            return .red
        case .missingPermission, .missingCredential:
            return .orange
        case .flaggedLog:
            return BlinkWidgetTheme.accent
        case .staleSnapshot:
            return .yellow
        }
    }
...
private struct BlinkWidgetCardStyle: ViewModifier {
        ...
        content
            .background(
                RoundedRectangle(cornerRadius: 15, style: .continuous)
                    .fill(
                        LinearGradient(
                            colors: [
                                .white.opacity(0.045),
                                .white.opacity(0.018),
                                BlinkWidgetTheme.accent.opacity(0.018)
                            ],
                            startPoint: .topLeading,
                            endPoint: .bottomTrailing
                        )
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 15, style: .continuous)
                            .strokeBorder(BlinkWidgetTheme.accent.opacity(0.05), lineWidth: 1)
                            .blur(radius: 0.5)
                    )
...
private struct BlinkWidgetMark: View {
    var body: some View {
        ZStack {
            Circle()
                .fill(BlinkWidgetTheme.accent.opacity(0.18))
            Circle()
                .strokeBorder(BlinkWidgetTheme.accent.opacity(0.55), lineWidth: 1)
            Circle()
                .fill(BlinkWidgetTheme.accent)
