...
final class CodexProcessManager: @unchecked Sendable {
    ...
    /// Ring of the last few non-benign stderr lines from codex. Surfaced in
    /// the terminationHandler error message so users can see WHY codex
    /// exited (missing native binary, bad auth, etc.) instead of just
    /// "exited with status 1".
    private var recentStderrLines: [String] = []
    private static let recentStderrLineCapacity = 8
    private var nextRequestID = 1
    private var pending: [Int: CheckedContinuation<[String: Any], Error>] = [:]
    private let stateQueue = DispatchQueue(label: "com.rishithc.blink.codex-process")
    ...
    func start(executableURL: URL, codexHome: URL) throws {
        ...
        let usesHuggingFaceProvider = configText.contains("model_provider = \"huggingface\"")

        if usesHuggingFaceProvider {
            if environment["HUGGINGFACE_API_KEY"]?.isEmpty != false,
               let configuredHFKey = AppBundleConfiguration.huggingFaceAPIKey(),
               !configuredHFKey.isEmpty {
                environment["HUGGINGFACE_API_KEY"] = configuredHFKey
            }
        } else if prefersChatGPTAuth, FileManager.default.fileExists(atPath: codexAuthFile.path) {
        ...
        process.terminationHandler = { [weak self] terminated in
            let stderrSummary = self?.stateQueue.sync { self?.recentStderrLines.joined(separator: " | ") ?? "" } ?? ""
            let baseMessage = "Codex app-server exited with status \(terminated.terminationStatus)."
            let fullMessage = stderrSummary.isEmpty
                ? baseMessage
                : "\(baseMessage) \(stderrSummary)"
            self?.failAllPendingRequests(message: fullMessage)
        }
    ...
    private static func applyGogCLIEnvironment(to environment: inout [String: String]) {
        ...
        if environment["BLINK_GOG_PATH"]?.isEmpty != false,
           let gogExecutablePath = AppBundleConfiguration.gogExecutablePath() {
            environment["BLINK_GOG_PATH"] = gogExecutablePath
        }
    ...
    @discardableResult
    func initialize(clientName: String = "open-blink", title: String = "Blink", version: String = "1.0.0") async throws -> [String: Any] {
    ...
    static func makeInitializeRequest(clientName: String = "open-blink", title: String = "Blink", version: String = "1.0.0") -> CodexRPCRequest {
    ...
    private func consumeStderr(_ data: Data) {
        ...
        consumeLines(from: &stderrBuffer) { [weak self] line in
            ...
            if !isBenign, let self {
                self.recentStderrLines.append(line)
                if self.recentStderrLines.count > Self.recentStderrLineCapacity {
                    self.recentStderrLines.removeFirst(self.recentStderrLines.count - Self.recentStderrLineCapacity)
                }
            }
            BlinkMessageLogStore.shared.append(
                lane: "agent",
