...
struct BlinkApp: App {
    ...
    var body: some Scene {
        ...
        // one scene, while the app menu command below routes to Blink's full
...
final class CompanionAppDelegate: NSObject, NSApplicationDelegate, SPUUpdaterDelegate, SPUStandardUserDriverDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Terminate any duplicate running instances of Blink to prevent port (error 48) and permission conflicts
        ...
        if let bundleID = currentApp.bundleIdentifier {
            ...
            for app in duplicateApps {
                print("Blink: Terminating duplicate running instance (PID: \(app.processIdentifier)) to free resources/ports.")
                app.terminate()
            }
    ...
    func showBrowserWorkspaceFromApplicationMenu() {
        BlinkBrowserWorkspaceWindowManager.shared.show(delegate: companionManager)
    }
