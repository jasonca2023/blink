...
private final class AppleSpeechTranscriptionSession: NSObject, BuddyStreamingTranscriptionSession {
    ...
    init(
        ...
        // Don't force on-device. `supportsOnDeviceRecognition` returns true
        // as soon as the locale is in principle supported, but the actual
        // on-device model file may not yet be downloaded — in that state
        // the service throws kAFAssistantErrorDomain 1101 the moment audio
        // arrives. Leaving this off lets the framework use the network
        // path (or on-device opportunistically when the model is ready),
        // which is far more reliable for first launches and uncommon
        // locales.
