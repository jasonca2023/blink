...
final class CodexVoiceSession {
        ...
        Task { @MainActor [weak self] in
            ...
            do {
                _ = try await self.sendTurn(
                    ...
                    conversationHistory: [],
                    userPrompt: "get ready. prime the Blink Codex voice response session and reply with ready only.",
                    onTextChunk: { _ in }
                )
    ...
    private func ensureThread() async throws {
        ...
        if !hasInitializedProcess {
            _ = try await processManager.initialize(clientName: "blink-voice", title: "Blink Voice", version: "1.0.0")
        ...
        let baseInstructions = (try? String(contentsOf: layout.modelInstructionsFile, encoding: .utf8))
            ?? "You are Blink, a voice-first macOS cursor companion."
        let developerInstructions = """
        You are Blink's persistent local Codex voice response session.

        Stay fast, concise, and voice-first. Each turn includes the current Blink voice policy, recent conversation, memory context, and any screenshots as localImage input items. Inspect attached localImage inputs directly when screen context matters.

        This voice-response session is not Agent Mode. Do not spawn agents, assign background tasks, or ask for agent confirmation. Simple computer-control requests are handled by Blink's native CUA router before they reach you; clear file, code, log, settings, current-research, and other tool-heavy work should be routed by Blink to Agent Mode automatically. When you do receive a voice-response turn, answer briefly and include Blink's [POINT:x,y:label] tag only when pointing would help.

        Do not run shell commands, Python, find, ls, or other terminal tools from this voice-response session to inspect local files or folders. If the user asks to view, find, inspect, clean up, or open a local file or folder and the request was not handled before it reaches you, do not ask for a special agent phrase; give a short handoff-style acknowledgement that Blink will take care of it in the background.
    ...
    private func ensureCodexAuthentication() async throws {
        ...
        if AppBundleConfiguration.openAIAPIKey() != nil {
            ...
            throw CodexRPCError(message: "Codex ChatGPT login is not available. Blink is falling back to the OpenAI API key.")
        }
        ...
        throw CodexRPCError(message: "Blink found no Codex ChatGPT login. Finish the Codex sign-in that just opened, then try again.")
    }
    ...
    private func makeTemporaryDirectory() throws -> URL {
        let directory = fileManager.temporaryDirectory
            .appendingPathComponent("BlinkCodexVoice", isDirectory: true)
    ...
    private static func composePrompt(
        ...
        if conversationHistory.isEmpty {
            ...
            for entry in conversationHistory {
                ...
                lines.append("Blink: \(entry.assistantResponse)")
            }
