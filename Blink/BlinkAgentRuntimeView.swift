...
/// Full management UI for the isolated blink-agent runtime.
...
struct BlinkAgentRuntimeView: View {
    @StateObject private var manager = BlinkAgentManager.shared
    ...
    private var overviewSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            Text("Blink Agent Runtime")
                .font(.headline)

            Text("Isolated agent engine (blink-agent). Separate from any existing LightClaw installation.")
                .font(.system(size: 12))
    ...
    private var providersSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 8) {
                Button("Sync from Blink settings") {
    ...
    private var cronSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Use the cron tool inside the agent or the TUI (`blink-agent cron list`).")
                .font(.system(size: 11))
        }
    }
    ...
    private var sessionsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Sessions")
                .font(.headline)
            Text("Blink Agent Mode sessions currently held by the Swift app. blink-agent is the execution backend when available; these rows are the UI session wrappers receiving its stream.")
                .font(.caption)
                .foregroundColor(.secondary)
    ...
    private var skillsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Skills")
                .font(.headline)
            Text("SKILL.md discovery and activation. Skills are loaded from ~/Library/Application Support/Blink/agent/workspace/skills/")
                .font(.caption)
                .foregroundColor(.secondary)
        }
    }
    ...
    private var channelsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Configure tokens in the agent config or via `blink-agent configure`.")
                .font(.system(size: 11))
        }
    }
}
private struct BlinkRuntimeSessionRow: View {
