...
enum BuddyTranscriptionProviderFactory {
    ...
    private static func resolveProvider(preferredProvider: BuddyTranscriptionProviderID?) -> any BuddyTranscriptionProvider {
        ...
        let voxtralProvider = VoxtralHFTranscriptionProvider()

        if resolvedPreferredProvider == .appleSpeech {
            return AppleSpeechTranscriptionProvider()
        }

        if resolvedPreferredProvider == .voxtral {
            if voxtralProvider.isConfigured {
                return voxtralProvider
            }
            print("⚠️ Transcription: Voxtral preferred but not configured, falling back")
            return configuredFallback(
                excluding: .voxtral,
                assemblyAIProvider: assemblyAIProvider,
                deepgramProvider: deepgramProvider,
                openAIProvider: openAIProvider,
                voxtralProvider: voxtralProvider
            )
        }
    ...
    private static func configuredFallback(
        ...
        voxtralProvider: VoxtralHFTranscriptionProvider
    ) -> any BuddyTranscriptionProvider {
        if excludedProvider != .voxtral, voxtralProvider.isConfigured {
            print("⚠️ Transcription: using Voxtral as fallback")
            return voxtralProvider
        }
