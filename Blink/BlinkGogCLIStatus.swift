...
struct BlinkGogCLIStatus: Equatable, Sendable {
    ...
    nonisolated static let unknown = BlinkGogCLIStatus(
        isInstalled: false,
        executablePath: nil,
        version: nil,
    ...
    var readinessTitle: String {
        if !isInstalled { return "gogcli not found by Blink" }
    ...
    var readinessDetail: String {
        if !isInstalled { return "Blink could not see gog at /opt/homebrew/bin/gog, /usr/local/bin/gog, or BLINK_GOG_PATH." }
        ...
        return "Credentials are present; run gog auth add for the Google account you want Blink agents to use."
    }
...
enum BlinkGogCLIStatusResolver {
    nonisolated private static let overridePathEnvironmentKey = "BLINK_GOG_PATH"
    ...
    nonisolated static func refresh() async -> BlinkGogCLIStatus {
    ...
    nonisolated private static func refreshSynchronously(fileManager: FileManager) -> BlinkGogCLIStatus {
        var status = BlinkGogCLIStatus.unknown
