...
final class FillerPhraseLibrary {
    ...
    private weak var client: (any BlinkTTSClient)?
    ...
    func prepare(client: any BlinkTTSClient) {
        self.client = client
...
/// client selector.
@MainActor
protocol BlinkTTSClient: AnyObject {
...
extension ElevenLabsTTSClient: BlinkTTSClient {}
extension CartesiaTTSClient: BlinkTTSClient {}

extension StreamingTTSSession {
    /// Builds a streaming session for non-PCM providers (SystemTTSClient).
    /// The sentence handler is responsible for actually voicing the text;
    /// we return zero PCM samples so the session's player-node scheduling
    /// is a no-op while `appendText` / `finish` still drive sentence
    /// cadence through the same StreamingTTSSession state machine.
    static func makeSystemVoiceSession(
        speakSentence: @escaping @Sendable (String) async throws -> Void
    ) -> StreamingTTSSession {
        return StreamingTTSSession(
            fetchSamples: { sentence in
                try await speakSentence(sentence)
                return []
            },
            playerNode: nil,
            format: nil,
...
@MainActor
final class MicrosoftEdgeTTSClient: BlinkTTSClient {
    ...
    nonisolated private static func decodeMP3DataToSamples(_ data: Data) throws -> [Int16] {
        let tempURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("blink-edge-tts-\(UUID().uuidString)")
...
/// when selected, Blink should not also route the same response
/// through ElevenLabs, Cartesia, or Deepgram.
@MainActor
final class OpenAIRealtimeSpeechClient: BlinkTTSClient {
    nonisolated static let streamSampleRate: Double = 24_000
    ...
    func beginBidirectionalVoiceTurn(
        ...
        let historyText = conversationHistory.suffix(8).map { entry in
            "User: \(entry.userPlaceholder)\nBlink: \(entry.assistantResponse)"
        }.joined(separator: "\n\n")
        let instructions = [
            systemPrompt,
            historyText.isEmpty ? nil : "Recent conversation:\n\(historyText)",
            "You are in Blink's bidirectional Realtime voice mode. Listen to the user's live microphone audio directly and reply out loud as Blink in one concise spoken answer. Do not claim you will start background work, take care of a task, or start an agent unless the app already routed the turn before you receive it. Do not mention transcription, Whisper, markdown, or [POINT:] tags."
    ...
    func speakResponse(
        ...
        let instructions = [
            ...
            "Reply out loud as Blink in one concise spoken answer. Do not include markdown. Do not include [POINT:] tags."
        ].compactMap { $0 }.joined(separator: "\n\n")
    ...
    func fetchSentenceSamples(_ text: String) async throws -> [Int16] {
        ...
        try await sendJSON([
            ...
            "response": [
                "conversation": "none",
                "output_modalities": ["audio"],
                "instructions": "Speak exactly this text in a natural Blink voice. Do not add, remove, summarize, or preface anything: \(text)"
            ]
        ], to: webSocket)
    ...
    private final class BidirectionalVoiceTurn {
        ...
        func finish(
            ...
            // Let the Realtime transcription event reach Blink's app
...
/// listen/think/speak loop over one WebSocket: Blink streams PCM
...
final class DeepgramVoiceAgentClient {
    ...
    private nonisolated static func normalizedListenModel(_ model: String) -> String {
        ...
        // defaults when Blink is used as a live realtime conversation.
    ...
    func beginBidirectionalVoiceTurn(
        ...
        let instructions = [
            systemPrompt,
            "You are in Blink's Deepgram Voice Agent realtime mode. Listen to the user's live microphone audio directly and reply out loud as Blink in one concise spoken answer. Do not claim you will start background work, take care of a task, or start an agent unless the app already routed the turn before you receive it. Do not mention transcription, Whisper, markdown, or [POINT:] tags."
        ].compactMap { $0 }.joined(separator: "\n\n")
        ...
        if listenModel.hasPrefix("flux-") {
            ...
            // premature cutoffs in Blink's realtime voice mode.
...
// MARK: - BlinkTTSProvider

enum BlinkTTSProvider: String, CaseIterable, Identifiable {
    ...
    static func resolve(_ raw: String?) -> BlinkTTSProvider {
        guard let raw, let parsed = BlinkTTSProvider(rawValue: raw) else { return .openAITTS }
        return parsed
    }
}

struct MicrosoftEdgeVoiceOption: Identifiable, Hashable {
    let id: String
    let label: String
    let subtitle: String
...
extension DeepgramTTSClient: BlinkTTSClient {}
