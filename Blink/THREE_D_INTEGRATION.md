...
This module adds text-to-3D (low-poly stylized) to Blink. Provider: **Tripo AI**.
...
Blink's "chat" is voice-driven — `CompanionPanelView.swift` is a settings/control panel,
...
| `ThreeDGenerationDispatcher.swift` | Parses `/3d` slash command + `[BLINK_3D]` sentinel |
...
| `ClaudeAgentSDKAPI.swift` | Appended `ThreeDGenerationDispatcher.systemPromptInstruction` to the persistent bridge system prompt — teaches Claude Agent SDK to emit `[BLINK_3D] prompt: "…"` when the user asks for a 3D model. |
...
### 1. Add files to the `Blink` target

In Xcode → File → Add Files to "Blink"… → select all eight new
...
Make sure **Add to targets: Blink** is ticked. The three existing-file
...
`0.4.0` → add the `GLTFSceneKit` library product to the `Blink`
target.
...
1. **UserDefaults** (production path): set `Blink.Tripo3D.APIKey` from
   the Settings UI. Quickest test:
   ```bash
   defaults write com.blink.Blink Blink.Tripo3D.APIKey 'tsk_...'
   ```
...
4. ~30–60s later the bubble swaps to an inline rotatable model. "Show in
   ...
   `~/Library/Application Support/Blink/Generated3D/<task_id>.glb`.
