...
/// Local-only bridge used by other apps/agents to display Blink UI affordances
/// without entering the normal voice/conversation/agent state machine.
struct BlinkExternalCursorSpec {
    var point: CGPoint
    var caption: String?
...
enum BlinkExternalCursorMode: String {
    case primary
    case secondary
}

enum BlinkExternalControlCommand {
    case showCursor(point: CGPoint, caption: String?, duration: TimeInterval, accentHex: String?, mode: BlinkExternalCursorMode, travelDuration: TimeInterval)
    case showCursors([BlinkExternalCursorSpec])
...
struct BlinkExternalControlResponse {
    var statusCode: Int
    var body: [String: Any]

    static func ok(_ body: [String: Any] = [:]) -> BlinkExternalControlResponse {
        BlinkExternalControlResponse(statusCode: 200, body: ["ok": true].merging(body) { _, new in new })
    }

    static func accepted(_ body: [String: Any] = [:]) -> BlinkExternalControlResponse {
        BlinkExternalControlResponse(statusCode: 202, body: ["ok": true, "accepted": true].merging(body) { _, new in new })
    }

    static func error(_ statusCode: Int, _ message: String) -> BlinkExternalControlResponse {
        BlinkExternalControlResponse(statusCode: statusCode, body: ["ok": false, "error": message])
    }

}

typealias BlinkExternalControlHandler = @MainActor (BlinkExternalControlCommand) async -> BlinkExternalControlResponse

final class BlinkExternalControlBridgeServer: @unchecked Sendable {
    private let port: UInt16
    private let handler: BlinkExternalControlHandler
    private let queue = DispatchQueue(label: "com.blink.blink.external-control-bridge")
    private var listener: NWListener?
    private var sseConnections: [UUID: NWConnection] = [:]
    private let proxySession: URLSession

    init(port: UInt16 = 32123, handler: @escaping BlinkExternalControlHandler) {
        self.port = port
    ...
    func start() {
        ...
        do {
            ...
            print("Blink external control bridge could not start: \(error)")
        }
    }
    ...
    private func handle(_ request: HTTPRequest, on connection: NWConnection) {
        ...
        if request.method == "GET", request.path == "/health" || request.path == "/" {
            sendJSON([
                "ok": true,
                "name": "Blink External Control Bridge",
                "port": port,
        ...
        let command: BlinkExternalControlCommand?
        switch request.path {
        case "/cursor":
    ...
    private func proxyInferenceRequest(_ request: HTTPRequest, on connection: NWConnection) {
        ...
        guard let proxyRequest = makeInferenceProxyURLRequest(from: request) else {
            ...
            sendJSON(["ok": false, "error": "Blink \(provider) API key is not configured"], statusCode: 401, on: connection)
            return
        }
    ...
    private static func cursorCommand(from json: [String: Any]) -> BlinkExternalControlCommand? {
    ...
    private static func cursorsCommand(from json: [String: Any]) -> BlinkExternalControlCommand? {
        guard let rawCursors = array(json["cursors"]) else { return nil }
        let fallbackDuration = duration(from: json)
        let specs = rawCursors.compactMap { value -> BlinkExternalCursorSpec? in
            ...
            return BlinkExternalCursorSpec(
                point: point,
    ...
    private static func captionCommand(from json: [String: Any]) -> BlinkExternalControlCommand? {
    ...
    private static func speakCommand(from json: [String: Any]) -> BlinkExternalControlCommand? {
        guard let text = string(json["text"]), !text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { return nil }
        return .speak(text: text, interrupt: bool(json["interrupt"]) ?? false)
    }

    private static func notifyCommand(from json: [String: Any]) -> BlinkExternalControlCommand? {
        let title = string(json["title"]) ?? "Blink"
    ...
    private static var mcpToolDescriptors: [[String: Any]] {
        [
            [
                "name": "show_cursor",
                "description": "Use Blink's native smooth primary-cursor pointing choreography by default, or show a secondary colored cursor when mode=secondary.",
                "inputSchema": [
                    "type": "object",
                    "properties": [
                        "x": ["type": "number"],
            ...
            [
                "name": "show_caption",
                "description": "Show an Blink proxy caption, optionally at macOS screen coordinates.",
                "inputSchema": [
                    "type": "object",
                    "properties": [
                        "text": ["type": "string"],
            ...
            [
                "name": "speak",
                "description": "Speak a short instruction through Blink's TTS without entering dictation or voice-response mode.",
                "inputSchema": [
                    "type": "object",
                    "properties": [
                        "text": ["type": "string"],
            ...
            [
                "name": "notify",
                "description": "Send a native macOS desktop notification from Blink without stealing focus.",
                "inputSchema": [
                    "type": "object",
                    "properties": [
                        "title": ["type": "string"],
            ...
            [
                "name": "clear",
                "description": "Clear the Blink proxy cursor/caption overlay.",
                "inputSchema": ["type": "object", "properties": [:]]
            ]
        ]
    }

    private static func mcpToolCommand(from json: [String: Any]) -> BlinkExternalControlCommand? {
        let tool = string(json["tool"]) ?? string(json["name"])
        let arguments = dictionary(json["arguments"]) ?? dictionary(json["args"]) ?? json
        switch tool {
        case "show_cursor", "blink_show_cursor":
            return cursorCommand(from: arguments)
        case "show_cursors", "blink_show_cursors":
            return cursorsCommand(from: arguments)
        case "show_caption", "blink_show_caption":
            return captionCommand(from: arguments)
        case "screenshot", "screenshots", "capture_screenshot", "blink_screenshot":
            return .captureScreenshot(focused: bool(arguments["focused"]) ?? false)
        case "clear", "blink_clear":
            return .clear
        case "speak", "blink_speak":
            return speakCommand(from: arguments)
        case "notify", "notification", "blink_notify":
    ...
    private static func cursorMode(from json: [String: Any]) -> BlinkExternalCursorMode {
...
private struct MCPJSONRPCBridgeResponse {
    ...
    func responseBody(result: BlinkExternalControlResponse?) -> [String: Any] {
