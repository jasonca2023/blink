//
//  BlinkAutomationsSettingsSection.swift
//  Blink
//
...
//  Blink agent. Persists via BlinkAutomationStore.
//

import SwiftUI

struct BlinkAutomationsSettingsSection: View {
  @ObservedObject var companion: CompanionManager
  @ObservedObject private var store = BlinkAutomationStore.shared
  @ObservedObject private var agentStore = BlinkAgentStore.shared
  @State private var showingNew = false
  @State private var editing: BlinkAutomation?
  ...
  private func row(_ a: BlinkAutomation) -> some View {
...
private struct AutomationEditorSheet: View {
  let initial: BlinkAutomation?
  let agents: [BlinkAgentDefinition]
  var onSave: (BlinkAutomation) -> Void
  ...
  private func commit() {
    let schedule: BlinkAutomationSchedule
    switch kind {
    case .interval:
    ...
    var auto = initial ?? BlinkAutomation(name: name, schedule: schedule, prompt: prompt)
    auto.name = name
    auto.prompt = prompt
