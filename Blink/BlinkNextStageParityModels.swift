...
enum PermissionGuideAssistant {
    ...
    enum StepKind: String, Equatable, CaseIterable {
        ...
        var detail: String {
            switch self {
            case .accessibility:
                return "Lets Blink follow the cursor and respond to the global hotkey."
            case .screenRecording:
                return "Lets Blink capture a screenshot only when you ask for help."
            case .microphone:
                return "Lets Blink hear your push-to-talk request."
            case .screenContent:
    ...
    static func viewState(for snapshot: PermissionSnapshot, entryContext: EntryContext) -> ViewState {
        ...
        let summary: String
        if let primaryStep {
            summary = "Start with \(primaryStep.title). Blink needs all four checks before voice and Agent Mode can run cleanly."
        } else {
            summary = "Blink can listen, see the active screen when invoked, and hand work to Agent Mode."
        }
        return ViewState(headline: headline, summary: summary, steps: steps, primaryStep: primaryStep, entryContext: entryContext)
    }
}

struct BlinkResponseCard: Identifiable, Equatable {
    ...
    static func sanitizedDisplayText(from rawText: String, maximumCharacters: Int = maximumDisplayCharacters) -> String {
        ...
        // Strip bare URLs and "https://… and https://…" reference lists
        // so the response card shows the main report, not the sources.
        text = text.replacingOccurrences(of: #"https?://\S+"#, with: " ", options: .regularExpression)
        text = text.replacingOccurrences(of: #"(?i)\bsources?:.*$"#, with: " ", options: [.regularExpression])
