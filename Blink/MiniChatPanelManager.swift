...
enum ChatWorkspaceArchiveStore {
  ...
  private static let key = "openBlinkArchivedSessions"
  private static let snapshotsKey = "openBlinkArchivedSessionSnapshots"
  private static let relaunchableSnapshotsKey = "openBlinkRelaunchableAgentSessionSnapshots"
...
final class MiniChatPanelManager: NSObject {
  ...
  func show(session: CodexAgentSession, companion: CompanionManager) {
    if let existing = panels[session.id] {
      BlinkWindowLevels.applyPanelDialogLevel(to: existing)
