...
struct BlinkOpenableLink: Identifiable, Hashable {
    let url: URL
...
enum BlinkOpenableLinkExtractor {
    static func links(in text: String, limit: Int = 3) -> [BlinkOpenableLink] {
        ...
        var seen = Set<String>()
        return urls.compactMap { url -> BlinkOpenableLink? in
            let key = url.isFileURL ? url.standardizedFileURL.path : url.absoluteString
            guard seen.insert(key).inserted else { return nil }
            return BlinkOpenableLink(url: url)
        }
        .prefix(limit)
        .map { $0 }
    }
...
struct BlinkAgentDockHoverCard: View {
    ...
    private var linkTargets: [BlinkOpenableLink] {
        let text = ([item.caption ?? ""] + item.activityStatusLines + [item.progressStepText ?? ""])
            .joined(separator: "\n")
        return BlinkOpenableLinkExtractor.links(in: text, limit: 2)
    }
