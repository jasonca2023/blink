//
//  BlinkDesktopNotificationCenter.swift
//  Blink
//
//  Small wrapper around macOS UserNotifications so Blink can send
...
final class BlinkDesktopNotificationCenter: NSObject, UNUserNotificationCenterDelegate {
    static let shared = BlinkDesktopNotificationCenter()

    private let center = UNUserNotificationCenter.current()
    private let logQueue = DispatchQueue(label: "com.blink.blink.desktop-notifications")
    ...
    func post(
        ...
        let trimmedTitle = Self.cleaned(title, fallback: "Blink")
        let trimmedBody = Self.cleaned(body, fallback: "Blink has an update.")
        guard Self.notificationsEnabled else {
            logDeliveryResult(
                event: "blink.desktop_notification.disabled",
                fields: [
        ...
        center.getNotificationSettings { [weak self] settings in
            ...
            case .denied:
                self.logDeliveryResult(
                    event: "blink.desktop_notification.denied",
                    fields: [
            ...
            @unknown default:
                self.logDeliveryResult(
                    event: "blink.desktop_notification.unknown_authorization",
                    fields: [
                        "identifier": request.identifier,
                        "title": title,
                        "bodyLength": bodyLength,
    ...
    private func add(_ request: UNNotificationRequest, title: String, bodyLength: Int) {
        center.add(request) { [weak self] error in
            ...
            if let error {
                ...
                self.logDeliveryResult(event: "blink.desktop_notification.failed", fields: fields)
            } else {
                self.logDeliveryResult(event: "blink.desktop_notification.sent", fields: fields)
            }
        }
    }
    ...
    private func requestAuthorization(reason: String, completion: ((Bool) -> Void)? = nil) {
        center.requestAuthorization(options: [.alert, .sound, .badge]) { [weak self] granted, error in
            ...
            self.logDeliveryResult(event: "blink.desktop_notification.authorization", fields: fields)
            completion?(granted)
        }
    }
    ...
    private func logAuthorizationStatus(_ status: UNAuthorizationStatus, reason: String) {
        logDeliveryResult(
            event: "blink.desktop_notification.authorization_status",
            fields: [
                "reason": reason,
