...
final class BlinkSettingsWindowManager {
    ...
    private var hostingView: NSHostingView<BlinkSettingsView>?
    private var glassBackdrop: BlinkLiquidGlassBackdropView?
    ...
    private func bringSettingsWindowToFront(_ settingsWindow: NSWindow, shouldCenter: Bool) {
        // The main Blink panel uses `.statusBar`, so Settings must sit one
        // level above it instead of the default floating level.
        BlinkWindowLevels.applyPanelDialogLevel(to: settingsWindow)
    ...
    private func createWindow(companionManager: CompanionManager) {
        ...
        let containerView = BlinkGlassContainerView(frame: NSRect(origin: .zero, size: windowSize))
        containerView.autoresizingMask = [.width, .height]
        containerView.wantsLayer = true
        containerView.layer?.backgroundColor = NSColor.clear.cgColor

        let backdrop = BlinkLiquidGlassBackdropView(cornerRadius: 22)
        backdrop.frame = containerView.bounds
        backdrop.autoresizingMask = [.width, .height]
        backdrop.configure(
            cornerRadius: 22,
            roundsTopCorners: true,
            accentColor: BlinkNotchCaptureWindowManager.nsAccentColor(for: nil),
            strength: .expanded
        )
...
struct BlinkSettingsView: View {
    ...
    @AppStorage(AppBundleConfiguration.openBlinkVoicePlaybackVolumeDefaultsKey) private var openBlinkVoicePlaybackVolume = AppBundleConfiguration.voicePlaybackVolume()
        ...
        ZStack {
            ...
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text("Opacity and frosting update immediately here and on Blink panels.")
        ...
        VStack(alignment: .leading, spacing: 14) {
            settingsGroup("Companion") {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    HStack {
                        ...
                        Picker("", selection: $blinkTheme) {
                            ForEach(BlinkTheme.allCases) { theme in
                ...
                editableFieldRow(
                    ...
                    subtitle: "Adjusts the native Liquid Glass tint intensity used by Blink glass surfaces.",
            ...
            settingsGroup("Typography") {
                Picker("App font", selection: $appFontRawValue) {
                    ForEach(BlinkResponseCaptionFont.allCases) { appFont in
                ...
                toggleRow(
                    ...
                    subtitle: "Makes normal Blink labels and messages use a stronger weight.",
            ...
            settingsGroup("Cursor appearance") {
                VStack(alignment: .leading, spacing: 14) {
                    Text("Pick Blink’s cursor buddy and accent color. Pets ignore the color tint, but the accent still drives glows, buttons, and task badges.")
                        .font(appUIFont(size: subtextFontSize, weight: .regular))
                        .foregroundColor(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                    ...
                    LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 10), count: 4), spacing: 10) {
                        ForEach([BlinkAccentTheme.rose, .blue, .amber, .mint, .white]) { accentTheme in
    ...
    private var voicePanel: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            settingsGroup("Response voice model") {
                Text("Pick Realtime when one model should listen and speak live, or use a normal model when Blink should think first and hand the reply to a playback engine.")
                    .font(appUIFont(size: subtextFontSize, weight: .regular))
                    .foregroundColor(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            ...
            settingsGroup("Playback") {
                Text(BlinkModelCatalog.isSpeechModelID(companionManager.selectedModel)
                ...
                editableFieldRow(
                    ...
                    HStack(spacing: 10) {
                        Slider(
                            value: Binding(
                                get: { openBlinkVoicePlaybackVolume },
                                set: { openBlinkVoicePlaybackVolume = min(max($0, 0.0), 1.0) }
                            ),
                            in: 0...1
                        )
                        Text("\(Int((openBlinkVoicePlaybackVolume * 100).rounded()))%")
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            settingsGroup("Desktop notifications") {
                ...
                toggleRow(
                    ...
                    subtitle: "Shows native macOS banners when Blink background agents finish, stop, or are cancelled.",
                    ...
                    isOn: Binding(
                        ...
                        set: { newValue in
                            ...
                            if newValue {
                                BlinkDesktopNotificationCenter.shared.requestAuthorizationForUserAction { _ in
                                    refreshNotificationAuthorizationStatus()
                                }
                            }
                        }
                ...
                actionRow(title: "Send test notification", systemImageName: "bell") {
                    desktopNotificationsEnabled = true
                    BlinkDesktopNotificationCenter.shared.postTestNotification()
                    refreshNotificationAuthorizationStatus()
                }
            }
    ...
    private var agentsPanel: some View {
        BlinkAgentsSettingsSection(companion: companionManager)
    }

    private var automationsPanel: some View {
        BlinkAutomationsSettingsSection(companion: companionManager)
    }
    ...
    private var mcpCuaDriverStatusText: String {
        guard mcpComputerUseEnabled else {
            return "Disabled. Turn this on to expose Blink's computer-use path to Agent Mode."
        }
    ...
    private func openSettingsFile(_ url: URL) {
        if ["md", "markdown", "mdown", "mkd"].contains(url.pathExtension.lowercased()) {
            companionManager.openBlinkDocument(url)
            return
        }
    ...
    private func settingsPathOpenHelpText(for rawPath: String) -> String {
        ...
        if ["md", "markdown", "mdown", "mkd"].contains(URL(fileURLWithPath: path).pathExtension.lowercased()) {
            return "Open in Blink Markdown viewer"
        }
        return "Open in TextEdit"
    }

    private func refreshNotificationAuthorizationStatus() {
        BlinkDesktopNotificationCenter.shared.refreshAuthorizationStatus { summary, isGranted in
            ...
            HStack(alignment: .firstTextBaseline, spacing: 10) {
                VStack(alignment: .leading, spacing: 3) {
                    Text("OpenPets gallery")
                        .font(appUIFont(size: bodyFontSize, weight: .semibold))
                    Text("Browse installable pets from openpets.dev. Installed packs land in the same local pet library Blink already watches.")
                        .font(appUIFont(size: subtextFontSize, weight: .regular))
                        .foregroundColor(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                }
    ...
    private func openPetsCatalogCard(_ pet: OpenPetsCatalogPet) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 8) {
            ...
            Button {
                if isInstalled {
                    avatarStyleRawValue = BlinkCursorAvatarStyle.pet(id: pet.id).storageValue
                } else {
