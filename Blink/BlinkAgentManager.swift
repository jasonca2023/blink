...
/// Manages the isolated blink-agent runtime (rebranded LightClaw).
...
@MainActor
final class BlinkAgentManager: ObservableObject {
    static let shared = BlinkAgentManager()

    private let logger = Logger(subsystem: "com.blink.blink", category: "AgentRuntime")
    ...
    var agentBinaryName: String { "blink-agent" }

    var supportDirectory: URL {
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        return appSupport.appendingPathComponent("Blink/agent", isDirectory: true)
    }
    ...
    var launchAgentPlistPath: URL {
        ...
        return launchAgents.appendingPathComponent("com.blink.blink.agent.plist")
    }
    ...
    func locateBinary() -> URL? {
        ...
        if let bundled = Bundle.main.url(forResource: "blink-agent", withExtension: nil) {
            return bundled
        }

        // 2. Next to the app bundle (development)
        let appDir = Bundle.main.bundleURL.deletingLastPathComponent()
        let devBinary = appDir.appendingPathComponent("blink-agent")
        ...
        if let path = ProcessInfo.processInfo.environment["PATH"] {
            for dir in path.split(separator: ":") {
                let candidate = URL(fileURLWithPath: String(dir)).appendingPathComponent("blink-agent")
        ...
        // 4. ~/bin
        let homeBin = FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("bin/blink-agent")
    ...
    func installService() throws {
        guard let binary = locateBinary() else {
            throw NSError(domain: "BlinkAgent", code: 1, userInfo: [NSLocalizedDescriptionKey: "blink-agent binary not found"])
        }

        // Make sure provider keys from Blink settings are in the daemon
        ...
        logger.info("Installed and loaded blink-agent service")
        status = .running
    }
    ...
    nonisolated static func performInstall(binaryPath: String, plistPath: String, supportDir: URL) throws {
        let installLog = Logger(subsystem: "com.blink.blink", category: "AgentRuntime")
        installLog.info("performInstall: writing plist to \(plistPath, privacy: .public)")
        let plist: [String: Any] = [
            "Label": "com.blink.blink.agent",
            ...
            "EnvironmentVariables": [
                "BLINK_AGENT_DATA_DIR": supportDir.path
            ]
        ]
    ...
    func uninstallService() throws {
        ...
        status = .notInstalled
        logger.info("Uninstalled blink-agent service")
    }
    ...
    func refreshStatus() {
        ...
        let process = Process()
        process.executableURL = URL(fileURLWithPath: "/bin/launchctl")
        process.arguments = ["list", "com.blink.blink.agent"]
    ...
    func sendRequest(_ request: AgentRequest) async throws -> AgentResponse {
        ...
        guard FileManager.default.fileExists(atPath: socketPath) else {
            throw NSError(domain: "BlinkAgent", code: 2, userInfo: [NSLocalizedDescriptionKey: "Agent IPC socket not found. Is the service running?"])
        }
    ...
    nonisolated private static func streamChatInternal(
        ...
        guard FileManager.default.fileExists(atPath: socketPath) else {
            throw NSError(domain: "BlinkAgent", code: 2, userInfo: [NSLocalizedDescriptionKey: "Agent IPC socket not found"])
        }
        ...
        while true {
            ...
            guard let chunk else {
                ...
                throw NSError(
                    domain: "BlinkAgent",
                    code: 6,
                    userInfo: [NSLocalizedDescriptionKey: "blink-agent did not produce a response within \(Int(limit)) seconds."]
                )
            }
        ...
        if receivedPayload {
            throw NSError(domain: "BlinkAgent", code: 7, userInfo: [NSLocalizedDescriptionKey: "blink-agent closed the IPC stream before sending a completion marker."])
        }

        throw NSError(domain: "BlinkAgent", code: 8, userInfo: [NSLocalizedDescriptionKey: "blink-agent accepted the request but returned no response."])
    }
    ...
    nonisolated private static func handleStreamLine(
        ...
        if let type = json["type"] as? String {
            ...
            if type == "error" {
                let message = json["error"] as? String ?? json["message"] as? String ?? "blink-agent stream failed"
                throw NSError(domain: "BlinkAgent", code: 4, userInfo: [NSLocalizedDescriptionKey: message])
            }
        }

        if let error = json["error"] as? String, !error.isEmpty {
            throw NSError(domain: "BlinkAgent", code: 4, userInfo: [NSLocalizedDescriptionKey: error])
        }
        ...
        if let status = json["status"] as? String,
           status.lowercased() != "ok" && status.lowercased() != "success" {
            throw NSError(domain: "BlinkAgent", code: 4, userInfo: [NSLocalizedDescriptionKey: "blink-agent returned status: \(status)"])
        }
    ...
    nonisolated private static func readChunk(from handle: FileHandle, timeout: TimeInterval) throws -> Data? {
        ...
        while true {
            ...
            if pollResult < 0 {
                ...
                throw posixSocketError("Could not read blink-agent IPC socket")
            }
    ...
    nonisolated private static func openConnectedSocketHandle(path socketPath: String) throws -> FileHandle {
        ...
        guard connectResult == 0 else {
            let error = Self.posixSocketError("Could not connect to blink-agent IPC socket")
            Darwin.close(fd)
            throw error
        }
        ...
        while true {
            ...
            guard let chunk else {
                throw NSError(
                    domain: "BlinkAgent",
                    code: 6,
                    userInfo: [NSLocalizedDescriptionKey: "blink-agent did not produce a response within \(Int(streamReadIdleTimeout)) seconds."]
                )
            }
            if chunk.isEmpty { break }
            buffer.append(chunk)
    ...
    func ensureRunning(progress: (@MainActor (String) -> Void)? = nil) async throws {
        ...
        guard let binary = locateBinary() else {
            logger.error("ensureRunning: blink-agent binary not found in bundle, app dir, PATH, or ~/bin")
            throw NSError(domain: "BlinkAgent", code: 1, userInfo: [NSLocalizedDescriptionKey: "blink-agent binary not found"])
        }
        ...
        try await Task.detached(priority: .userInitiated) {
            ...
            if plistExists {
                report("Kickstarting blink-agent via launchctl")
                let kickstart = (try? Self.runLaunchctlStatic(["kickstart", "-k", "gui/\(getuid())/com.blink.blink.agent"])) ?? -1
                let startCmd = (try? Self.runLaunchctlStatic(["start", "com.blink.blink.agent"])) ?? -1
            ...
            report("Installing blink-agent launch service")
            do {
            ...
            report("Waiting for blink-agent IPC socket")
            ...
            throw NSError(
                domain: "BlinkAgent",
                code: 5,
                userInfo: [NSLocalizedDescriptionKey: "blink-agent service started but its IPC socket did not become available."]
            )
        }.value
    ...
    func restartService(reason: String) async {
        logger.warning("Restarting blink-agent service: \(reason, privacy: .public)")

        guard let binary = locateBinary() else {
            status = .error("blink-agent binary not found")
            return
        }
        ...
        let result: ServiceStatus = await Task.detached(priority: .userInitiated) {
            _ = try? Self.runLaunchctlStatic(["bootout", "gui/\(getuid())/com.blink.blink.agent"])
            ...
            do {
                if FileManager.default.fileExists(atPath: plistPath) {
                    ...
                    _ = try? Self.runLaunchctlStatic(["kickstart", "-k", "gui/\(getuid())/com.blink.blink.agent"])
                } else {
                    try Self.performInstall(binaryPath: binaryPath, plistPath: plistPath, supportDir: supportDir)
                }
                ...
                if Self.waitForRunningSocketStatic(path: socketPath, timeout: 5.0) {
                    ...
                    return .error("blink-agent restart did not expose its IPC socket.")
                }
            } catch {
    ...
    func stopService() async {
        ...
        status = .stopped
        logger.info("Stopped blink-agent service (plist retained)")
    }
    ...
    nonisolated private static func agentProcesses(modes: Set<String>) -> [(pid: pid_t, command: String)] {
        ...
        do {
            ...
            return output.split(separator: "\n").compactMap { line -> (pid: pid_t, command: String)? in
                ...
                guard command.contains("blink-agent") else { return nil }
                guard modes.contains(where: { mode in
                    command.hasSuffix("blink-agent \(mode)")
                        || command.contains("/blink-agent \(mode)")
                        || command.contains(" blink-agent \(mode)")
                }) else { return nil }
                return (pid, command)
            }
    ...
    /// Reads provider keys from Blink's user settings and writes/merges them
    ...
    func syncProvidersFromSettings() -> Bool {
        ...
        // Mirror Blink's delegation model into the agent runtime so the
        ...
        if currentModel == nil {
            ...
            if effectiveProvider == "anthropic" {
                defaults["model"] = "anthropic/\(BlinkModelCatalog.defaultDelegationModelID)"
                changed = true
            }
        }
