//
//  BlinkNotchCaptureWindowManager.swift
//  Blink
//
//  Top-of-screen capture surface for Blink. It anchors beneath the
...
//  older floating text interface and mirrors Blink's top listening affordance.
//
...
private final class BlinkNotchCapturePanel: NSPanel {
    override var canBecomeKey: Bool { true }
    override var canBecomeMain: Bool { false }
}

enum BlinkWindowLevels {
...
private enum BlinkNotchCaptureAction: String, CaseIterable {
    case ask
    case aiText
    case agent
...
private struct BlinkNotchCaptureSuggestion {
    enum Kind {
        case slashCommand
        case mention
    ...
    static func candidates(for kind: Kind) -> [BlinkNotchCaptureSuggestion] {
        switch kind {
        case .slashCommand:
            return [
                BlinkNotchCaptureSuggestion(token: "/agent", title: "start background work", systemImage: "shippingbox"),
                BlinkNotchCaptureSuggestion(token: "/voice", title: "use shared voice context", systemImage: "waveform"),
                BlinkNotchCaptureSuggestion(token: "/screen", title: "include screen context", systemImage: "rectangle.dashed"),
                BlinkNotchCaptureSuggestion(token: "/workspace", title: "current workspace", systemImage: "folder"),
                BlinkNotchCaptureSuggestion(token: "/skills", title: "available skills", systemImage: "sparkles"),
                BlinkNotchCaptureSuggestion(token: "/tools", title: "available tools", systemImage: "wrench.and.screwdriver")
            ]
        case .mention:
            return [
                BlinkNotchCaptureSuggestion(token: "@agent", title: "background agent", systemImage: "shippingbox"),
                BlinkNotchCaptureSuggestion(token: "@voice", title: "main voice thread", systemImage: "waveform"),
                BlinkNotchCaptureSuggestion(token: "@screen", title: "visible screen", systemImage: "rectangle.dashed"),
                BlinkNotchCaptureSuggestion(token: "@workspace", title: "current workspace", systemImage: "folder"),
                BlinkNotchCaptureSuggestion(token: "@skills", title: "skills", systemImage: "sparkles"),
                BlinkNotchCaptureSuggestion(token: "@tools", title: "tools", systemImage: "wrench.and.screwdriver")
            ]
        }
    }
}
...
@MainActor
final class BlinkNotchCaptureWindowManager {
    ...
    private var panel: BlinkNotchCapturePanel?
    private var mainPanel: BlinkNotchCapturePanel?
    private var contentView: BlinkNotchCaptureRootView?
    private var mirroredStatusPanels: [CGDirectDisplayID: BlinkNotchCapturePanel] = [:]
    private var mirroredStatusContentViews: [CGDirectDisplayID: BlinkNotchCaptureRootView] = [:]
    private var mainHostingView: NSHostingView<BlinkNotchPanelView>?
    private var mainPanelGlassBackdrop: BlinkLiquidGlassBackdropView?
    ...
    private var mainPanelCurrentMinimumSize = BlinkNotchCaptureWindowManager.mainPanelMinimumSize
    ...
    private static let userPillFramesDefaultsKey = "BlinkUserPillFrames"
    ...
    init() {
        ...
        mainPanelContentSizeObserver = NotificationCenter.default.addObserver(
            forName: .blinkPanelContentSizeDidChange,
            object: nil,
            queue: .main
    ...
    func showTextInput(accentTheme: BlinkAccentTheme? = nil, submitText: @escaping (String) -> Void) {
        let accentColor = Self.nsAccentColor(for: accentTheme)
        expandTextInput(accentColor: accentColor, submitText: submitText)
    }

    func updateVoiceState(_ voicePhase: BlinkNotchVoicePhase, audioPowerLevel: CGFloat) {
        switch voicePhase {
        case .idle:
            if activeMode == .voice {
    ...
    private func showMainPanel(companionManager: CompanionManager, focusedAgentSessionID: UUID? = nil) {
        ...
        let notchPanelView = BlinkNotchPanelView(
            companionManager: companionManager,
        ...
        let hostingView = BlinkMainPanelHostingView(rootView: notchPanelView)
        ...
        let resizeContainer = BlinkMainPanelResizeContainerView(frame: NSRect(origin: .zero, size: initialSize))
        ...
        resizeContainer.onResizeBegan = { [weak self] in
            ...
            NotificationCenter.default.post(
                name: .blinkMainPanelResizeStateDidChange,
                object: nil,
    ...
    private func syncMirroredStatusPanels(
        ...
        configure: (BlinkNotchCaptureRootView, NSScreen) -> Void
    ) {
    ...
    private func mirroredStatusPanel(for screen: NSScreen, width: CGFloat, height: CGFloat) -> BlinkNotchCapturePanel {
        if let existing = mirroredStatusPanels[screen.displayID] {
            return existing
        }
        let statusPanel = Self.makeStatusPanel(width: width, height: height)
        let rootView = BlinkNotchCaptureRootView(frame: NSRect(x: 0, y: 0, width: width, height: height))
    ...
    private static func makeStatusPanel(width: CGFloat, height: CGFloat) -> BlinkNotchCapturePanel {
        let capturePanel = BlinkNotchCapturePanel(
    ...
    private func ensureMainPanel() {
        ...
        let interfacePanel = BlinkNotchCapturePanel(
    ...
    private func ensureStatsHUD() -> NSPanel {
        ...
        let container = BlinkRoundedView(cornerRadius: 10)
    ...
    private func resizeVisibleMainPanelToCurrentContent(animated: Bool) {
        ...
        if isMainPanelUserResizing || (mainPanel.contentView as? BlinkMainPanelResizeContainerView)?.isUserResizing == true { return }
    ...
    private func applyMainPanelResizeBehavior() {
        ...
        if isMainPanelPinned {
            ...
            // handled entirely by Blink's edge/corner container below;
    ...
    private static func preferredAnchorScreen() -> NSScreen? {
        ...
        if let keyWindow = NSApp.keyWindow,
           !(keyWindow is BlinkNotchCapturePanel),
           let keyWindowScreen = keyWindow.screen {
            return keyWindowScreen
        }
        if let mainWindow = NSApp.mainWindow,
           !(mainWindow is BlinkNotchCapturePanel),
    ...
    static func nsAccentColor(for theme: BlinkAccentTheme?) -> NSColor {
        switch theme ?? BlinkAccentTheme.current {
        case .blue:
...
private final class BlinkNotchCaptureRootView: NSView, NSTextFieldDelegate {
    private enum Mode {
        case collapsed
        case text
        case voice
    ...
    private let notchHandle = BlinkRoundedView(cornerRadius: 5)
    fileprivate let shellGlassView = BlinkLiquidGlassBackdropView(cornerRadius: 24)
    fileprivate let shellView = BlinkRoundedView(cornerRadius: 24)
    ...
    private let rightEdgeGradientView = BlinkNotchRightEdgeGradientView()
    private let textStack = NSStackView()
    private let voiceStack = NSStackView()
    private let titleLabel = NSTextField(labelWithString: "Ask Blink")
    ...
    private let inputShell = BlinkRoundedView(cornerRadius: 21)
    ...
    private let waveformView = BlinkNotchWaveformNSView()
    private let collapsedAppIconView = NSImageView()
    private let collapsedPlayIconView = NSImageView()
    private let collapsedAgentDotsView = BlinkNotchDotsNSView()
    ...
    private func updateShellViewFillColor() {
        switch mode {
        case .collapsed:
            shellView.fillColor = BlinkLiquidGlassBackdropView.isLiquidGlassAvailable ? NSColor.black.withAlphaComponent(0.34) : NSColor.black.withAlphaComponent(0.93)
        case .text:
            shellView.fillColor = BlinkLiquidGlassBackdropView.isLiquidGlassAvailable ? NSColor.black.withAlphaComponent(0.44) : NSColor(calibratedRed: 0.025, green: 0.036, blue: 0.032, alpha: 0.96)
        case .voice:
            shellView.fillColor = BlinkLiquidGlassBackdropView.isLiquidGlassAvailable ? NSColor.black.withAlphaComponent(0.30) : NSColor.black.withAlphaComponent(0.91)
        }
        needsDisplay = true
    }
    ...
    override func updateTrackingAreas() {
        ...
        if let screen = window?.screen, BlinkNotchCaptureWindowManager.hasPhysicalNotch(on: screen) {
    ...
    private func updateShellConstraints(animated: Bool) {
        ...
        if let screen = window.screen, BlinkNotchCaptureWindowManager.hasPhysicalNotch(on: screen) {
            ...
            notchWidth = BlinkNotchCaptureWindowManager.physicalNotchWidth(on: screen) ?? 172
    ...
    private func buildViewHierarchy() {
        ...
        // BlinkNotchCaptureRootView.mouseDown). A single source of truth
    ...
    private func configureForegroundAppIconViews() {
        ...
        collapsedPlayIconView.image = NSImage(systemSymbolName: "play.fill", accessibilityDescription: "Open Blink")
    ...
    private func configureTextStack() {
        ...
        for action in BlinkNotchCaptureAction.allCases {
    ...
    private func configureInputRow() {
        ...
        textField.placeholderString = "Ask Blink…"
        textField.delegate = self
        row.addArrangedSubview(textField)

        row.addArrangedSubview(makeGlyphButton(systemName: "arrow.up.circle.fill", tooltip: "Send Blink prompt", action: { [weak self] in self?.submit(.ask) }))
        row.addArrangedSubview(makeGlyphButton(systemName: "xmark.circle.fill", tooltip: "Close Blink capture", action: { [weak self] in self?.dismiss?() }))
    ...
    private func makeIconOrb(systemName: String) -> NSView {
        let container = BlinkRoundedView(cornerRadius: 17)
        container.translatesAutoresizingMaskIntoConstraints = false
    ...
    private func makeSmallIcon(systemName: String) -> NSView {
        let container = BlinkRoundedView(cornerRadius: 12)
        container.translatesAutoresizingMaskIntoConstraints = false
    ...
    private func makeGlyphButton(systemName: String, tooltip: String, action: @escaping () -> Void) -> NSButton {
        let button = BlinkClosureButton(systemName: systemName, title: nil, action: action)
    ...
    private func makeActionButton(_ action: BlinkNotchCaptureAction) -> NSView {
        let button = BlinkActionPillButton(
            title: action.title,
    ...
    private func makeSuggestionButton(_ suggestion: BlinkNotchCaptureSuggestion) -> NSButton {
        let button = BlinkClosureButton(systemName: suggestion.systemImage, title: "\(suggestion.token)  \(suggestion.title)", action: { [weak self] in
    ...
    private func updateButtons(in stack: NSStackView) {
        for case let button as BlinkActionPillButton in stack.arrangedSubviews {
            button.accentColor = accentColor
        }
    }

    private func updateVoiceLabels(for phase: BlinkNotchVoicePhase, foregroundAppName _: String, hidesStatusText: Bool) {
    ...
    private func updateSuggestions() {
        ...
        let candidates = BlinkNotchCaptureSuggestion.candidates(for: trigger.kind)
    ...
    private func submit(_ action: BlinkNotchCaptureAction) {
    ...
    private func routedText(for action: BlinkNotchCaptureAction) -> String {
    ...
    private func applySuggestion(_ suggestion: BlinkNotchCaptureSuggestion) {
    ...
    private func currentTrigger() -> (kind: BlinkNotchCaptureSuggestion.Kind, query: String, range: Range<String.Index>)? {
        ...
        guard let first = token.first,
              let kind = BlinkNotchCaptureSuggestion.Kind(trigger: first) else {
            return nil
        }
    ...
    override func hitTest(_ point: NSPoint) -> NSView? {
        ...
        if let window = window, let screen = window.screen, BlinkNotchCaptureWindowManager.hasPhysicalNotch(on: screen) {
...
final class BlinkLiquidGlassBackdropView: NSView {
    enum Strength {
        case compact
        case expanded
    }
...
@MainActor
enum BlinkLiquidGlassWindowSurface {
    @discardableResult
    static func install<Content: View>(
        ...
        backdrop.configure(
            cornerRadius: cornerRadius,
            roundsTopCorners: roundsTopCorners,
            accentColor: accentColor ?? BlinkNotchCaptureWindowManager.nsAccentColor(for: nil),
            strength: strength
        )
...
final class BlinkGlassContainerView: NSView {
    override func hitTest(_ point: NSPoint) -> NSView? {
...
private final class BlinkNotchRightEdgeGradientView: NSView {
...
private final class BlinkRoundedView: NSView {
...
private final class BlinkActionPillButton: NSControl {
...
private final class BlinkClosureButton: NSButton {
    var onAction: (() -> Void)?
    var fillColor: NSColor = .clear { didSet { needsDisplay = true } }
...
private final class BlinkMainPanelHostingView<Content: View>: NSHostingView<Content> {
...
private final class BlinkMainPanelResizeContainerView: NSView {
    ...
    private let resizeHandleView = BlinkMainPanelResizeHandleView(frame: .zero)
...
private final class BlinkMainPanelResizeHandleView: NSView {
    override var isFlipped: Bool { true }
...
private final class BlinkNotchWaveformNSView: NSView {
...
private final class BlinkNotchDotsNSView: NSView {
    var accentColor: NSColor = NSColor(calibratedRed: 0.20, green: 0.50, blue: 1.00, alpha: 1.0) { didSet { needsDisplay = true } }
    var isActive = false { didSet { needsDisplay = true } }
