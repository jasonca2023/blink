...
extension Notification.Name {
    static let blinkDismissPanel = Notification.Name("blinkDismissPanel")
    static let blinkShowPanel = Notification.Name("blinkShowPanel")
    static let blinkPanelContentSizeDidChange = Notification.Name("blinkPanelContentSizeDidChange")
    static let blinkMainPanelResizeStateDidChange = Notification.Name("blinkMainPanelResizeStateDidChange")
}
...
final class MenuBarPanelManager: NSObject {
    ...
    private func createStatusItem() {
        ...
        button.image = makeBlinkMenuBarIcon()
    ...
    private func makeBlinkMenuBarIcon() -> NSImage {
    ...
    private func showStatusItemContextMenu(from sender: NSStatusBarButton) {
        ...
        let quickItem = NSMenuItem(
            title: "Quick Ask Blink",
            action: #selector(quickAskBlinkFromStatusMenu),
            keyEquivalent: ""
        )
    ...
    @objc private func quickAskBlinkFromStatusMenu() {
    ...
    private func createPanel() {
        ...
        BlinkWindowLevels.applyPanelDialogLevel(to: menuBarPanel)
        ...
        let containerView = BlinkGlassContainerView(frame: NSRect(x: 0, y: 0, width: panelWidth, height: panelHeight))
        containerView.autoresizingMask = [.width, .height]
        containerView.wantsLayer = true
        containerView.layer?.backgroundColor = .clear

        let backdrop = BlinkLiquidGlassBackdropView(cornerRadius: 28)
...
final class AgentMenuBarStatusManager: NSObject {
    ...
    private var latestItemsByID: [UUID: BlinkAgentDockItem] = [:]
    ...
    private func menuBarItems(from companionManager: CompanionManager) -> [BlinkAgentDockItem] {
        var dockItemsBySessionID: [UUID: BlinkAgentDockItem] = [:]
        ...
        let sessionItems = companionManager.codexAgentSessions
            ...
            .map { session -> BlinkAgentDockItem in
                if let existingItem = dockItemsBySessionID[session.id] {
                    return existingItem
                }

                return BlinkAgentDockItem(
                    id: session.id,
    ...
    private static func dockStatus(for status: CodexAgentSessionStatus) -> BlinkAgentDockStatus {
        switch status {
        case .starting:
            return .starting
        case .running:
    ...
    private func makeStatusItem(for item: BlinkAgentDockItem) -> NSStatusItem {
    ...
    private func update(statusItem: NSStatusItem, with item: BlinkAgentDockItem) {
        guard let button = statusItem.button else { return }
    ...
    private func handleAgentStatusItemClick(item: BlinkAgentDockItem, from sender: NSStatusBarButton, isRightClick: Bool) {
        let itemID = item.id
        ...
        let rootView = BlinkAgentDockHoverCard(
            item: item,
    ...
    private func openMenuBarAgent(_ item: BlinkAgentDockItem, companionManager: CompanionManager?) {
        guard let companionManager else { return }
    ...
    private func openMenuBarAgentTextFollowUp(_ item: BlinkAgentDockItem, companionManager: CompanionManager?) {
        guard let companionManager else { return }
        if companionManager.agentDockItems.contains(where: { $0.id == item.id }) {
    ...
    private func stopMenuBarAgent(_ item: BlinkAgentDockItem, companionManager: CompanionManager?) {
        guard let companionManager else { return }
        if companionManager.agentDockItems.contains(where: { $0.id == item.id }) {
    ...
    private func dismissMenuBarAgent(_ item: BlinkAgentDockItem, companionManager: CompanionManager?) {
        guard let companionManager else { return }
        if companionManager.agentDockItems.contains(where: { $0.id == item.id }) {
    ...
    private func installDropTarget(on button: NSStatusBarButton, itemID: UUID) {
        let targetIdentifier = NSUserInterfaceItemIdentifier("BlinkAgentStatusDropTarget")
    ...
    private func showAgentContextMenu(for item: BlinkAgentDockItem, from sender: NSStatusBarButton) {
        let menu = NSMenu()
    ...
    private func agentHistoryMenuItem() -> NSMenuItem {
        ...
        guard let companionManager else {
            let emptyItem = NSMenuItem(title: "Blink is not ready", action: nil, keyEquivalent: "")
            emptyItem.isEnabled = false
            submenu.addItem(emptyItem)
            historyItem.submenu = submenu
            return historyItem
    ...
    private func tooltip(for item: BlinkAgentDockItem) -> String {
        let status: String
        switch item.status {
    ...
    private func makeAgentStatusIcon(theme: BlinkAccentTheme, status: BlinkAgentDockStatus) -> NSImage {
        let size: CGFloat = 18
    ...
    private static func nsColor(for theme: BlinkAccentTheme) -> NSColor {
        switch theme {
