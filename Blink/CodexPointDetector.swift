...
//  Blink
//
//  Uses the local Codex runtime and image input to produce Blink
//  [POINT:x,y:label] tags without requiring an OpenAI API key in Blink.
//
...
final class CodexPointDetector {
    ...
    func analyzeImageResponse(
        ...
        for entry in conversationHistory {
            ...
            historyLines.append("Blink: \(entry.assistantResponse)")
        }
    ...
    func detectDisplayLocalPoint(
        ...
        let systemPrompt = """
        You are Blink's screen pointing detector. Look at the screenshot and identify the single visible UI element that best answers the user's request. If the request is conceptual or no visible target helps, return [POINT:none].
    ...
    private func makeTemporaryDirectory() throws -> URL {
        let directory = fileManager.temporaryDirectory
            .appendingPathComponent("BlinkCodexPointDetector", isDirectory: true)
