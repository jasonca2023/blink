...
struct CompanionPanelView: View {
    ...
    private var mainPanelContent: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if !companionManager.allPermissionsGranted && !isPermissionOnboardingActive {
                ...
                BlinkPermissionGuideSection(viewState: companionManager.permissionGuideViewState)
                    .padding(.horizontal, 14)
            }
            ...
            if companionManager.hasCompletedOnboarding && companionManager.allPermissionsGranted && companionManager.isAdvancedModeEnabled {
                ...
                CodexAgentModePanelSection(
                    ...
                    isBlinkCursorEnabled: companionManager.isBlinkCursorEnabled,
                    setBlinkCursorEnabled: { companionManager.setBlinkCursorEnabled($0) },
            ...
            // Show Blink toggle - hidden for now
            // if companionManager.hasCompletedOnboarding && companionManager.allPermissionsGranted {
            //     Spacer()
            //         .frame(height: 16)
            //
            //     showBlinkCursorToggleRow
    ...
    private var permissionsCopySection: some View {
        if isReadyForFirstOnboarding {
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text("Turn your sound on and tap Meet Blink.")
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text("Ask Blink about anything on your screen.")
                    .font(.system(size: 11, weight: .medium))
                    .foregroundColor(DS.Colors.textTertiary)

                Text("Ask naturally. Blink handles quick voice and computer-use requests itself, and hands deeper research, writing, coding, or file work to an agent in the background.")
                    .font(.system(size: 11, weight: .medium))
                    .foregroundColor(DS.Colors.textTertiary)
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text("Some permissions were revoked. Grant all four below to keep using Blink.")
                    .font(.system(size: 11))
                    .foregroundColor(DS.Colors.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
            }
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text("Nothing runs in the background. Blink will only take a screenshot when you press the hot key. So, you can give that permission in peace. If you are still sus, eh, I can't do much there champ.")
                    .font(.system(size: 11))
    ...
    // MARK: - Show Blink Cursor Toggle

    private var showBlinkCursorToggleRow: some View {
        HStack {
            HStack(spacing: 4) {
    ...
    private var bottomBlinkControlsSection: some View {
        VStack(alignment: .leading, spacing: 0) {
            if isReadyForFirstOnboarding {
    ...
    private var panelBackground: some View {
        // Dark scrim sits behind the material so the panel stays high-
        // contrast over bright/light app backgrounds (Chrome pages,
        // light-mode editors). The material on top still gives the
        // panel its glass blur, just over a darker base now.
        RoundedRectangle(cornerRadius: 12, style: .continuous)
