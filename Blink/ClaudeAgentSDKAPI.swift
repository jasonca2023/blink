...
final class ClaudeAgentSDKAPI {
    ...
    You are Blink's persistent local Claude Agent SDK voice response session.
    Keep the session warm and follow the current Blink voice policy and context supplied with each user turn.
    ...
    init?(
        model: String = BlinkModelCatalog.defaultVoiceResponseModelID,
    ...
    static func findExecutable(fileManager: FileManager = .default) -> URL? {
        ...
        if let explicitPath = environment["BLINK_CLAUDE_EXECUTABLE"],
    ...
    static func findNodeExecutable(fileManager: FileManager = .default) -> URL? {
        let environment = ProcessInfo.processInfo.environment
        if let explicitPath = environment["BLINK_NODE_EXECUTABLE"],
           let executable = executableURL(atPath: explicitPath, fileManager: fileManager) {
            return executable
        }
        ...
        Task { [weak self] in
            ...
            do {
                _ = try await self.sendRequest(
                    ...
                    conversationHistory: [],
                    userPrompt: "get ready. prime the Blink voice response session and reply with ready only.",
                    onTextChunk: { _ in }
                )
            } catch {
    ...
    private func ensureBridge() throws {
        ...
        guard let bridgeScriptURL else {
            throw NSError(
                domain: "ClaudeAgentSDKAPI",
                code: -21,
                userInfo: [NSLocalizedDescriptionKey: "Blink could not find ClaudeAgentSDKBridge/bridge.mjs."]
            )
        }
    ...
    private func bridgeEnvironment(
        ...
        let baselinePath = "/Users/blink/.local/bin:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin"
        environment["PATH"] = [nodeDirectory, existingPath, baselinePath]
            .filter { !$0.isEmpty }
            .joined(separator: ":")
        environment["BLINK_CLAUDE_EXECUTABLE"] = executableURL.path
        environment["BLINK_CLAUDE_MODEL"] = model
        environment["BLINK_CLAUDE_MAX_OUTPUT_TOKENS"] = String(maxOutputTokens)
        environment["CLAUDE_CODE_MAX_OUTPUT_TOKENS"] = String(maxOutputTokens)
        environment["BLINK_CLAUDE_CWD"] = workingDirectory.path
        environment["BLINK_CLAUDE_SYSTEM_PROMPT"] = systemPrompt
        environment["BLINK_CLAUDE_AGENT_SDK_PATHS"] = Self.nodeModuleSearchPaths(
            bridgeScriptURL: bridgeScriptURL,
            fileManager: fileManager
        ).joined(separator: ":")
        environment["CLAUDE_AGENT_SDK_CLIENT_APP"] = "blink/1.0"
        return environment
    }
    ...
    private static func findBridgeScript(fileManager: FileManager = .default) -> URL? {
        ...
        for key in ["BLINK_CLAUDE_AGENT_BRIDGE", "BLINK_CLAUDE_AGENT_SDK_BRIDGE"] {
        ...
        appendBridgeCandidate(URL(
            fileURLWithPath: "\(fileManager.homeDirectoryForCurrentUser.path)/Documents/GitHub/blink/AppResources/Blink/ClaudeAgentSDKBridge",
            isDirectory: true
        ))
