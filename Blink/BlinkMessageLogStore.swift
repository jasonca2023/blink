...
nonisolated final class BlinkMessageLogStore: @unchecked Sendable {
    static let shared = BlinkMessageLogStore()

    private let fileManager: FileManager
    private let lock = NSLock()
    private let writeQueue = DispatchQueue(label: "com.blink.blink.message-log-writes", qos: .utility)
    ...
    func availableMessageLogFiles() -> [URL] {
        do {
            ...
            print("Blink message log listing failed: \(error.localizedDescription)")
            return []
        }
    }
    ...
    func ensureAgentReviewCommentsFile() {
        ...
        do {
            ...
            print("Blink log review comment file setup failed: \(error.localizedDescription)")
        }
    }
    ...
    func appendReviewComment(
        ...
        do {
            ...
            print("Blink log review comment write failed: \(error.localizedDescription)")
        }
    }
    ...
    func append(lane: String, direction: String, event: String, fields: [String: Any] = [:]) {
        ...
        writeQueue.async { [weak self] in
            ...
            do {
                ...
                print("Blink message log write failed: \(error.localizedDescription)")
            }
        }
    }
    ...
    private func emitConsoleLog(entry: [String: Any]) {
        ...
        NSLog("[BlinkLog][%@][%@/%@] %@ %@", timestamp, lane, direction, event, previewText)
    }
}
