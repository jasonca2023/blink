...
@MainActor
final class BlinkLogViewerWindowManager {
    ...
    func show() {
        if window == nil {
            createWindow()
        } else if let hostingView: NSHostingView<BlinkLogViewerView> = BlinkLiquidGlassWindowSurface.hostingView(in: window) {
            hostingView.rootView = BlinkLogViewerView()
        }

        guard let logWindow = window else { return }
        NSApp.activate(ignoringOtherApps: true)
        BlinkWindowLevels.applyPanelDialogLevel(to: logWindow)
        logWindow.center()
    ...
    private func createWindow() {
        ...
        BlinkLiquidGlassWindowSurface.install(
            hostingView: NSHostingView(rootView: BlinkLogViewerView()),
            in: logWindow,
...
private struct BlinkLogViewerEntry: Identifiable, Equatable {
    let id: String
...
private struct BlinkLogReviewComment: Identifiable, Equatable {
    let id: String
...
struct BlinkLogViewerView: View {
    @State private var logFiles: [URL] = []
    @State private var selectedLogFile: URL?
    @State private var entries: [BlinkLogViewerEntry] = []
    ...
    private var selectedEntry: BlinkLogViewerEntry? {
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                Button {
                    ...
                    BlinkMarkdownViewerWindowManager.shared.show(fileURL: BlinkMessageLogStore.shared.agentReviewCommentsFile)
                } label: {
    ...
    private func header(for entry: BlinkLogViewerEntry) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(entry.event)
    ...
    private func commentEditor(for entry: BlinkLogViewerEntry) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Flag For Agent")
                .font(.system(size: 12, weight: .semibold))
                .foregroundColor(.secondary)
            ...
            HStack {
                Text("Saved comments are written to \(BlinkMessageLogStore.shared.agentReviewCommentsFile.path)")
    ...
    private func reloadAll() {
        logFiles = BlinkMessageLogStore.shared.availableMessageLogFiles()
    ...
    private func parseEntry(rawLine: String, sourceFileURL: URL, sourceLineNumber: Int) -> BlinkLogViewerEntry? {
        guard let data = rawLine.data(using: .utf8),
              let object = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            return BlinkLogViewerEntry(
    ...
    private func saveComment(for entry: BlinkLogViewerEntry) {
        BlinkMessageLogStore.shared.appendReviewComment(
        ...
        BlinkWidgetStateStore().refreshLogReviewAttentionSnapshot()
        commentText = ""
        loadReviewComments()
    ...
    private func loadReviewComments() {
        let fileURL = BlinkMessageLogStore.shared.reviewCommentsFile
