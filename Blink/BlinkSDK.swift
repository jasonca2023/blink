...
/// Public execution mode for an embedded Blink runtime.
///
/// Use ``embeddedWindow`` for SwiftUI integrations where you want a local,
/// app-owned Blink surface that is independent from any menu-bar experience.
public enum BlinkSDKMode: Equatable {
    /// Keep the existing menu-bar runtime.
    case menuBar
    /// Run Blink in an in-window runtime suitable for SDK embedding.
    case embeddedWindow
}
...
public struct BlinkSDKPanelActions {
    /// Called when the close affordance in the panel UI is activated.
    public var onPanelDismiss: () -> Void
    /// Called when the user taps the "Quit Blink" action.
    public var onQuit: () -> Void
    /// Opens the Blink HUD surface in the host environment.
    public var onOpenHUD: () -> Void
    /// Opens the Blink memory window in the host environment.
    public var onOpenMemory: () -> Void
    /// Opens the Blink feedback/inbox link.
...
/// self-contained Blink runtime for app embedding.
@MainActor
public final class BlinkSDKSession: ObservableObject {
    private let manager: CompanionManager
    public let mode: BlinkSDKMode

    @Published public private(set) var isStarted = false

    public init(mode: BlinkSDKMode = .embeddedWindow) {
        self.mode = mode
        let runtimeMode: BlinkCompanionRuntimeMode = mode == .embeddedWindow ? .embeddedWindow : .menuBar
    ...
    public func setCodexAgentAPIKey(_ key: String) {
        // Blink currently routes this through the same API client path.
...
public struct BlinkSDKPanel: View {
