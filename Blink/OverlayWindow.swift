...
nonisolated enum BlinkResponseCaptionFont: String, CaseIterable, Identifiable {
    case systemRounded
    case systemSerif
...
final class BlinkAgentDockWindowManager {
    ...
    func dockIconCenter(for itemID: UUID, in items: [BlinkAgentDockItem]) -> CGPoint? {
        guard let panel,
              let itemIndex = items.firstIndex(where: { $0.id == itemID }) else {
            return nil
        }

        // Mirror `BlinkAgentDockStackView`'s row placement so cursor handoff
