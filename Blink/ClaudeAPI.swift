...
class ClaudeAPI {
    ...
    /// Result of a single tool-use turn. `contentBlocks` is the raw
    /// assistant content array (text + tool_use blocks) preserved
    /// verbatim so it can be echoed back in the next user turn.
    struct ToolTurnResult {
        let contentBlocks: [[String: Any]]
        let stopReason: String?
    }

    /// Non-streaming Anthropic call with tool support. Caller passes
    /// the full message history (including any prior tool_use /
    /// tool_result rounds), the tool catalog, and a system prompt.
    func runToolTurn(
        systemPrompt: String,
        messages: [[String: Any]],
        tools: [[String: Any]]
        ...
        guard let http = response as? HTTPURLResponse else {
            throw NSError(domain: "ClaudeAPI", code: -1, userInfo: [NSLocalizedDescriptionKey: "Invalid HTTP response"])
        }
        ...
        guard let json = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            throw NSError(domain: "ClaudeAPI", code: -2, userInfo: [NSLocalizedDescriptionKey: "Unparseable JSON"])
        }
        let content = (json["content"] as? [[String: Any]]) ?? []
        let stopReason = json["stop_reason"] as? String
        return ToolTurnResult(contentBlocks: content, stopReason: stopReason)
    }
