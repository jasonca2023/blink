...
struct Triangle: Shape {
    func path(in rect: CGRect) -> Path {
        // An equilateral-style triangle whose CENTROID lands at the
        // rect's geometric center, not its bounding box. Without the
        // y-shift, the centroid sits 1/3 up from the base (i.e. below
        // the bounding-box center), so a triangle dropped into a square
        // container or rotated around its bounds visually drifts. The
        // -rect.height/6 shift moves the apex / base lines up so the
        // centroid coincides with (rect.midX, rect.midY) and rotation
        // around the bounds center stays visually balanced.
        let centroidShift = rect.height / 6
        var path = Path()
        path.move(to: CGPoint(x: rect.midX, y: rect.minY - centroidShift))
        path.addLine(to: CGPoint(x: rect.maxX, y: rect.maxY - centroidShift))
        path.addLine(to: CGPoint(x: rect.minX, y: rect.maxY - centroidShift))
        path.closeSubpath()
        return path
    }
}

enum BlinkAccentTheme: String, CaseIterable, Identifiable {
    case blue
    case mint
    case amber
    case rose
    ...
    static let userDefaultsKey = "blinkAccentTheme"
    ...
    static var current: BlinkAccentTheme {
...
enum DS {
    ...
    enum Colors {
        ...
        // ── Backgrounds ──────────────────────────────────────────────
        // "Dusk" palette — deep midnight with a violet undertone so the
        // chrome doesn't read as another generic dark-mode utility. The
        // accent colors layered on top stay punchy against this base.
        // Layered surfaces from deepest to most elevated; higher
        // surfaces lighten slightly, creating a sense of depth.

        /// The deepest background — used for the main app window fill.
        static let background = Color(hex: "#16131F")

        /// First elevation layer — used for cards, sidebar, top bar backgrounds.
        static let surface1 = Color(hex: "#1E1A2A")

        /// Second elevation layer — used for input fields, elevated cards, chat bubbles.
        static let surface2 = Color(hex: "#272135")

        /// Third elevation layer — used for hover backgrounds on interactive elements.
        static let surface3 = Color(hex: "#322B43")

        /// Fourth elevation layer — used for active/pressed states on interactive elements.
        static let surface4 = Color(hex: "#3D3553")

        // ── Borders ──────────────────────────────────────────────────

        /// Subtle border — used for card outlines, dividers, input field borders.
        static let borderSubtle = Color(hex: "#473E60")

        /// Strong border — used for focused inputs, hovered card outlines.
        static let borderStrong = Color(hex: "#5A506E")

        // ── Text ─────────────────────────────────────────────────────
        // All three tiers are translucent white so they stay legible
        // against any backdrop the panel sits over (light Chrome page,
        // dark editor, photo wallpaper). Hierarchy comes from opacity,
        // not hue — keeps the UI feeling like a single layer of glass.
        ...
        /// Tertiary text — section labels, timestamps, disabled text.
        ...
        static var accent: Color { BlinkAccentTheme.current.accent }

        /// Accent hover — slightly darker blue for hover state.
        /// #1d4ed8 → ~6.5:1 contrast with white text (WCAG AA+).
        static var accentHover: Color { BlinkAccentTheme.current.accentHover }
        ...
        static var accentText: Color { BlinkAccentTheme.current.accentText }
        ...
        static var accentSubtle: Color { BlinkAccentTheme.current.accentSubtle }
        ...
        static var overlayCursorBlue: Color { BlinkAccentTheme.current.cursorColor }
    ...
    enum Animation {
        ...
        /// Signature Blink spring — slightly snappier and more elastic than
        /// the default SwiftUI spring. Used for panel reveals, cursor
        /// arrival snaps, and anywhere we want a "confident" feel.
        static let blinkSpring: SwiftUI.Animation = .spring(response: 0.32, dampingFraction: 0.62, blendDuration: 0)

        /// Slower companion spring used for layout settles where the
        /// `blinkSpring` would feel too jittery.
        static let blinkSpringSettle: SwiftUI.Animation = .spring(response: 0.48, dampingFraction: 0.75, blendDuration: 0)
    }

    // MARK: - Typography
    // Blink uses SF Pro *Rounded* for headings/labels and the default
    // SF Pro Text for body. The rounded face is the cheapest signature
    // typographic move — no font bundling, no licensing, but the chrome
    // immediately reads as not-another-default-Mac-app.

    enum Typography {
        /// Headings, section titles, button labels.
        static func heading(_ size: CGFloat, weight: Font.Weight = .semibold) -> Font {
            .system(size: size, weight: weight, design: .rounded)
        }

        /// Smaller labels (chips, badges, tool tips). Rounded with a
        /// little extra weight so they hold up at small sizes.
        static func label(_ size: CGFloat = 11) -> Font {
            .system(size: size, weight: .medium, design: .rounded)
        }

        /// Body text — regular SF Pro Text. Rounded would feel too
        /// casual for paragraphs of response content.
