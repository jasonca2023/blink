...
final class BlinkNativeComputerUseController: ObservableObject {
    ...
    func click(at point: CGPoint) throws {
        guard isEnabled else { throw BlinkComputerUseError.disabled }
        BlinkApplicationUsageLogStore.shared.recordFrontmostApplication(source: "native_cua_click")
        try BlinkComputerUseMouseInput.leftClick(at: point)
    }
...
nonisolated struct BlinkBackgroundComputerUseActionResult: Sendable, Hashable {
    let windowID: String
    let summary: String
    let ok: Bool
}

nonisolated enum BlinkBackgroundComputerUseError: Error, LocalizedError, Equatable {
...
private nonisolated struct BlinkBackgroundComputerUseRuntimeManifest: Decodable {
    let baseURL: String
...
private nonisolated struct BlinkBackgroundComputerUseRuntimePermissions: Decodable {
    let accessibility: BlinkBackgroundComputerUsePermission
    let screenRecording: BlinkBackgroundComputerUsePermission
}

private nonisolated struct BlinkBackgroundComputerUsePermission: Decodable {
    let granted: Bool
}

private nonisolated struct BlinkBackgroundComputerUseRuntimeInstructions: Decodable {
    let ready: Bool
    let summary: String
}

private nonisolated struct BlinkBackgroundComputerUseEmptyRequest: Encodable {}

private nonisolated struct BlinkBackgroundComputerUseListAppsResponse: Decodable {
    let frontmostApp: BlinkBackgroundComputerUseRunningApp?
    let runningApps: [BlinkBackgroundComputerUseRunningApp]
}

private nonisolated struct BlinkBackgroundComputerUseRunningApp: Decodable {
    let name: String
    let bundleID: String
    let pid: Int32
...
private nonisolated struct BlinkBackgroundComputerUseListWindowsRequest: Encodable {
    let app: String
}

private nonisolated struct BlinkBackgroundComputerUseListWindowsResponse: Decodable {
    let windows: [BlinkBackgroundComputerUseWindow]
}

private nonisolated struct BlinkBackgroundComputerUseWindow: Decodable, Sendable, Hashable {
    let windowID: String
    let title: String
    let bundleID: String
    let pid: Int32
...
private nonisolated struct BlinkBackgroundComputerUseWindowStateRequest: Encodable {
    let window: String
    let maxNodes: Int
...
private nonisolated struct BlinkBackgroundComputerUseWindowStateResponse: Decodable {
    let stateToken: String
    let window: BlinkBackgroundComputerUseResolvedWindow
    let screenshot: BlinkBackgroundComputerUseScreenshot
}

private nonisolated struct BlinkBackgroundComputerUseResolvedWindow: Decodable, Sendable, Hashable {
    let windowID: String
    let title: String
    let bundleID: String
    let pid: Int32
...
private nonisolated struct BlinkBackgroundComputerUseScreenshot: Decodable {
    let status: String
    let image: BlinkBackgroundComputerUseScreenshotImage?
    let captureError: String?
}

private nonisolated struct BlinkBackgroundComputerUseScreenshotImage: Decodable {
...
private nonisolated struct BlinkBackgroundComputerUseCursorRequest: Encodable {
    let id: String
    let name: String
    let color: String
}

private nonisolated struct BlinkBackgroundComputerUsePressKeyRequest: Encodable {
    let window: String
    let key: String
...
private nonisolated struct BlinkBackgroundComputerUseTypeTextRequest: Encodable {
    let window: String
    let text: String
...
private nonisolated struct BlinkBackgroundComputerUseActionResponse: Decodable {
    let ok: Bool
    let summary: String
}
...
enum BlinkComputerUseMouseInput {
    ...
    private static func postMouseEvent(type: CGEventType, at point: CGPoint) throws {
        guard let event = CGEvent(
            ...
            throw BlinkComputerUseError.eventCreationFailed("mouse \(type.rawValue) at \(Int(point.x)),\(Int(point.y))")
        }
        event.post(tap: .cghidEventTap)
    }
}
