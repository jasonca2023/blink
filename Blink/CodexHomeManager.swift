...
final class CodexHomeManager {
    ...
    let modelInstructionsFileName = "BlinkModelInstructions.md"
    let runtimeMapFileName = "BlinkRuntimeMap.md"
    let bundledSkillsDirectoryName = "BlinkBundledSkills"
    let learnedSkillsDirectoryName = "BlinkLearnedSkills"
    let bundledWikiSeedDirectoryName = "BlinkBundledWikiSeed"
    ...
    init(
        ...
        model: String = BlinkModelCatalog.codexActionsModel(
            withID: UserDefaults.standard.string(forKey: "blinkCodexModel") ?? BlinkModelCatalog.defaultCodexActionsModelID
        ).id,
        reasoningEffort: String = UserDefaults.standard.string(forKey: "blinkCodexReasoningEffort") ?? "medium"
    ) {
    ...
    var modelProviderID: String {
        BlinkCodexBackend.isDefaultOpenAIBaseURL(workerBaseURL)
            ? BlinkCodexConfigTemplate.defaultModelProviderID
            : BlinkCodexConfigTemplate.customModelProviderID
    }
    ...
    func prepare(bundle: Bundle = .main) throws -> CodexHomeLayout {
        ...
        BlinkMessageLogStore.shared.ensureAgentReviewCommentsFile()
        ...
        if let source = resourceURL(named: soulFileName, bundle: bundle) {
            ...
            try "Blink is a voice-first macOS companion with durable memory and background Agent Mode.\n".write(to: soul, atomically: true, encoding: .utf8)
        }
        ...
        if let source = resourceURL(named: modelInstructionsFileName, bundle: bundle) {
            ...
            try "You are Blink, a friendly macOS cursor companion with Codex Agent Mode.\n".write(to: modelInstructions, atomically: true, encoding: .utf8)
        }
        ...
        let config = BlinkCodexConfigTemplate(
            model: model,
    ...
    /// Re-renders just config.toml against the current backend/model
    /// preferences. The next Codex spawn picks it up automatically.
    func regenerateConfig() throws {
        let home = codexHomeDirectory
        try fileManager.createDirectory(at: home, withIntermediateDirectories: true)
        let config = BlinkCodexConfigTemplate(
            model: model,
            reasoningEffort: reasoningEffort,
            workerBaseURL: BlinkCodexBackend.configuredWorkerBaseURL(),
            backendKind: BlinkAgentBackendKind.current(),
            modelInstructionsFileName: modelInstructionsFileName,
            bundledSkillsDirectoryName: bundledSkillsDirectoryName,
            learnedSkillsDirectoryName: learnedSkillsDirectoryName,
            includeOpenAIDeveloperDocsMCP: true
    ...
    func persistentMemoryContext(maxCharacters: Int = 6_000, includeArchives: Bool = false) -> String {
        do {
            ...
            return "Blink persistent memory is not available yet: \(error.localizedDescription)"
        }
    }
    ...
    func saveMemory(title: String, body: String, createdAt: Date = Date()) throws -> WikiManager.Article {
        ...
        guard !trimmedTitle.isEmpty else {
            throw NSError(domain: "Blink.Memory", code: 1, userInfo: [
                NSLocalizedDescriptionKey: "Blink needs a title before it can save a memory."
            ])
        }

        guard !trimmedBody.isEmpty else {
            throw NSError(domain: "Blink.Memory", code: 2, userInfo: [
                NSLocalizedDescriptionKey: "Blink needs some memory content before saving."
            ])
        }
    ...
    private func initialPersistentMemoryFileContent() -> String {
        """
        # Blink Persistent Memory

        This file is Blink's durable memory for Agent Mode. Agents must read it before starting user tasks and update it when they learn stable facts, preferences, project context, or reusable workflow knowledge.
        ...
        - When a task reveals a repeatable workflow, create or update a skill in `BlinkLearnedSkills/<workflow_name>/SKILL.md`.
        """
    }
    ...
    private func writeRuntimeMap(
        ...
        This file tells Blink Agent Mode where durable context, logs, skills, and app state live. Agents may read or edit these files when the user asks, subject to normal safety rules for destructive changes, credentials, and permissions.
        ...
        - Read `SOUL.md` before agent work. Treat it as Blink's persona and operating identity.
        ...
        - When optimizing skills, prompts, memory files, logs-derived notes, or other Blink artifacts, archive the previous version under \(archivesDirectory.path) before replacing it. Do not delete old versions.
        ...
        - Do not claim Blink cannot remember or cannot inspect its own logs, memory, skills, or runtime files. Use the paths above.
        """
