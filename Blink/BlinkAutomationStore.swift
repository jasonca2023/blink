//
//  BlinkAutomationStore.swift
//  Blink
//
//  JSON-backed automation registry + a single 30-second tick scheduler.
//  Persists to ~/Library/Application Support/Blink/automations.json.
...
@MainActor
final class BlinkAutomationStore: ObservableObject {
  static let shared = BlinkAutomationStore()
  ...
  static var skillDiscoveryAutomationPrompt: String {
    """
    Blink scheduled skill discovery pass.

    Goal: find useful Agent Mode skills for the apps and workflows the user is actively using, then surface install/connect options in the Blink Connect tab.

    Be efficient:
    1. Identify likely active apps/workflows from recent Blink logs, current screen/window context if provided, and obvious local project folders. Keep Blink's default suggestions available, but when an active app is visible, include suggestions tailored to that app using known skills, MCP/connectors, gog routes, browser automation, or screen-context workflows. Do not scan huge folders blindly.
    2. Search local skills first under ~/Library/Application Support/Blink/AgentMode/CodexHome/BlinkBundledSkills, ~/Library/Application Support/Blink/AgentMode/CodexHome/BlinkLearnedSkills, ~/.codex/skills, ~/.agents/skills, ~/Documents/GitHub/*/skills, and any directly relevant repo skill folders. Prefer `find`/metadata over reading every large file.
    ...
    4. Recommend only practical, low-risk options that Blink can install locally or connect through existing app/tool routes.
    ...
    [
      {
        ...
        "installPrompt": "Exact Blink Agent Mode prompt to install or connect it"
      }
    ]
  ...
  @Published private(set) var automations: [BlinkAutomation] = []
  ...
  private init() {
    let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
      ?? FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
    let dir = appSupport.appendingPathComponent("Blink", isDirectory: true)
  ...
  func add(_ automation: BlinkAutomation) {
    var a = automation
  ...
  func update(_ automation: BlinkAutomation) {
  ...
  @discardableResult
  func ensureSkillDiscoveryAutomationInstalled() -> BlinkAutomation {
    _ = BlinkAgentStore.shared.ensureSkillDiscoveryAgentInstalled()
  ...
  var skillDiscoveryAutomation: BlinkAutomation? {
    automations.first(where: { isProtectedSystemAutomation($0) })
  }

  func isProtectedSystemAutomation(_ automation: BlinkAutomation) -> Bool {
    automation.name == Self.skillDiscoveryAutomationName || automation.agentSlug == BlinkAgentStore.skillDiscoveryAgentSlug
  }
  ...
  private func fire(automation: BlinkAutomation) {
    guard let companion else { return }
    let prompt = automation.prompt
    if let slug = automation.agentSlug, let agent = BlinkAgentStore.shared.agent(slug: slug) {
  ...
  private func load() {
    ...
    if let list = try? decoder.decode([BlinkAutomation].self, from: data) {
...
struct BlinkSkillDiscoverySuggestion: Codable, Identifiable, Equatable {
  var id: String
  var title: String
  var detail: String
  var source: String
...
private struct BlinkSkillSuggestionRule: Codable, Equatable {
  var id: String
  var appMatches: [String]
  var suggestions: [BlinkSkillDiscoverySuggestion]
}

private struct BlinkSkillSuggestionRegistry: Codable, Equatable {
  var defaultSuggestions: [BlinkSkillDiscoverySuggestion]
  var appRules: [BlinkSkillSuggestionRule]

  static func load(from url: URL) -> BlinkSkillSuggestionRegistry {
    ensureUserConfigExists(at: url)

    if let data = try? Data(contentsOf: url),
       let decoded = try? JSONDecoder().decode(BlinkSkillSuggestionRegistry.self, from: data) {
      return decoded
    }
  ...
  func suggestions(for context: BlinkSkillDiscoveryStore.ApplicationContext?, slug: (String) -> String) -> [BlinkSkillDiscoverySuggestion] {
    ...
    return [
      BlinkSkillDiscoverySuggestion(
        id: "active-\(slug(context.name))-screen-context",
        title: "\(context.name) screen context",
        detail: "Blink can use the active \(context.name) window as visual context and suggest the safest available automation route.",
        source: "app",
        installPrompt: "Use Blink's screen context for the active \(context.name) window and suggest the best available skill, MCP, or agent workflow.",
  ...
  private static let fallback = BlinkSkillSuggestionRegistry(defaultSuggestions: [], appRules: [])

}

@MainActor
final class BlinkSkillDiscoveryStore: ObservableObject {
  static let shared = BlinkSkillDiscoveryStore()

  @Published private(set) var suggestions: [BlinkSkillDiscoverySuggestion] = []
  @Published private(set) var activeApplicationName: String?

  private let storeURL = BlinkAutomationStore.skillDiscoverySuggestionsURL
  ...
  func reload() {
    ...
    let registry = BlinkSkillSuggestionRegistry.load(from: BlinkAutomationStore.skillSuggestionRulesURL)
  ...
  private func loadSavedSuggestions() -> [BlinkSkillDiscoverySuggestion] {
    guard let data = try? Data(contentsOf: storeURL),
          let decoded = try? JSONDecoder().decode([BlinkSkillDiscoverySuggestion].self, from: data) else {
      return []
    }
    return decoded
  }

  private func mergeSuggestions(_ prioritized: [BlinkSkillDiscoverySuggestion],
                                _ defaults: [BlinkSkillDiscoverySuggestion]) -> [BlinkSkillDiscoverySuggestion] {
    var seen: Set<String> = []
    var merged: [BlinkSkillDiscoverySuggestion] = []
  ...
  private func mostRecentRecordedApplication(excluding ownBundleIdentifier: String?) -> ApplicationContext? {
    let logURL = BlinkApplicationUsageLogStore.shared.logURL
    ...
    for entry in sortedApps {
      ...
      if name.localizedCaseInsensitiveContains("Blink") {
        continue
      }
