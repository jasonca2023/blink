...
enum CodexRuntimeLocator {
    ...
    enum LocatorError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .codexExecutableNotFound:
                return "Blink could not find a bundled, installed, or PATH Codex executable."
            }
        }
    }
    ...
    static func codexExecutableCandidates(bundle: Bundle = .main, fileManager: FileManager = .default) -> [URL] {
        ...
        candidates.append(contentsOf: nodeGlobalCodexExecutableURLs(fileManager: fileManager))
    ...
    static func sourceAppResourcesDirectory(fileManager: FileManager = .default) -> URL? {
        ...
        if let bundledBlink = bundle.url(forResource: "Blink", withExtension: nil) {
            candidates.append(bundledBlink)
        }
        if let resourceURL = bundle.resourceURL {
            candidates.append(resourceURL.appendingPathComponent("Blink", isDirectory: true))
            candidates.append(
                resourceURL
    ...
    /// macOS GUI apps launched from Finder/Dock do NOT inherit the user's
    /// shell `PATH` — they get a stripped-down environment that excludes
    /// per-user npm/node directories. The Codex CLI is normally installed
    /// via `npm install -g`, which on a default user setup lands in one
    /// of the locations below. We probe them explicitly so the locator
    /// finds Codex even when `PATH` doesn't.
    static func nodeGlobalCodexExecutableURLs(fileManager: FileManager = .default) -> [URL] {
        ...
        // nvm installs node per-version under ~/.nvm/versions/node/<version>/bin.
        // Pick up codex from whichever versions the user has installed.
        let nvmRoot = "\(home)/.nvm/versions/node"
        if let versionDirs = try? fileManager.contentsOfDirectory(atPath: nvmRoot) {
            for versionDir in versionDirs {
                paths.append("\(nvmRoot)/\(versionDir)/bin/codex")
            }
        }

        // pnpm installs to a per-user data directory.
        let pnpmHome = ProcessInfo.processInfo.environment["PNPM_HOME"]
            ?? "\(home)/Library/pnpm"
        paths.append("\(pnpmHome)/codex")
