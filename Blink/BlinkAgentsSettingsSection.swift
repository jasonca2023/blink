//
//  BlinkAgentsSettingsSection.swift
//  Blink
//
//  Real "Agents" settings tab — backed by BlinkAgentStore. List on
...
struct BlinkAgentsSettingsSection: View {
  @ObservedObject var companion: CompanionManager
  @ObservedObject private var store = BlinkAgentStore.shared
  ...
  private func row(agent: BlinkAgentDefinition) -> some View {
...
private struct AgentEditorView: View {
  let agent: BlinkAgentDefinition
  @ObservedObject var store: BlinkAgentStore
  @ObservedObject var companion: CompanionManager
  ...
  init(agent: BlinkAgentDefinition, store: BlinkAgentStore, companion: CompanionManager) {
    self.agent = agent
    self.store = store
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 10) {
      ...
      HStack {
        ...
        Spacer()
        if agent.isProtectedSystemAgent {
          Label("Required by Blink", systemImage: "lock.fill")
  ...
  private func scheduleHeartbeat() {
    ...
    BlinkAutomationStore.shared.add(automation)
  }
}
