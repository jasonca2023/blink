...
/// Compact menu-bar surface inspired by the recovered Blink notch architecture.
///
/// This is intentionally an Blink-original implementation. It only reads
...
/// pipeline.
@MainActor
struct BlinkNotchPanelView: View {
    ...
    private struct HomeSuggestionItem: Identifiable, Equatable {
        ...
        let prompt: String?
        let mode: BlinkQuickPromptMode?
        let opensSettings: Bool
    }

    @ObservedObject var companionManager: CompanionManager
    @ObservedObject private var agentStore = BlinkAgentStore.shared
    @ObservedObject private var automationStore = BlinkAutomationStore.shared
    @ObservedObject private var skillDiscoveryStore = BlinkSkillDiscoveryStore.shared
    ...
    @AppStorage(BlinkCursorAvatarSizePreference.userDefaultsKey) private var cursorAvatarSizeScale = BlinkCursorAvatarSizePreference.defaultScale
    @AppStorage(AppBundleConfiguration.userAppFontDefaultsKey) private var appFontRawValue = BlinkResponseCaptionFont.fallback.rawValue
    ...
    @AppStorage(AppBundleConfiguration.userThemeDefaultsKey) private var blinkTheme = BlinkTheme.system.rawValue
    ...
    @State private var selectedTab: BlinkNotchTab = .home
    @State private var quickPromptMode: BlinkQuickPromptMode = .ask
    ...
    private var appFont: BlinkResponseCaptionFont {
        BlinkResponseCaptionFont.resolved(appFontRawValue)
    }
    ...
    private var quickPromptAutocompleteOptions: [BlinkPromptAutocompleteOption] {
        BlinkPromptAutocomplete.options(
            for: quickPrompt,
    ...
    private var expandedAgentAutocompleteOptions: [BlinkPromptAutocompleteOption] {
        BlinkPromptAutocomplete.options(
    ...
    @State private var agentPanelSelection: BlinkAgentPanelSelection = .sessions
    @State private var agentSessionFilter: BlinkAgentSessionFilter = .active
    ...
    @State private var gogStatus: BlinkGogCLIStatus = .unknown
    ...
    private var voiceModelLabel: String {
        BlinkModelCatalog.voiceResponseModel(withID: companionManager.selectedModel).label
    }

    private var speechModelLabel: String {
        BlinkModelCatalog.speechModel(withID: companionManager.selectedSpeechModel).label
    }
    ...
    private var connectionRows: [BlinkNotchConnectionRow] {
    ...
    var body: some View {
        resizeAwarePanel(panelLifecycle(panelDialogs(panelRoot)))
            .preferredColorScheme(blinkTheme == BlinkTheme.light.rawValue ? .light : (blinkTheme == BlinkTheme.dark.rawValue ? .dark : nil))
    }
    ...
    private func panelDialogs<Content: View>(_ content: Content) -> some View {
        ...
        .confirmationDialog(
            "Archive this incomplete Blink task?",
    ...
    private func resizeAwarePanel<Content: View>(_ content: Content) -> some View {
        content
        .onReceive(NotificationCenter.default.publisher(for: .blinkMainPanelResizeStateDidChange)) { notification in
    ...
    private var mainSurface: some View {
        ...
        .background(
            BlinkBottomRoundedRectangle(cornerRadius: 28)
                .fill(
                    BlinkLiquidGlassBackdropView.isLiquidGlassAvailable ?
        ...
        .clipShape(BlinkBottomRoundedRectangle(cornerRadius: 28))
        .contentShape(BlinkBottomRoundedRectangle(cornerRadius: 28))
        .overlay {
    ...
    private var topStatusRail: some View {
        HStack(spacing: 7) {
            ...
            .buttonStyle(.plain)
            .accessibilityLabel("Show Blink agents")
            .help("Show Blink agents")
    ...
    private var tabStrip: some View {
        HStack(spacing: 6) {
            BlinkPanelDragHandle()
                .frame(width: 30, height: 30)
                .accessibilityLabel("Drag Blink panel")
                .help("Drag Blink panel")

            ForEach(BlinkNotchTab.primaryTabs) { tab in
                Button {
            ...
            panelChromeButton(
                ...
                accessibilityLabel: "Open Blink panel settings"
            ) {
            ...
            panelChromeButton(
                ...
                accessibilityLabel: isPanelPinned ? "Unpin Blink panel" : "Pin Blink panel"
            ) {
            ...
            panelChromeButton(
                ...
                accessibilityLabel: "Close Blink panel",
                action: closePanel
            )
        }
        .frame(height: 32)
    ...
    private func selectPrimaryTab(_ tab: BlinkNotchTab) {
    ...
    private var homeSuggestionItems: [HomeSuggestionItem] {
        ...
        let defaults = [
            ...
            HomeSuggestionItem(
                ...
                systemImageName: "terminal.fill",
                prompt: "Look at the current Blink screen context and fix the visible issue.",
                mode: .agent,
                opensSettings: false
            ),
            HomeSuggestionItem(
    ...
    private func agentPanelSelectorButton(
        selection: BlinkAgentPanelSelection,
        title: String,
        value: String,
        detail: String,
        color: Color,
        ...
        .buttonStyle(.plain)
        .accessibilityLabel("Show Blink \(title.lowercased())")
        .help("Show \(title)")
    }
    ...
    private func specialistAgentTile(_ agent: BlinkAgentDefinition) -> some View {
    ...
    private func specialistAgentAccentColor(_ agent: BlinkAgentDefinition) -> Color {
    ...
    private var agentsFooter: some View {
        VStack(spacing: 8) {
            HStack(spacing: 6) {
                ForEach(BlinkAgentSessionFilter.allCases) { filter in
            ...
            HStack(spacing: 8) {
                ...
                agentUtilityIconButton(
                    ...
                    accessibilityLabel: "Open Blink memory browser"
                ) {
                ...
                agentUtilityIconButton(
                    ...
                    accessibilityLabel: "Open Blink memory file"
                ) {
                    companionManager.openBlinkDocument(companionManager.codexHomeManager.persistentMemoryFile)
                }

                agentUtilityIconButton(
                    systemImageName: "wand.and.stars",
                    accessibilityLabel: "Open Blink learned skills folder"
                ) {
    ...
    private func agentFilterButton(_ filter: BlinkAgentSessionFilter) -> some View {
    ...
    private var skillDiscoveryPanel: some View {
        VStack(alignment: .leading, spacing: 9) {
            ...
            if skillDiscoveryStore.suggestions.isEmpty {
                Text("Blink will populate this with local and online skill matches after the scheduled pass runs.")
                    .font(panelUIFont(size: 10, weight: .medium))
                    .foregroundColor(DS.Colors.textSecondary)
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .padding(9)
    ...
    private func skillDiscoverySuggestionRow(_ suggestion: BlinkSkillDiscoverySuggestion) -> some View {
        HStack(alignment: .top, spacing: 9) {
            ...
            .help("Start an Blink agent to \(suggestion.actionLabel.lowercased()) \(suggestion.title)")
        }
        .padding(9)
    ...
    private func skillDiscoverySuggestionIcon(for suggestion: BlinkSkillDiscoverySuggestion) -> String {
    ...
    private func runSkillDiscoveryNow() {
        automationStore.ensureSkillDiscoveryAutomationInstalled()
        if let agent = agentStore.agent(slug: BlinkAgentStore.skillDiscoveryAgentSlug) {
            let session = companionManager.createAndSelectNewCodexAgentSession(asAgent: agent)
            session.submitPromptFromUI(BlinkAutomationStore.skillDiscoveryAutomationPrompt, screenContext: nil)
        } else {
            companionManager.submitAgentPromptFromUI(BlinkAutomationStore.skillDiscoveryAutomationPrompt)
        }
    }

    private func installSkillSuggestion(_ suggestion: BlinkSkillDiscoverySuggestion) {
        ...
        companionManager.submitNewAgentTaskFromUI(
            prompt.isEmpty ? "Install or connect the \(suggestion.title) skill for Blink Agent Mode, preserving local skill and memory rules." : prompt,
            source: "open_blink_connect_tab"
        )
        notifyPanelSizeChanged()
    }
    ...
    private var quickPromptField: some View {
        VStack(alignment: .leading, spacing: 7) {
            ...
            BlinkPromptAutocompletePanel(options: quickPromptAutocompleteOptions) { option in
                BlinkPromptAutocomplete.apply(option, to: &quickPrompt)
            }
            ...
            HStack(spacing: 8) {
                ...
                TextField(quickPromptAttachments.isEmpty ? quickPromptMode.placeholder : "Ask about the attachment…", text: $quickPrompt, axis: .vertical)
                    ...
                    .onKeyPress(.tab, phases: .down) { _ in
                        BlinkPromptAutocomplete.acceptFirstOption(
                            in: &quickPrompt,
                            agents: agentStore.agents,
                            skillSuggestions: skillDiscoveryStore.suggestions
                        ) ? .handled : .ignored
    ...
    @ViewBuilder
    private func quickPromptModeButton(_ mode: BlinkQuickPromptMode) -> some View {
        let action = {
        ...
        VStack(alignment: .leading, spacing: 7) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                Slider(
                    ...
                    in: BlinkCursorAvatarSizePreference.minScale...BlinkCursorAvatarSizePreference.maxScale
                )
                .controlSize(.small)
                .tint((BlinkAccentTheme(rawValue: selectedAccentThemeID) ?? .blue).cursorColor)
            }
        }
    }
    ...
    private func avatarTile(_ style: BlinkCursorAvatarStyle, label: String) -> some View {
    ...
    private func avatarTileForPet(_ pet: BlinkBuddyPet) -> some View {
    ...
    private var cursorColorThemeOrder: [BlinkAccentTheme] {
    ...
    private var hatchPetSheet: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack {
                ...
                Button("Hatch") {
                    let theme = BlinkAccentTheme(rawValue: selectedAccentThemeID) ?? .blue
                    _ = BlinkPetHatchCoordinator.shared.beginHatch(
                        name: hatchPetName,
        ...
        return VStack(spacing: 0) {
            HStack(spacing: 6) {
                Button {
                    ...
                    HStack(spacing: 10) {
                        if isRunning {
                            BlinkRunningAgentIndicator(color: agentStatusColor(session.status))
                ...
                if canStop {
                    agentArchiveButton(
                        ...
                        helpText: "Stop this running Blink task",
                    ...
                    agentArchiveButton(
                        ...
                        helpText: "Unarchive this Blink task"
                    ) {
                    ...
                    agentArchiveButton(
                        ...
                        helpText: "Archive this Blink task"
                    ) {
            ...
            ScrollViewReader { proxy in
                ScrollView(.vertical, showsIndicators: true) {
                    LazyVStack(alignment: .leading, spacing: 7) {
                        if entries.isEmpty {
                            Text("No conversation yet. Send a follow-up to talk to this Blink task.")
                                .font(panelUIFont(size: 10, weight: .semibold))
                                .foregroundColor(DS.Colors.textSecondary)
                                .fixedSize(horizontal: false, vertical: true)
                                .frame(maxWidth: .infinity, alignment: .leading)
                        ...
                        if session.canResumeAfterRelaunch {
                            ...
                            .buttonStyle(.plain)
                            .accessibilityLabel("Resume Blink task after relaunch")
                            .help("Resume this unfinished Blink task after relaunch")
    ...
    private func agentReplyField(for session: CodexAgentSession) -> some View {
        VStack(alignment: .leading, spacing: 7) {
            ...
            BlinkPromptAutocompletePanel(options: expandedAgentAutocompleteOptions) { option in
                BlinkPromptAutocomplete.apply(option, to: &expandedAgentPrompt)
            }
            ...
            HStack(spacing: 8) {
                ...
                .buttonStyle(.plain)
                .keyboardShortcut(.return, modifiers: [])
                .accessibilityLabel("Send message to Blink task")
                .help("Send message to Blink task")
            }
        }
        .padding(.horizontal, 12)
        .padding(.vertical, 10)
    ...
    private func connectionRow(_ row: BlinkNotchConnectionRow) -> some View {
        HStack(spacing: 10) {
    ...
    private func submitQuickAskPrompt() {
        ...
        // expandTextInput dialog ("Ask Blink / Voice / Text / Agent"),
                ...
                HStack(spacing: 8) {
                    ...
                    VStack(alignment: .leading, spacing: 1) {
                        Text("Drop images or docs")
                            .font(panelUIFont(size: 11, weight: .heavy))
                            .foregroundColor(DS.Colors.textPrimary)
                        Text("Blink will attach them before sending")
    ...
    private func refreshGogStatus() async {
        gogStatus = await BlinkGogCLIStatusResolver.refresh()
        hasLoadedGogStatus = true
    }

    private func notifyPanelSizeChanged() {
        NotificationCenter.default.post(
            name: .blinkPanelContentSizeDidChange,
            object: nil,
...
private enum BlinkAgentSessionFilter: String, CaseIterable, Identifiable {
    case active
    case running
    case completed
    case archived
    ...
    var accessibilityLabel: String { "Show \(title.lowercased()) Blink agent tasks" }
    ...
    var emptyStateSubtitle: String {
        ...
        case .running: return "Running tasks will show here while Blink works."
...
private enum BlinkAgentPanelSelection: String, Equatable {
    case sessions
    case specialists
}

private enum BlinkNotchTab: String, CaseIterable, Identifiable {
    case home
    case agents
    case connections
    case settings

    static let primaryTabs: [BlinkNotchTab] = [.home, .agents, .connections]
...
private enum BlinkQuickPromptMode: Equatable {
    case ask
    case agent
    case chat
    ...
    var title: String {
        switch self {
        case .ask: return "Ask Blink"
        case .agent: return "Task an agent"
        case .chat: return "Chat inside Blink"
        }
    }
    ...
    var subtitle: String {
        ...
        case .agent:
            return "Write the task here, then press Return to launch an Blink background agent."
        case .chat:
            return "Send here to expand the panel into the active Blink chat."
        }
    }
    ...
    var placeholder: String {
        ...
        case .ask: return "Ask Blink…"
        case .agent: return "Task an agent…"
        case .chat: return "Chat with Blink…"
        }
    }
...
private struct BlinkNotchConnectionRow: Identifiable, Equatable {
    let id = UUID()
    let title: String
    let detail: String
    let state: BlinkNotchConnectionState
    let systemImageName: String
}

private enum BlinkNotchConnectionState: Equatable {
    case ready
    case available
    case needsAttention
...
private struct BlinkPanelTypography {
    ...
    private var appFont: BlinkResponseCaptionFont {
        BlinkResponseCaptionFont.resolved(fontRawValue)
    }
...
private struct BlinkNotchHeroCard<Content: View>: View {
    @AppStorage(AppBundleConfiguration.userAppFontDefaultsKey) private var appFontRawValue = BlinkResponseCaptionFont.fallback.rawValue
    @AppStorage(AppBundleConfiguration.userAppBoldTextDefaultsKey) private var appBoldTextEnabled = false
    @AppStorage(AppBundleConfiguration.userAppTitleFontSizeDefaultsKey) private var appTitleFontSize = 26.0
    @AppStorage(AppBundleConfiguration.userAppBodyFontSizeDefaultsKey) private var appBodyFontSize = 13.0
    ...
    private var typography: BlinkPanelTypography {
        BlinkPanelTypography(
...
private struct BlinkNotchMetricCard: View {
    @AppStorage(AppBundleConfiguration.userAppFontDefaultsKey) private var appFontRawValue = BlinkResponseCaptionFont.fallback.rawValue
    @AppStorage(AppBundleConfiguration.userAppBoldTextDefaultsKey) private var appBoldTextEnabled = false
    @AppStorage(AppBundleConfiguration.userAppTitleFontSizeDefaultsKey) private var appTitleFontSize = 26.0
    @AppStorage(AppBundleConfiguration.userAppBodyFontSizeDefaultsKey) private var appBodyFontSize = 13.0
...
private struct BlinkPanelDragHandle: NSViewRepresentable {
    func makeNSView(context: Context) -> BlinkPanelDragHandleView {
        BlinkPanelDragHandleView()
    }

    func updateNSView(_ nsView: BlinkPanelDragHandleView, context: Context) {}
}

private final class BlinkPanelDragHandleView: NSView {
...
private struct BlinkBottomRoundedRectangle: Shape {
...
private struct BlinkRunningAgentIndicator: View {
    let color: Color
...
private struct BlinkNotchEmptyState: View {
    @AppStorage(AppBundleConfiguration.userAppFontDefaultsKey) private var appFontRawValue = BlinkResponseCaptionFont.fallback.rawValue
    @AppStorage(AppBundleConfiguration.userAppBoldTextDefaultsKey) private var appBoldTextEnabled = false
    @AppStorage(AppBundleConfiguration.userAppTitleFontSizeDefaultsKey) private var appTitleFontSize = 26.0
    @AppStorage(AppBundleConfiguration.userAppBodyFontSizeDefaultsKey) private var appBodyFontSize = 13.0
