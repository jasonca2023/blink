...
//   2. `[BLINK_3D] prompt: <prompt>`       — explicit sentinel for LLMs
//   3. `[BLINK_3D] prompt: "<p>" style: "<s>"`  — with optional style
//
...
enum ThreeDGenerationDispatcher {
    ...
    static func scanAndDispatch(_ text: String) -> [Match] {
        ...
        // 2. [BLINK_3D] sentinel — anywhere in text, possibly multi-line.
    ...
    /// Find every `[BLINK_3D] prompt: ...` occurrence in arbitrary text.
    private static func parseSentinels(_ text: String) -> [Match] {
        let pattern = #"\[BLINK_3D\][^\n]*"#
    ...
    /// already have system-prompt machinery in Blink.
    ...
    3D model generation is available. When the user asks you to make, create, generate, sculpt, or visualise a 3D object, prop, asset, character, or artifact, emit a single line in your reply with EXACTLY this format and nothing else on that line:
        [BLINK_3D] prompt: "<short description of the object>" style: "low_poly_stylized"
    Blink will detect that line, start a 3D generation job, and show the rotating preview in a floating window. The default style is low_poly_stylized; alternatives are clay, voxel, game_asset, realistic.
    ...
    You may include normal conversational text before or after the line. Do NOT call any tool, do NOT use Bash, do NOT try to write any files yourself — just emit the [BLINK_3D] line and Blink handles the rest.
    """
}
