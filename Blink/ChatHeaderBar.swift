...
struct ChatHeaderBar: View {
  ...
  var body: some View {
    HStack(spacing: 6) {
      ...
      iconButton(systemName: "xmark", help: "Close Blink HUD") {
        dismissHUD()
      }
  ...
  private var headerTitle: String {
    "Blink " + currentModelLabel
  }

  private func modelButton(_ opt: BlinkModelOption) -> some View {
  ...
  private var claudeOptions: [BlinkModelOption] {
    BlinkModelCatalog.voiceResponseModels.filter { $0.provider == .anthropic }
  }

  private var codexOptions: [BlinkModelOption] {
    BlinkModelCatalog.codexActionsModels
  }

  private func selectModel(_ id: String) {
    session.model = id
    UserDefaults.standard.set(id, forKey: "blinkCodexModel")
  }
