...
enum BlinkCompanionRuntimeMode {
    case menuBar
    case embeddedWindow
}

struct BlinkExternalProxyCursor: Identifiable {
    let id: UUID
...
final class CursorOverlayState: ObservableObject {
    ...
    @Published var externalSecondaryCursors: [BlinkExternalProxyCursor] = []
}

enum BlinkAgentDockStatus: Equatable {
    case starting
    case running
    case done
    case failed
...
struct BlinkAgentDockItem: Identifiable, Equatable {
    let id: UUID
    let sessionID: UUID?
    var title: String
    ...
    var accentTheme: BlinkAccentTheme
    var status: BlinkAgentDockStatus
...
private struct BlinkAppOpenRequest {
    let appName: String
    let instruction: String
}

private struct BlinkAgentSelectionRequest {
    let agentName: String
    let followUpText: String?
    let instruction: String
}

private struct BlinkNativeTypeRequest {
    let text: String
    let targetDescription: String
}

private struct BlinkNativeKeyPressRequest {
    let key: String
    let modifiers: [String]
    let targetDescription: String
}

private struct BlinkFolderOpenRequest {
    let url: URL
    let displayName: String
    let instruction: String
}

private struct BlinkWebOpenRequest {
    let url: URL
    let displayName: String
    let instruction: String
}

private struct BlinkReminderAddRequest {
    let title: String
    let instruction: String
}

private struct BlinkReminderCountRequest {
    let instruction: String
}

private struct BlinkMessagesSearchRequest {
    let personName: String
    let instruction: String
}

private struct BlinkLocalAutomationResult: Sendable {
    let output: String
    let errorOutput: String
    let terminationStatus: Int32
}

private struct BlinkRequestTiming {
...
private final class BlinkRequestCompletionState: @unchecked Sendable {
    var didComplete = false
}

private enum BlinkLocalAutomationRunner {
    static func runAppleScript(_ script: String) -> BlinkLocalAutomationResult {
        let process = Process()
        let outputPipe = Pipe()
        let errorPipe = Pipe()
        ...
        do {
            ...
            return BlinkLocalAutomationResult(
                output: "",
...
final class CompanionManager: ObservableObject {
    ...
    let slideshowWindowManager = BlinkSlideshowWindowManager()

    func showSlideshow() {
        slideshowWindowManager.show()
    }
    ...
    let textModeWindowManager = BlinkTextModeWindowManager()
    let agentDockWindowManager = BlinkAgentDockWindowManager()
    let agentMenuBarStatusManager = AgentMenuBarStatusManager()
    let settingsWindowManager = BlinkSettingsWindowManager()
    let logViewerWindowManager = BlinkLogViewerWindowManager()
    let widgetStateStore = BlinkWidgetStateStore()
    let codexHomeManager = CodexHomeManager()
    let nativeComputerUseController = BlinkNativeComputerUseController()
    let backgroundComputerUseController = BlinkBackgroundComputerUseController()
    ...
    @Published private(set) var latestVoiceResponseCard: BlinkResponseCard?
    @Published private(set) var handoffQueue: [HandoffQueuedRegionScreenshot] = []
    @Published private(set) var agentDockItems: [BlinkAgentDockItem] = []
    ...
    private lazy var claudeAPI: ClaudeAPI = {
        let modelOption = BlinkModelCatalog.voiceResponseModel(withID: selectedModel)
        return ClaudeAPI(
    ...
    private lazy var systemTTSClient: SystemTTSClient = {
    ...
    private var richResponseObserverCancellable: AnyCancellable?

    /// Posts the most recent voice response to the rich showcase panel
    /// using `lastVoiceUserTranscript` as the image search query.
    /// No-op when the response text is empty or duplicates the previous
    /// surfaced card.
    private var lastSurfacedRichResponseCardID: String?
    func surfaceRichResponse(for card: BlinkResponseCard) {
        guard card.id != lastSurfacedRichResponseCardID else { return }
        lastSurfacedRichResponseCardID = card.id
        let transcriptForImage = lastVoiceUserTranscript ?? card.contextTitle ?? ""
        playResponseReadyChime()
        richResponseWindowManager.show(
            transcript: transcriptForImage,
            responseText: card.displayText
        )
    }

    /// Auto-observer removed: the rich panel is now opt-in, surfaced
    /// only when the brain router explicitly chooses speak_answer with
    /// needs_visual=true. URL opens, app launches, web searches, and
    /// background agent dispatches stay graphics-free.
    private func observeRichResponseCardUpdates() {
        // Intentionally empty — see surfaceRichResponse callers.
    }

    // MARK: - Brain router hooks

    /// Lazy-initialised intent router. Keeps Claude wired only when an
    /// Anthropic key is available; falls back to the legacy regex flow
    /// when missing.
    private lazy var brainRouter: BlinkBrainRouter = {
        return BlinkBrainRouter(companionManager: self, openAIAPI: openAIAPI)
    }()

    /// Calls the brain router with the user transcript. Returns true
    /// when the brain dispatched an action; false on auth/transport
    ...
    /// Siri voice by default) without spawning a full Claude response.
    func brainSpeak(_ text: String) {
    ...
    func brainOpenApp(_ appName: String) {
        let appRequest = BlinkAppOpenRequest(appName: appName, instruction: "open \(appName)")
        let didOpen = openRequestedApplication(appRequest, shouldSpeak: false)
        if didOpen { return }

        // Silent fallback: the model called open_app for something
        // that isn't an installed Mac app (e.g. "github"). Try it as
        // a brand URL instead of speaking the legacy "couldn't open
        // through native CUA" refusal.
        let lowered = appName
            .folding(options: .diacriticInsensitive, locale: .current)
            .lowercased()
            .trimmingCharacters(in: CharacterSet(charactersIn: " .,!?:;"))
        if let host = Self.knownBrandHost(for: lowered),
           let url = URL(string: "https://\(host)") {
            NSWorkspace.shared.open(url)
            return
        }
        if let url = URL(string: "https://\(lowered).com"),
           lowered.range(of: #"^[a-z0-9-]+$"#, options: .regularExpression) != nil {
            NSWorkspace.shared.open(url)
        }
    }

    /// Submits the brain's task description to the background Codex
    /// agent so longer work runs without blocking the voice turn.
    func brainSpawnAgent(instruction: String) {
        submitAgentPromptFromUI(instruction)
        brainSpeak("On it. I'll handle this in the background.")
    }

    /// Posts a CGEvent click at the given screen coordinates. Used by
    /// the brain when the model grounds a UI element it can see in the
    /// supplied screenshot.
    func brainClickAt(x: Double, y: Double, label: String?) {
        do {
            try BlinkComputerUseMouseInput.click(at: CGPoint(x: x, y: y))
            if let label, !label.isEmpty {
                brainSpeak("Clicking \(label)")
            }
        } catch {
            print("brainClickAt error: \(error.localizedDescription)")
        }
    }

    /// Types literal text into the focused field via CGEvent keystrokes.
    /// Used by the brain when filling in URL bars, search fields, etc.
    func brainTypeText(_ text: String) {
        guard !text.isEmpty else { return }
        do {
            try BlinkComputerUseKeyboardInput.typeCharacters(text, delayMilliseconds: 25, toPid: nil)
        } catch {
            print("brainTypeText error: \(error.localizedDescription)")
        }
    }

    /// Parses a "+" / "-" separated key combo like "cmd+shift+m" or
    /// "ctrl+option+escape" and posts it via CGEvent. Used for native
    /// shortcuts ("end screen sharing" → cmd+shift+m in Zoom etc.).
    func brainPressKeys(combo: String) {
        let parts = combo
            .lowercased()
        ...
        do {
            try BlinkComputerUseKeyboardInput.press(
                key,
                modifiers: Array(modifiers),
                toPid: nil
            )
        } catch {
            print("brainPressKeys error: \(error.localizedDescription)")
        }
    }

    /// Schedules a delayed removal of a completed agent dock item so
    ...
    /// Best-effort JPEG screenshot of the cursor's screen. Returns nil
    /// when ScreenCaptureKit is not permitted yet — the brain still
    /// runs without vision, just without UI grounding.
    func brainCaptureScreenshot() async -> Data? {
        do {
            let captures = try await CompanionScreenCaptureUtility.captureCursorScreenAsJPEG()
            return captures.first?.imageData
        } catch {
            print("brainCaptureScreenshot error: \(error.localizedDescription)")
            return nil
        }
    }
    ...
    /// `openBlinkTTSProvider` for compatibility with earlier builds.
    @Published var selectedTTSProvider: BlinkTTSProvider = {
        let migrationKey = "openBlinkRealtimeSpeechPlaybackMigrationV1"
        let explicitSpeechModel = UserDefaults.standard.string(forKey: "openBlinkSpeechModel")
        ...
        if explicitSpeechModel != nil,
           rawPlaybackEngine != BlinkTTSProvider.openAIRealtime.rawValue,
           !UserDefaults.standard.bool(forKey: migrationKey) {
            UserDefaults.standard.set(BlinkTTSProvider.openAIRealtime.rawValue, forKey: AppBundleConfiguration.userTTSProviderDefaultsKey)
            UserDefaults.standard.set(true, forKey: migrationKey)
            return .openAIRealtime
        }
        return BlinkTTSProvider.resolve(AppBundleConfiguration.ttsProviderRaw())
    }()
    ...
    /// Blink's spoken replies.
    @Published var selectedSpeechModel: String = BlinkModelCatalog.speechModel(
        withID: UserDefaults.standard.string(forKey: "openBlinkSpeechModel")
    ).id
    ...
    var voiceTTSClient: any BlinkTTSClient {
        switch selectedTTSProvider {
        case .openAITTS: return openAITTSClient
        case .system: return systemTTSClient
    ...
    private var activeTTSExecutionMethodSpeakText: String {
        switch selectedTTSProvider {
        case .openAITTS: return "OpenAITTSClient.speakText"
        case .system: return "SystemTTSClient.speakText"
    ...
    private var activeTTSExecutionMethodBeginStreaming: String {
        switch selectedTTSProvider {
        case .openAITTS: return "OpenAITTSClient.beginStreamingResponse"
        case .system: return "SystemTTSClient.beginStreamingResponse"
    ...
    func setTTSProvider(_ provider: BlinkTTSProvider) {
    ...
    func setSelectedSpeechModel(_ model: String) {
        let resolvedModel = BlinkModelCatalog.speechModel(withID: model).id
        ...
        UserDefaults.standard.set(resolvedModel, forKey: "openBlinkSpeechModel")
        setTTSProvider(.openAIRealtime)
    }
    ...
    private func voiceConversationHistoryForAPI() -> [(userPlaceholder: String, assistantResponse: String)] {
        ...
        if let compactedVoiceConversationArchive,
           !compactedVoiceConversationArchive.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            history.append((
                userPlaceholder: "[earlier Blink voice context]",
    ...
    private func compactVoiceConversationHistoryIfNeeded(reason: String) {
        ...
        let archiveChunk = archivedEntries.map { entry in
            "User: \(Self.voiceArchiveSnippet(entry.userTranscript))\nBlink: \(Self.voiceArchiveSnippet(entry.assistantResponse))"
        }.joined(separator: "\n")
    ...
    private var activeRequestTiming: BlinkRequestTiming?
    private var agentRequestTimingsBySessionID: [UUID: BlinkRequestTiming] = [:]
    ...
    private var externalControlBridgeServer: BlinkExternalControlBridgeServer?
    ...
    var latestResponseCard: BlinkResponseCard? {
    ...
    init(runtimeMode: BlinkCompanionRuntimeMode = .menuBar) {
    ...
    @Published var selectedModel: String = BlinkModelCatalog.voiceResponseModel(
        withID: UserDefaults.standard.string(forKey: "selectedVoiceResponseModel")
            ...
            ?? BlinkModelCatalog.defaultVoiceResponseModelID
    ).id
    @Published var selectedComputerUseModel: String = BlinkModelCatalog.computerUseModel(
        withID: UserDefaults.standard.string(forKey: "selectedComputerUseModel") ?? BlinkModelCatalog.defaultComputerUseModelID
    ).id
    @Published var selectedComputerUseBackendID: String = BlinkComputerUseBackendID.resolving(
    ...
    func setSelectedModel(_ model: String) {
        let selectedVoiceResponseModel = BlinkModelCatalog.voiceResponseModel(withID: model)
        ...
        if BlinkModelCatalog.isSpeechModelID(resolvedModel) {
            selectedSpeechModel = resolvedModel
            UserDefaults.standard.set(resolvedModel, forKey: "openBlinkSpeechModel")
    ...
    func setSelectedComputerUseModel(_ model: String) {
        let resolvedModel = BlinkModelCatalog.computerUseModel(withID: model).id
        selectedComputerUseModel = resolvedModel
        UserDefaults.standard.set(resolvedModel, forKey: "selectedComputerUseModel")
    }

    var selectedComputerUseBackend: BlinkComputerUseBackendID {
        BlinkComputerUseBackendID.resolving(selectedComputerUseBackendID)
    }

    private func applyVoiceResponseModelSettings(_ modelOption: BlinkModelOption) {
    ...
    func setSelectedComputerUseBackend(_ backendID: String) {
        let backend = BlinkComputerUseBackendID.resolving(backendID)
    ...
    func openFullDiskAccessSettings() {
        NSWorkspace.shared.open(BlinkMacPrivacyPermissionProbe.fullDiskAccessSettingsURL)
    }

    func openAutomationSettings() {
        NSWorkspace.shared.open(BlinkMacPrivacyPermissionProbe.automationSettingsURL)
    }
    ...
    func requestRemindersAutomationPermission() {
        ...
        Task.detached(priority: .userInitiated) {
            let result = BlinkLocalAutomationRunner.runAppleScript("""
    ...
    func handleWidgetDeepLink(_ url: URL) {
        guard url.scheme == "blink" else { return }
    ...
    /// User preference for whether the Blink cursor should be shown.
    /// When toggled off, the overlay is hidden and push-to-talk is disabled.
    /// Persisted to UserDefaults so the choice survives app restarts.
    @Published var isBlinkCursorEnabled: Bool = UserDefaults.standard.object(forKey: "isBlinkCursorEnabled") == nil
        ? true
        : UserDefaults.standard.bool(forKey: "isBlinkCursorEnabled")

    @Published var isActivationShortcutEnabled: Bool = true

    func setBlinkCursorEnabled(_ enabled: Bool) {
        isBlinkCursorEnabled = enabled
        UserDefaults.standard.set(enabled, forKey: "isBlinkCursorEnabled")
    ...
    func start() {
        // Force OpenAI HTTP TTS as the provider on every launch — simple
        // POST to /v1/audio/speech, plays mp3 via AVAudioPlayer. Way more
        // reliable than the WebSocket Realtime path. Uses OPENAI_API_KEY
        // from ~/.config/blink/secrets.env.
        if selectedTTSProvider != .openAITTS {
            selectedTTSProvider = .openAITTS
            UserDefaults.standard.set(
                BlinkTTSProvider.openAITTS.rawValue,
                forKey: AppBundleConfiguration.userTTSProviderDefaultsKey
            )
        }

        // Pin the background-agent dock to the top-right corner on every
        // launch so it never lands at the bottom of the screen.
        if agentParkingPosition != .topRight {
            agentParkingPosition = .topRight
            UserDefaults.standard.set(
                AgentParkingPosition.topRight.rawValue,
                forKey: AgentParkingPosition.userDefaultsKey
            )
        }

        loadBundledKnowledgeIndex()
        refreshAllPermissions()
        // Drop the stale UserDefault from the removed "replace system
        // cursor" feature so anyone who had toggled it on doesn't carry
        // around an orphaned key.
        UserDefaults.standard.removeObject(forKey: "blinkReplaceSystemCursorEnabled")
        ...
        print("Blink runtime identity - bundleID: \(Bundle.main.bundleIdentifier ?? "unknown"), appPath: \(Bundle.main.bundleURL.path)")
        print("Blink start - accessibility: \(hasAccessibilityPermission), screen: \(hasScreenRecordingPermission), mic: \(hasMicrophonePermission), screenContent: \(hasScreenContentPermission), fullDiskAccess: \(hasFullDiskAccessPermission), onboarded: \(hasCompletedOnboarding)")
        ...
        let selectedVoiceResponseModel = BlinkModelCatalog.voiceResponseModel(withID: selectedModel)
        switch selectedVoiceResponseModel.provider {
        case .anthropic:
        ...
        case .openAI where BlinkModelCatalog.isSpeechModelID(selectedVoiceResponseModel.id):
            ...
            UserDefaults.standard.set(BlinkTTSProvider.openAIRealtime.rawValue, forKey: AppBundleConfiguration.userTTSProviderDefaultsKey)
            UserDefaults.standard.set(selectedVoiceResponseModel.id, forKey: "openBlinkSpeechModel")
            openAIRealtimeSpeechClient.warmUpConnection()
        case .deepgram:
            selectedSpeechModel = selectedVoiceResponseModel.id
            UserDefaults.standard.set(selectedVoiceResponseModel.id, forKey: "openBlinkSpeechModel")
        ...
        if hasCompletedOnboarding && isBlinkCursorEnabled {
            showCursorOverlayIfAvailable()
        }

        observeRichResponseCardUpdates()
    }
    ...
    func triggerOnboarding() {
        NotificationCenter.default.post(name: .blinkDismissPanel, object: nil)
        hasCompletedOnboarding = true

        BlinkAnalytics.trackOnboardingStarted()
    ...
    private func startOnboardingMusic() {
        ...
        guard let musicURL = Bundle.main.url(forResource: "ff", withExtension: "mp3") else {
            print("⚠️ Blink: ff.mp3 not found in bundle")
            return
        }
        ...
        do {
            ...
            print("⚠️ Blink: Failed to play onboarding music: \(error)")
        }
    }
    ...
    private func startExternalControlBridgeIfNeeded() {
        ...
        let server = BlinkExternalControlBridgeServer { [weak self] command in
            guard let self else {
                return .error(503, "Blink is not ready")
            }
    ...
    private func handleExternalControlCommand(_ command: BlinkExternalControlCommand) async -> BlinkExternalControlResponse {
        switch command {
        ...
        case .notify(let title, let body, let threadID, let sound):
            let identifier = BlinkDesktopNotificationCenter.shared.post(
                title: title,
                body: body,
                threadID: threadID ?? "blink.external",
                playSound: sound,
    ...
    private func showExternalPrimaryCursor(at point: CGPoint, caption: String?, duration: TimeInterval, accentHex: String?, travelDuration: TimeInterval) {
        ...
        // Use Blink's existing smooth pointing choreography — the same
        // path used when voice asks "show me the Apple menu". This makes the
        // little Blink triangle detach, zip to the target, caption it, and
    ...
    private func showExternalSecondaryCursor(at point: CGPoint, caption: String?, duration: TimeInterval, accentHex: String?) {
        let targetPoint = Self.clampedExternalCursorPoint(point)
        let id = UUID()
        let cursor = BlinkExternalProxyCursor(
            id: id,
    ...
    private func animateAgentSpawnProxyFromCursorToDock(accentTheme: BlinkAccentTheme, caption: String? = nil) {
        ...
        // For agent starts, use the primary Blink buddy itself rather
    ...
    private static func cursorHex(for accentTheme: BlinkAccentTheme) -> String {
        switch accentTheme {
        case .blue:
            return "#3380FF"
        case .mint:
    ...
    private func captureExternalControlScreenshots(focused: Bool) async -> BlinkExternalControlResponse {
        do {
            ...
            let directory = FileManager.default.temporaryDirectory
                .appendingPathComponent("BlinkExternalControlScreenshots", isDirectory: true)
    ...
    private func speakExternalProxyText(_ text: String, interrupt: Bool) -> BlinkExternalControlResponse {
        let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return .error(400, "Missing text") }
        if voiceTTSClient.isPlaying {
            guard interrupt else {
                return .error(409, "Blink voice is already playing; retry or pass interrupt=true")
            }
    ...
    func refreshAllPermissions() {
        ...
        hasFullDiskAccessPermission = BlinkMacPrivacyPermissionProbe.hasLikelyFullDiskAccess()
        ...
        if !previouslyHadAccessibility && hasAccessibilityPermission {
            BlinkAnalytics.trackPermissionGranted(permission: "accessibility")
        }
        if !previouslyHadScreenRecording && hasScreenRecordingPermission {
            BlinkAnalytics.trackPermissionGranted(permission: "screen_recording")
        }
        if !previouslyHadMicrophone && hasMicrophonePermission {
            BlinkAnalytics.trackPermissionGranted(permission: "microphone")
        }
        if !previouslyHadFullDiskAccess && hasFullDiskAccessPermission {
            BlinkAnalytics.trackPermissionGranted(permission: "full_disk_access")
        }

        if !previouslyHadAll && allPermissionsGranted {
            BlinkAnalytics.trackAllPermissionsGranted()
        }
    }
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    BlinkAnalytics.trackPermissionGranted(permission: "screen_content")

                    // If onboarding was already completed, show the cursor overlay now
                    if hasCompletedOnboarding && !isOverlayVisible && isBlinkCursorEnabled {
                        showCursorOverlayIfAvailable()
                    }
                }
            } catch {
    ...
    private func loadBundledKnowledgeIndex() {
        ...
        do {
            ...
            print("⚠️ Blink memory index load failed: \(error)")
    ...
    @discardableResult
    func createAndSelectNewCodexAgentSession(title: String? = nil, accentTheme: BlinkAccentTheme? = nil) -> CodexAgentSession {
    ...
    func createAndLaunchCodexAgentSession(
        title: String? = nil,
        prompt: String,
        accentTheme: BlinkAccentTheme? = nil,
    ...
    func closeCodexAgentSession(_ sessionID: UUID) {
        ...
        BlinkMessageLogStore.shared.append(
            lane: "agent",
            direction: "incoming",
            event: "blink.agent_session.closed",
            fields: [
    ...
    private func beginRequestTiming(source: String, text: String) -> BlinkRequestTiming {
        let timing = BlinkRequestTiming(
    ...
    private func withActiveRequestTiming<T>(_ timing: BlinkRequestTiming, perform work: () -> T) -> T {
    ...
    private func markRequestExecutionStarted(
        ...
        BlinkMessageLogStore.shared.append(
            lane: "request",
            direction: "outgoing",
            event: "blink.request.execution_started",
            fields: requestTimingFields(
    ...
    private func markRequestStageCompleted(
        ...
        BlinkMessageLogStore.shared.append(
            lane: "request",
            direction: "outgoing",
            event: "blink.request.stage_completed",
            fields: requestTimingFields(
                timing ?? activeRequestTiming,
                route: route,
                status: status,
    ...
    private func voiceResponseExecutionFields() -> [String: Any] {
        ...
        case .openAI:
            if BlinkModelCatalog.isSpeechModelID(selectedVoiceResponseModel.id) {
                ...
                fields["playbackEngine"] = BlinkTTSProvider.openAIRealtime.rawValue
    ...
    private func handleShortcutTransition(_ transition: BuddyPushToTalkShortcut.ShortcutTransition) {
        ...
        case .pressed:
            ...
            if !isBlinkCursorEnabled && !isOverlayVisible {
                overlayWindowManager.hasShownOverlayBefore = true
                overlayWindowManager.showOverlay(onScreens: NSScreen.screens, companionManager: self)
                isOverlayVisible = true
            }
            ...
            BlinkAnalytics.trackPushToTalkStarted()
        ...
        case .released:
            ...
            BlinkAnalytics.trackPushToTalkReleased()
            pendingKeyboardShortcutStartTask?.cancel()
    ...
    private var shouldUseBidirectionalRealtimeVoiceInput: Bool {
        BlinkModelCatalog.isSpeechModelID(selectedModel)
    }

    private var activeRealtimeSpeechVoiceID: String {
        let model = BlinkModelCatalog.voiceResponseModel(withID: selectedModel)
        ...
        realtimeBidirectionalVoiceTask = Task { [weak self] in
            do {
                ...
                let onAssistantTextChunk: @MainActor @Sendable (String) -> Void = { [weak self] accumulatedText in
                    ...
                    self?.latestVoiceResponseCard = BlinkResponseCard(
                        source: .voice,
                        rawText: trimmed,
                ...
                let selectedVoiceResponseModel = BlinkModelCatalog.voiceResponseModel(withID: self.selectedModel)
        ...
        realtimeBidirectionalVoiceTask = Task { [weak self] in
            do {
                ...
                let assistantText = result.assistantTranscript.isEmpty
                    ? (result.didCreateAssistantResponse ? "Done." : "Routed to Blink.")
                ...
                if result.didCreateAssistantResponse && !result.userTranscript.isEmpty && !wasRoutedByApp {
                    do {
                        ...
                        print("⚠️ Blink memory update failed: \(error)")
                    }
                    BlinkAnalytics.trackAIResponseReceived(response: assistantText)
                }
            } catch {
                await MainActor.run {
    ...
    private func handleFinalVoiceTranscript(_ finalTranscript: String) {
        // Short percussive click — gives the user immediate feedback
        // that Blink committed to processing their utterance.
        playUserInputChime()
        ...
        BlinkAnalytics.trackUserMessageSent(transcript: finalTranscript)
        ...
        // Agent-status / cancellation are the only legacy fast-paths
        // worth keeping — they need to be instant and don't depend on
        // screen context. Everything else goes to the brain.
        if handleAgentCancellationRequestIfNeeded(from: finalTranscript) { return }
        if handleAgentStatusQuestionIfNeeded(from: finalTranscript) { return }
        ...
        // Cancel any in-flight speculative pre-fire — the brain router
        // is the only thing that should land a response now.
        cancelSpeculativeFireIfActive()

        // Brain router: Claude tool-use classifies the intent and calls
        // exactly one of open_url / open_app / speak_answer /
        // run_background_agent / web_search. Always runs first — only
        // falls back to the legacy screenshot pipeline if the brain
        // cannot reach Claude at all.
        Task { [weak self] in
            guard let self else { return }
            let handled = await self.routeThroughBrain(transcript: finalTranscript)
            if !handled {
                await MainActor.run {
    ...
    /// Cancels any pending speculative pre-fire Claude task so the
    /// brain router's verdict is the only response that lands.
    private func cancelSpeculativeFireIfActive() {
        speculativeStabilityDwellTask?.cancel()
        speculativeStabilityDwellTask = nil
        discardActiveSpeculativeFire(reason: "brain_router_authoritative")
    }
            ...
            BlinkMessageLogStore.shared.append(
                ...
                fields: [
                    ...
                    "executionMethod": backend == .backgroundComputerUse
                        ...
                        : "BlinkNativeComputerUseController.pressKey",
    ...
    private func startImplicitAgentTaskIfNeeded(from finalTranscript: String) -> Bool {
        ...
        BlinkMessageLogStore.shared.append(
            lane: "agent",
            direction: "incoming",
            event: "blink.agent_task.implicit_route",
            fields: [
    ...
    private func handleDirectComputerUseRequest(from transcript: String, source: String) -> Bool {
        // Pre-empt the app-open path when the transcript clearly names a URL
        // or domain (e.g. "open github.com", "go to apple.com/store"). Without
        // this, localAppOpenRequest swallows "open <anything>" and never lets
        // the URL handler fire.
        if Self.transcriptLooksLikeURL(transcript), let webOpenRequest = Self.webOpenRequest(from: transcript) {
            BlinkMessageLogStore.shared.append(
                lane: "computer-use",
                direction: "incoming",
                event: "native_cua.direct_request.web_detected_preempt",
                fields: [
                    "source": source,
        ...
        if let reminderAddRequest = Self.reminderAddRequest(from: transcript) {
            BlinkMessageLogStore.shared.append(
                ...
                fields: [
                    ...
                    "executionMethod": "BlinkLocalAutomationRunner.runAppleScript",
            ...
            BlinkMessageLogStore.shared.append(
                ...
                fields: [
                    ...
                    "executionMethod": backend == .backgroundComputerUse
                        ...
                        : "BlinkNativeComputerUseController.pressKey/typeText",
            ...
            BlinkMessageLogStore.shared.append(
                ...
                fields: [
                    ...
                    "executionMethod": backend == .backgroundComputerUse
                        ...
                        : "BlinkNativeComputerUseController.typeText",
    ...
    private func openRequestedWebsite(_ request: BlinkWebOpenRequest, shouldSpeak: Bool = true) {
        let executionStartedAt = markRequestExecutionStarted(
            route: "native_cua.open_url",
            extra: [
                "executor": "native_cua",
        ...
        latestVoiceResponseCard = BlinkResponseCard(
            source: .voice,
    ...
    private func openRequestedApplication(
        ...
        // Refusal speech intentionally suppressed — the brain router
        // owns failure handling now and falls back to URL opens for
        // anything that isn't a real Mac app.
        _ = shouldSpeak
        print("openRequestedApplication: failed to open \(request.appName)")
        if logTiming {
            markRequestCompleted(
                route: "native_cua.open_app",
                executionStartedAt: executionStartedAt,
    ...
    private func openRequestedFolder(_ request: BlinkFolderOpenRequest, shouldSpeak: Bool = true) {
        let executionStartedAt = markRequestExecutionStarted(
            route: "native_cua.open_folder",
            extra: [
                "executor": "native_cua",
        ...
        BlinkDirectActionMemoryStore.shared.recordFolderShortcut(
            instruction: request.instruction,
            url: request.url,
    ...
    private func folderOpenRequest(from transcript: String) -> BlinkFolderOpenRequest? {
    ...
    private func addReminderUsingNativeAutomation(_ request: BlinkReminderAddRequest) {
        interruptCurrentVoiceResponse()
        ...
        Task.detached(priority: .userInitiated) {
            let titleLiteral = BlinkLocalAutomationRunner.appleScriptStringLiteral(title)
            let script = """
            tell application "Reminders"
            ...
            """
            let result = BlinkLocalAutomationRunner.runAppleScript(script)
    ...
    private func countRemindersUsingNativeAutomation(_ request: BlinkReminderCountRequest) {
        interruptCurrentVoiceResponse()
        let timing = activeRequestTiming
        let executionStartedAt = markRequestExecutionStarted(
            route: "native_cua.reminder_count",
    ...
    private func searchMessagesUsingSelectedComputerUse(_ request: BlinkMessagesSearchRequest) {
    ...
    private func typeTextUsingSelectedComputerUse(_ request: BlinkNativeTypeRequest) {
        switch selectedComputerUseBackend {
        case .backgroundComputerUse:
    ...
    private func pressKeyUsingSelectedComputerUse(_ request: BlinkNativeKeyPressRequest, shouldSpeak: Bool = true) {
        switch selectedComputerUseBackend {
        case .backgroundComputerUse:
    ...
    private func searchMessagesUsingBackgroundComputerUse(_ request: BlinkMessagesSearchRequest) {
        interruptCurrentVoiceResponse()
        let timing = activeRequestTiming
        let executionStartedAt = markRequestExecutionStarted(
            ...
            extra: [
                ...
                "controller": "BlinkBackgroundComputerUseController",
                "appName": "Messages",
                "personName": request.personName,
                "runtimeStatus": backgroundComputerUseController.status.summary
            ]
        )
        let appRequest = BlinkAppOpenRequest(appName: "Messages", instruction: "Open Messages.")
    ...
    private func typeTextUsingBackgroundComputerUse(_ request: BlinkNativeTypeRequest) {
        interruptCurrentVoiceResponse()
        let executionStartedAt = markRequestExecutionStarted(
    ...
    private func pressKeyUsingBackgroundComputerUse(_ request: BlinkNativeKeyPressRequest, shouldSpeak: Bool = true) {
        if shouldSpeak {
            interruptCurrentVoiceResponse()
        }
        let executionStartedAt = markRequestExecutionStarted(
    ...
    private func searchMessagesUsingNativeComputerUse(_ request: BlinkMessagesSearchRequest) {
        interruptCurrentVoiceResponse()
        let timing = activeRequestTiming
        let executionStartedAt = markRequestExecutionStarted(
            ...
            extra: [
                "executor": "native_cua",
                "executionMethod": "BlinkNativeComputerUseController.pressKey/typeText",
                "controller": "BlinkNativeComputerUseController",
                "appName": "Messages",
                "personName": request.personName
            ]
        )
        ...
        Task { @MainActor in
            ...
            guard let targetWindow = nativeComputerUseController.refreshFocusedTarget() else {
                BlinkMessageLogStore.shared.append(
                    ...
                    fields: [
                        "executor": "native_cua",
                        "executionMethod": "BlinkNativeComputerUseController.refreshFocusedTarget",
                        "controller": "BlinkNativeComputerUseController",
                        "appName": "Messages",
                        "personName": personName,
                        "instruction": instruction,
    ...
    private func typeTextUsingNativeComputerUse(_ request: BlinkNativeTypeRequest) {
        interruptCurrentVoiceResponse()
        let executionStartedAt = markRequestExecutionStarted(
            route: "native_cua.type_text",
            extra: [
                "executor": "native_cua",
                "executionMethod": "BlinkNativeComputerUseController.typeText",
                "controller": "BlinkNativeComputerUseController",
                "textLength": request.text.count
            ]
        )
    ...
    private func pressKeyUsingNativeComputerUse(_ request: BlinkNativeKeyPressRequest, shouldSpeak: Bool = true) {
        if shouldSpeak {
            interruptCurrentVoiceResponse()
        }
        let executionStartedAt = markRequestExecutionStarted(
            ...
            extra: [
                "executor": "native_cua",
                "executionMethod": "BlinkNativeComputerUseController.pressKey",
                "controller": "BlinkNativeComputerUseController",
                "key": request.key,
                "modifiers": request.modifiers.joined(separator: ","),
                "shouldSpeak": shouldSpeak
    ...
    private func runOpenApplication(arguments: [String]) -> Bool {
        ...
        do {
            ...
            print("Blink app open failed for arguments \(arguments): \(error)")
            return false
        }
    }
    ...
    private func submitTextModePrompt(_ submittedText: String) {
        ...
        BlinkAnalytics.trackUserMessageSent(transcript: trimmedText)
        interruptCurrentVoiceResponse()
        clearDetectedElementLocation()
    ...
    private func submitPendingAgentVoiceFollowUp(_ transcript: String) -> Bool {
        ...
        if let createdAt = pendingAgentVoiceFollowUpCreatedAt,
           Date().timeIntervalSince(createdAt) > Self.pendingAgentVoiceFollowUpTTL {
            ...
            BlinkMessageLogStore.shared.append(
                lane: "agent",
                direction: "internal",
                event: "blink.agent_followup.voice_target_expired",
                fields: [
        ...
        if Self.isProbablyIncompleteAgentVoiceFollowUp(trimmedTranscript) {
            ...
            BlinkMessageLogStore.shared.append(
                lane: "agent",
                direction: "outgoing",
                event: "blink.agent_followup.deferred_incomplete_voice",
                fields: [
                    "source": pendingSource,
                    "sessionID": sessionID.uuidString,
        ...
        // to that agent even if it sounds like a normal local Blink
        // command such as “open Blink.” Keep this ahead of new-task, direct
    ...
    private func submitContextualAgentFollowUp(_ transcript: String, source: String) -> Bool {
        ...
        BlinkMessageLogStore.shared.append(
            lane: "agent",
            direction: "incoming",
            event: "blink.agent_followup.steered",
            fields: [
                "sessionID": session.id.uuidString,
                "title": session.title,
                "source": source,
    ...
    private func handleAgentSelectionRequestIfNeeded(from transcript: String, source: String) -> Bool {
        ...
        guard let session = agentSession(matchingSpokenName: request.agentName) else {
            BlinkMessageLogStore.shared.append(
                lane: "agent",
                direction: "error",
                event: "blink.agent_select.not_found",
                fields: [
                    "source": source,
                    "agentName": request.agentName,
                    "instruction": request.instruction
        ...
        BlinkMessageLogStore.shared.append(
            lane: "agent",
            direction: "incoming",
            event: "blink.agent_select.selected",
    ...
    private static func partialIsEligibleForSpeculativeFire(_ partialTranscript: String) -> Bool {
        ...
        if blinkAgentInstruction(from: partialTranscript) != nil { return false }
    ...
    private func fireSpeculativePreFire(forPartial partialTranscript: String) {
        ...
        let task = Task<String, Error> { [weak self] in
            ...
            let (focusedBundleID, focusedName) = await MainActor.run { () -> (String?, String?) in
                let app = NSWorkspace.shared.frontmostApplication
                return (app?.bundleIdentifier, app?.localizedName)
            }
            let ragContext = await BlinkAppKnowledgeStore.promptContextAsync(
                for: speculativeFireForCapture,
    ...
    private func commitSpeculativeFire(_ fire: SpeculativeFire, transcript: String) {
        ...
        currentResponseTask = Task {
            ...
            do {
                ...
                BlinkAnalytics.trackAIResponseReceived(response: spokenText)
                self.latestVoiceResponseCard = BlinkResponseCard(
                    source: .voice,
                    rawText: spokenText,
                ...
                if !spokenText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                    do {
                        ...
                        BlinkAnalytics.trackTTSError(error: error.localizedDescription)
                        speakResponseFailureFallback(error)
                    }
                } else {
                    streamingTTSSession.cancel()
    ...
    private func cancelAllAgentTasks(reason: String = "agent.cancel_all") {
        ...
        BlinkMessageLogStore.shared.append(
            lane: "agent",
            direction: "incoming",
            event: "blink.agent_tasks.cancelled_all",
            fields: [
    ...
    private func cancelAgentTask(sessionID: UUID, removeDockItems: Bool, reason: String = "agent.cancel") {
        ...
        BlinkMessageLogStore.shared.append(
            lane: "agent",
            direction: "incoming",
            event: "blink.agent_task.cancelled",
            fields: [
                "sessionID": sessionID.uuidString,
    ...
    private func startExplicitAgentTaskIfRequested(from transcript: String) -> Bool {
        ...
        if let taskCreationInstruction = Self.agentTaskCreationInstruction(from: transcript) {
            ...
            print("Blink agent task creation request detected: \(taskCreationInstruction)")
        ...
        let explicitInstructionFromCliky = Self.blinkAgentInstruction(from: transcript)
        ...
        guard !explicitInstruction.isEmpty else {
            print("Blink agent trigger detected without an instruction.")
            speakShortSystemResponse("what should the agent do?")
            return true
        }
        ...
        if Self.isReferentialAgentInstruction(instruction) {
            guard let resolvedInstruction = referentialAgentInstructionContext(excluding: transcript) else {
                BlinkMessageLogStore.shared.append(
                    lane: "agent",
                    direction: "incoming",
                    event: "blink.agent_task.referential_instruction_unresolved",
                    fields: [
                        "transcript": transcript,
            ...
            BlinkMessageLogStore.shared.append(
                lane: "agent",
                direction: "incoming",
                event: "blink.agent_task.referential_instruction_resolved",
                fields: [
                    "transcript": transcript,
                    "explicitInstruction": explicitInstruction,
        ...
        print("Blink agent task detected; starting agent task: \(instruction)")
        startVoiceAgentTask(instruction: instruction)
        return true
    }
    ...
    private static func blinkAgentInstruction(from transcript: String) -> String? {
        ...
        for tokenIndex in tokens.indices {
            ...
            // Apple Speech often hears "Blink agent" as "click the agent".
            ...
            guard isBlinkInvocationToken(tokens[scanningIndex].normalizedText) else { continue }
    ...
    private static func isBlinkInvocationToken(_ normalizedText: String) -> Bool {
        switch normalizedText {
        case "blink", "klicky", "cookie", "quick":
            return true
        default:
            return false
        }
    ...
    static func agentTaskCreationInstruction(from transcript: String) -> String? {
        ...
        let patterns = [
            #"(?i)^\s*(?:(?:blink|blink)\s+)?(?:(?:can|could|would|will)\s+you\s+)?(?:please\s+)?(?:create|start|spin\s+up|spawn|run|launch|kick\s+off|set\s+up)\s+(?:an?\s+|the\s+)?(?:new\s+)?(?:background\s+)?(?:agent|agenty|codex)\s*(?:task|job|session)?\s+(?:to|for|that|which|who)?\s*(.+?)\s*$"#,
            #"(?i)^\s*(?:(?:blink|blink)\s+)?(?:(?:can|could|would|will)\s+you\s+)?(?:please\s+)?set\s+(?:an?\s+|the\s+)?(?:new\s+)?(?:background\s+)?(?:agent|agenty|codex)\s*(?:task|job|session)?\s+(?:off|going)\s+(?:to|for|that|which|who)?\s*(.+?)\s*$"#,
            ...
            #"(?i)^\s*(?:(?:blink|blink)\s+)?(?:(?:can|could|would|will)\s+you\s+)?(?:please\s+)?(?:ask|tell|have|get)\s+(?:an?\s+|the\s+)?(?:agent|agenty|codex)\s+to\s+(.+?)\s*$"#,
    ...
    private static func isIncompleteAgentTaskCreationRequest(from transcript: String) -> Bool {
        ...
        let patterns = [
            #"(?i)^\s*(?:(?:blink|blink)\s+)?(?:(?:can|could|would|will)\s+you\s+)?(?:please\s+)?(?:create|start|spin\s+up|spawn|run|launch|kick\s+off|set\s+up)\s+(?:an?\s+|the\s+)?(?:new\s+)?(?:background\s+)?(?:agent|codex)\s*(?:task|job|session)?[\s\.\!\?]*$"#,
            #"(?i)^\s*(?:(?:blink|blink)\s+)?(?:(?:can|could|would|will)\s+you\s+)?(?:please\s+)?set\s+(?:an?\s+|the\s+)?(?:new\s+)?(?:background\s+)?(?:agent|codex)\s*(?:task|job|session)?\s+(?:off|going)[\s\.\!\?]*$"#,
            ...
            #"(?i)^\s*(?:(?:blink|blink)\s+)?(?:(?:can|could|would|will)\s+you\s+)?(?:please\s+)?(?:ask|tell|have|get)\s+(?:an?\s+|the\s+)?(?:agent|codex)(?:\s+to)?[\s\.\!\?]*$"#
        ]
    ...
    private static func agentSelectionRequest(from transcript: String) -> BlinkAgentSelectionRequest? {
        let candidate = normalizedCommandCandidate(from: transcript)
        guard !candidate.isEmpty else { return nil }
        ...
        for pattern in typedFollowUpPatterns {
            ...
            return BlinkAgentSelectionRequest(
                agentName: agentName,
    ...
    /// Returns true when the transcript references a URL or bare domain
    /// (e.g. "open github.com", "navigate to https://anthropic.com"). Used
    /// to pre-empt the app-open path so URL phrasing isn't misrouted into
    /// localAppOpenRequest.
    private static func transcriptLooksLikeURL(_ transcript: String) -> Bool {
        // Apply the same speech-to-text artifact pass webOpenRequest uses
        // so spoken "github dot com" or "github . com" trigger the URL
        // preempt instead of falling through to app open.
        let normalized = normalizedSpeechURLArtifacts(transcript)
        let lowered = normalized.lowercased()
        if lowered.contains("http://") || lowered.contains("https://") || lowered.contains("www.") {
            return true
        }
        // Match bare domain like github.com, sub.example.io, etc.
        return lowered.range(
            of: #"\b[a-z0-9-]+\.(?:com|org|net|io|dev|app|ai|co|so|me|gg|tv|xyz|sh|edu|gov|page|site|blog|store|tech|cloud|dev|wiki|news|tools|run)\b"#,
            options: .regularExpression
        ) != nil
    }

    /// Common-sense brand → canonical URL lookup. When the user says "open
    /// github" the speech recogniser hands us "github" with no TLD; this
    /// table closes the gap without going through the LLM. Entries are
    /// limited to web-first services so commands like "open spotify" or
    /// "open slack" still land on the native Mac apps. Keys are matched
    /// case-insensitively after stripping leading filler ("my", "the").
    private static let knownWebBrands: [String: String] = [
        // Search / AI
        "google": "google.com",
    ...
    /// Looks up `candidate` (case-insensitive, with leading "my"/"the"/etc
    /// filler stripped) in `knownWebBrands`. Returns the canonical bare
    /// host like `"github.com"` or `nil` when there's no match.
    private static func knownBrandHost(for candidate: String) -> String? {
        let lowered = candidate
            .folding(options: [.caseInsensitive, .diacriticInsensitive], locale: .current)
            .lowercased()
            .trimmingCharacters(in: CharacterSet(charactersIn: " \n\t.,!?:;"))
        guard !lowered.isEmpty else { return nil }
        if let host = knownWebBrands[lowered] { return host }
        let stripped = lowered.replacingOccurrences(
            of: #"^(?:my|the|a|an|some)\s+"#,
            with: "",
            options: .regularExpression
        )
        return knownWebBrands[stripped]
    }

    /// Collapses speech-to-text artifacts so spoken URLs match the existing
    /// regex/brand pipeline. Handles:
    ///   "github dot com"      → "github.com"
    ///   "github . com"        → "github.com"
    ///   "github dot c o m"    → "github.com"   (rare; spelled out)
    ///   "get hub"             → "github"       (common Apple Speech mishearing)
    ///   "you tube"            → "youtube"
    ///   "chat g p t"          → "chatgpt"
    /// Applied inside webOpenRequest only — keeps the app-open path
    /// untouched so "open Hub" / "open Tube" still resolve to apps.
    private static func normalizedSpeechURLArtifacts(_ input: String) -> String {
        var s = input
        // " dot " variants (with optional surrounding punctuation)
        s = s.replacingOccurrences(of: #"(?i)\s*\bdot\s+"#, with: ".", options: .regularExpression)
        s = s.replacingOccurrences(of: #"(?i)\s+dot\s*$"#, with: ".", options: .regularExpression)
        // Spaces around literal dots
        s = s.replacingOccurrences(of: #"\s+\.\s*"#, with: ".", options: .regularExpression)
        s = s.replacingOccurrences(of: #"\.\s+"#, with: ".", options: .regularExpression)
        // Common mishearings / split brand names
        let aliasMap: [(String, String)] = [
            (#"(?i)\bget\s+hub\b"#, "github"),
            (#"(?i)\bgit\s+hub\b"#, "github"),
            (#"(?i)\byou\s+tube\b"#, "youtube"),
            (#"(?i)\btik\s+tok\b"#, "tiktok"),
            (#"(?i)\bsnap\s+chat\b"#, "snapchat"),
            (#"(?i)\blinked\s+in\b"#, "linkedin"),
            (#"(?i)\bpin\s+terest\b"#, "pinterest"),
            (#"(?i)\bface\s+book\b"#, "facebook"),
            (#"(?i)\bwhats?\s+app\b"#, "whatsapp"),
            (#"(?i)\bduck\s+duck\s+go\b"#, "duckduckgo"),
            (#"(?i)\bopen\s+a\s*i\b(?!\.)"#, "openai"),
            (#"(?i)\bchat\s+g\s*p\s*t\b"#, "chatgpt"),
            (#"(?i)\bg\s*p\s*t\b(?=\s|$)"#, "gpt"),
            (#"(?i)\bnew\s+york\s+times\b"#, "new york times"),
            (#"(?i)\bhacker\s+news\b"#, "hacker news"),
            (#"(?i)\bstack\s+overflow\b"#, "stack overflow")
        ]
        for (pattern, replacement) in aliasMap {
            s = s.replacingOccurrences(of: pattern, with: replacement, options: .regularExpression)
        }
        return s
    }

    private static func webOpenRequest(from transcript: String) -> BlinkWebOpenRequest? {
        let speechNormalized = normalizedSpeechURLArtifacts(transcript)
        let trimmedTranscript = normalizedCommandCandidate(from: speechNormalized)
    ...
    private static func normalizedWebOpenURL(from rawTarget: String) -> URL? {
        ...
        // Common-sense fallback: speech-recognised bare brand names like
        // "github", "hacker news", or "my gmail" resolve through the
        // hand-curated table.
        if let host = knownBrandHost(for: trimmed) {
    ...
    private static func localAppOpenRequest(from transcript: String) -> BlinkAppOpenRequest? {
    ...
    private static func bareLocalAppOpenRequest(from transcript: String) -> BlinkAppOpenRequest? {
        let candidate = normalizedCommandCandidate(from: transcript)
        return bareLocalAppOpenRequest(fromNormalizedCandidate: candidate)
    }

    private static func bareLocalAppOpenRequest(fromNormalizedCandidate candidate: String) -> BlinkAppOpenRequest? {
    ...
    private static func reminderAddRequest(from transcript: String) -> BlinkReminderAddRequest? {
        let candidate = normalizedCommandCandidate(from: transcript)
        guard !candidate.isEmpty else { return nil }
        ...
        for pattern in titlePatterns {
            ...
            return BlinkReminderAddRequest(title: title, instruction: candidate)
        }

        return nil
    }

    private static func reminderCountRequest(from transcript: String) -> BlinkReminderCountRequest? {
        let candidate = normalizedCommandCandidate(from: transcript)
        guard !candidate.isEmpty else { return nil }
        ...
        return BlinkReminderCountRequest(instruction: candidate)
    }

    private static func messagesSearchRequest(from transcript: String) -> BlinkMessagesSearchRequest? {
        let candidate = normalizedCommandCandidate(from: transcript)
        guard !candidate.isEmpty else { return nil }
        ...
        for pattern in patterns {
            ...
            return BlinkMessagesSearchRequest(personName: personName, instruction: candidate)
        }

        return nil
    }

    private static func localFolderOpenRequest(from transcript: String) -> BlinkFolderOpenRequest? {
        let trimmedTranscript = normalizedCommandCandidate(from: transcript)
        guard !trimmedTranscript.isEmpty else { return nil }
        ...
        if sourceFolderTerms.contains(where: { normalizedTranscript.contains($0) }),
           let sourceURL = existingBlinkSourceDirectoryURL() {
            return BlinkFolderOpenRequest(
                url: sourceURL,
        ...
        if let rememberedShortcut = BlinkDirectActionMemoryStore.shared.folderShortcut(matching: normalizedTranscript) {
            return BlinkFolderOpenRequest(
    ...
    private static func existingBlinkSourceDirectoryURL(fileManager: FileManager = .default) -> URL? {
        let home = fileManager.homeDirectoryForCurrentUser.path
        let candidates = [
            "/Users/blink/Documents/GitHub/blink",
            "\(home)/Documents/GitHub/blink"
        ]
    ...
    private static func isExplicitAgentRoutingCandidate(_ transcript: String) -> Bool {
        explicitNewTaskInstruction(from: transcript) != nil
            ...
            || blinkAgentInstruction(from: transcript) != nil
    ...
    private static func deferredLiveAgentRouteInstruction(
        ...
        // "Blink agent ..." being heard live as "click the agent ..." while
    ...
    private static func explicitAgentRouteInstruction(from transcript: String) -> String? {
        ...
        if let instruction = blinkAgentInstruction(from: transcript) { return instruction }
    ...
    private static func nativeAutomationErrorMessage(
        appName: String,
        result: BlinkLocalAutomationResult
    ) -> String {
        ...
        if normalizedDetail.contains("not authorized")
            ...
            return "macOS blocked \(appName) automation. enable Blink for \(appName) in System Settings."
        }
    ...
    private static func isLikelyWebOpenTarget(_ value: String) -> Bool {
        ...
        // Bare brand match — defer to the web open path so localAppOpenRequest
        // doesn't try to launch a Mac app for a web-only service.
        if knownBrandHost(for: raw) != nil {
            return true
        }
    ...
    private static func nativeTypeRequest(from transcript: String) -> BlinkNativeTypeRequest? {
        let candidate = normalizedCommandCandidate(from: transcript)
        guard !candidate.isEmpty else { return nil }
    ...
    private static func nativeKeyPressRequest(from transcript: String) -> BlinkNativeKeyPressRequest? {
        let candidate = normalizedCommandCandidate(from: transcript)
        guard !candidate.isEmpty else { return nil }
        ...
        for pattern in patterns {
            ...
            return BlinkNativeKeyPressRequest(
                key: parsed.key,
    ...
    private static func legacyBlinkAgentInstruction(from transcript: String) -> String? {
        let triggerPattern = #"\b(?:hey[\s,]+)?(?:open[\s,.-]*)?blink[\s,.-]+agent\b"#
    ...
    private func startVoiceAgentTask(instruction: String, acknowledgement: String? = nil) {
        ...
        // Spawn the dock representation while the live Blink buddy makes
        ...
        Task { @MainActor in
            ...
            let dockItem = BlinkAgentDockItem(
                id: dockItemID,
            ...
            BlinkMessageLogStore.shared.append(
                lane: "agent",
                direction: "outgoing",
                event: "blink.agent_task.created",
                fields: [
                    "executor": "agent_mode",
                    "executionMethod": "CodexAgentSession.submitPromptFromUI",
                    "controller": "CodexAgentSession",
    ...
    private func directComputerUseAgentBoundaryCueText() -> String {
        ...
        case .nativeSwift:
            return "routing that through Blink's native CUA path."
        }
    }
    ...
    private static func shortAgentInstructionSummary(_ instruction: String) -> String {
        ...
        let directTitleRules: [(pattern: String, title: String)] = [
            (#"(?i)\b(?:see\s+(?:the\s+)?issue\s+here|look\s+at\s+this|fix\s+this)\b.*\b(?:BlinkLog|NSXPCDecoder|ViewBridge|unifiedReasons|NSXPCConnection)\b"#, "Log Issue Review"),
    ...
    /// Blink confirms the job without reading the raw dictated request
    /// back to the user.
    ...
    private static func nextAgentDockAccentTheme(existingCount: Int) -> BlinkAccentTheme {
        // Always match the user's cursor accent so the dock, the agent
        // card, and the cursor share one identity instead of rotating
        // through a palette.
        _ = existingCount
        if let raw = UserDefaults.standard.string(forKey: BlinkAccentTheme.userDefaultsKey),
           let theme = BlinkAccentTheme(rawValue: raw) {
            return theme
        }
        return .blue
    }
    ...
    private func updateAgentDockItem(for sessionID: UUID, status: CodexAgentSessionStatus) {
        ...
        case .ready:
            ...
            if agentDockItems[itemIndex].status == .running
                ...
                scheduleAutoDismissOfCompletedAgent(sessionID: sessionID)
            }
    ...
    private static func agentDockResponseDisplaySummary(for session: CodexAgentSession?) -> String? {
        ...
        let displayText = BlinkResponseCard.sanitizedDisplayText(from: raw, maximumCharacters: 1_200)
    ...
    private static func cursorAgentTaskLabel(for item: BlinkAgentDockItem, session: CodexAgentSession?) -> String {
    ...
    private func completeAgentRequestTimingIfNeeded(
        ...
        // grow without bound across long-running Blink sessions.
    ...
    private func announceAgentCompletionIfNeeded(
        ...
        switch outcome {
        case "cancelled": notificationTitle = "Blink task cancelled"
        case "failed": notificationTitle = "Blink task stopped"
        default: notificationTitle = "Blink task done"
        }
        BlinkDesktopNotificationCenter.shared.post(
            title: notificationTitle,
            body: line,
            threadID: "blink.agent.\(sessionID.uuidString)",
    ...
    private func silenceAgentSpeech(for sessionID: UUID, reason: String) {
        ...
        BlinkMessageLogStore.shared.append(
            lane: "agent",
            direction: "incoming",
            event: "blink.agent_task.speech_silenced",
            fields: [
                "sessionID": sessionID.uuidString,
                "reason": reason
            ]
        ...
        while systemAnnouncementAudioWouldCollideWithVoiceInput {
            ...
            if Date().timeIntervalSince(start) >= maxWaitSeconds {
                BlinkMessageLogStore.shared.append(
                    lane: "agent",
                    direction: "internal",
                    event: "blink.agent_completion.speech_deferred_timeout",
                    fields: [
                        "sessionID": sessionID?.uuidString ?? "",
    ...
    /// Short percussive click used to acknowledge the user's final
    /// transcript — fires the moment Blink commits to a request.
    private var cachedEnterChime: NSSound?
    func playUserInputChime() {
        if cachedEnterChime == nil,
           let url = Bundle.main.url(forResource: "enter", withExtension: "mp3"),
           let sound = NSSound(contentsOf: url, byReference: false) {
            sound.volume = 0.65
            cachedEnterChime = sound
        }
        cachedEnterChime?.stop()
        cachedEnterChime?.play()
    }

    /// Notification chime used when a response surfaces in the rich
    /// panel — Nintendo-eShop style bell so the user knows to look.
    private var cachedEshopChime: NSSound?
    func playResponseReadyChime() {
        if cachedEshopChime == nil,
           let url = Bundle.main.url(forResource: "eshop", withExtension: "mp3"),
           let sound = NSSound(contentsOf: url, byReference: false) {
            sound.volume = 0.55
            cachedEshopChime = sound
        }
        cachedEshopChime?.stop()
        cachedEshopChime?.play()
    }
    ...
    private static func completionSpeechSummary(
        ...
        if let raw = session?.latestResponseCard?.rawText {
            var text = BlinkResponseCard.sanitizedDisplayText(from: raw, maximumCharacters: 10_000)
            text = text.replacingOccurrences(
                of: #"(?im)^\s*TASK_TITLE\s*:\s*.*$"#,
                with: " ",
                options: .regularExpression
            ...
            if !text.isEmpty {
                ...
                BlinkMessageLogStore.shared.append(
                    lane: "agent",
                    direction: "outgoing",
                    event: "blink.agent_completion.speech_budget",
                    fields: [
                        "rawLength": raw.count,
    ...
    private func armVoiceFollowUpTarget(_ sessionID: UUID, source: String) {
        ...
        BlinkMessageLogStore.shared.append(
            lane: "agent",
            direction: "internal",
            event: "blink.agent_followup.voice_target_armed",
            fields: [
                "source": source,
                "sessionID": sessionID.uuidString,
    ...
    private func submitTextFollowUp(_ submittedText: String, toAgentSessionID sessionID: UUID) {
        ...
        guard let session = codexAgentSessions.first(where: { $0.id == sessionID }) else {
            BlinkMessageLogStore.shared.append(
                lane: "agent",
                direction: "error",
                event: "blink.agent_followup.missing_session",
                fields: [
    ...
    func attachDroppedAgentFiles(_ urls: [URL], toAgentDockItem itemID: UUID, source: String) {
        ...
        guard let sessionID = agentDockItems.first(where: { $0.id == itemID })?.sessionID,
            ...
            BlinkMessageLogStore.shared.append(
                lane: "agent",
                direction: "error",
                event: "blink.agent_attachment_drop.missing_session",
                fields: [
                    "source": source,
        ...
        BlinkMessageLogStore.shared.append(
            lane: "agent",
            direction: "incoming",
            event: "blink.agent_attachment_drop.received",
            fields: [
                "source": source,
                "sessionID": session.id.uuidString,
                "attachmentCount": standardizedURLs.count
    ...
    func submitAgentPromptFromUI(_ prompt: String) {
        ...
        if handleDirectComputerUseRequest(from: trimmedPrompt, source: "agent_hud_prompt") {
            BlinkMessageLogStore.shared.append(
                lane: "agent",
                direction: "incoming",
                event: "blink.agent_prompt.intercepted_native_cua",
                fields: [
    ...
    private func stageDashboardAgentSubmission(prompt: String, session: CodexAgentSession) {
        ...
        let spawnAccentTheme: BlinkAccentTheme
    ...
    private func submitAgentPrompt(_ prompt: String, to session: CodexAgentSession, includeScreenContext: Bool = true) {
        ...
        BlinkApplicationUsageLogStore.shared.recordFrontmostApplication(source: "agent_prompt")
        Task {
            ...
            if !includeScreenContext {
                BlinkMessageLogStore.shared.append(
                    lane: "agent",
                    direction: "internal",
                    event: "blink.agent_screen_context.skipped",
                    fields: [
    ...
    private func prepareAgentScreenContextForNextTurn(minimumPasteboardChangeCount: Int) async -> CodexAgentScreenContext? {
        if !handoffQueue.isEmpty {
            ...
            do {
                ...
                print("Blink Agent Mode: failed to write queued screen context: \(error)")
            }
        }
        ...
        do {
            if selectedComputerUseBackend == .backgroundComputerUse {
                ...
                if backgroundStatus.isRuntimeReady {
                    do {
                        ...
                        print("Blink Agent Mode: Background Computer Use context unavailable: \(error)")
                    }
                } else {
                    BlinkMessageLogStore.shared.append(
                        lane: "computer-use",
                ...
                do {
                    ...
                    print("Blink Agent Mode: native CUA Swift focused-window context unavailable: \(error)")
                }
            }
            ...
            print("Blink Agent Mode: current screen context unavailable: \(error)")
            return nil
        }
    }
    ...
    private func writeNativeComputerUseScreenContext(_ capture: BlinkComputerUseWindowCapture, minimumPasteboardChangeCount: Int) throws -> CodexAgentScreenContext {
    ...
    private func writeBackgroundComputerUseScreenContext(_ capture: BlinkBackgroundComputerUseWindowCapture, minimumPasteboardChangeCount: Int) throws -> CodexAgentScreenContext {
        BlinkApplicationUsageLogStore.shared.recordApplication(
    ...
    func testVoiceResponseCaptionPlayback() {
        let line = "This is Blink's caption playback test. The selected caption font should show beside the cursor."
        latestVoiceResponseCard = BlinkResponseCard(
            source: .voice,
            rawText: line,
    ...
    you're blink, a quiet menu-bar companion. the user just gave you a command via push-to-talk and you can see their screen(s). your reply will be spoken aloud, so write the way you'd actually talk. this is an ongoing conversation — you remember everything they've said before.

    SPEAK ONLY WHEN GIVEN A COMMAND. you only respond to the turn the user just initiated. you do not greet, you do not idle-comment, you do not observe out loud, you do not check in. silence is the default. one turn in, one answer out.

    ANSWER THE QUESTION AND STOP. if the user asks a basic question, answer that exact question and nothing more. do not add suggestions. do not plant seeds. do not bring up related topics. do not ask follow-up questions. do not offer next steps. do not say "you could also". the user did not ask for ideas; they asked one thing.

    YOU CAN:
    1. POINT at a thing on screen using the [POINT:...] tag, but only when the user's command explicitly involves a visible target ("what's this", "where's that", "show me", "point at", "what does that button do").
    2. ANSWER QUESTIONS conversationally — definitions, explanations, conceptual coding questions, walkthroughs.
    3. ANSWER FROM YOUR OWN KNOWLEDGE. if the user explicitly wants live/current data, say briefly that you can't fetch live data here.

    YOU DO NOT, EVER:
    - run code, commands, shell, terminal, python, scripts
    - read, write, edit, create, move, delete, rename, organize, or inspect files
    - modify settings, config, memory, skills, logs, or any Blink state
    - perform any filesystem, git, build, install, or refactor work
    - speak first, speak when not asked, or continue after the answer is complete

    if the user asks for something in the "DO NOT" list, say briefly: "i can't do that from voice." nothing more.

    response style:
    - one or two sentences. shorter is better. only go longer if the user explicitly asked for a thorough explanation.
    - all lowercase, casual. no emojis.
    - write for the ear. short sentences. no lists, bullets, markdown, headings, tables, or code blocks.
    - spell out small numbers, write "for example" not "e.g."
    ...
    - never end with a question, suggestion, or "want me to". end on the answer.

    element pointing:
    you have a small triangle cursor that can fly to a target on screen. only point when the user's command explicitly involves a visible target. do not point speculatively. do not point to be helpful. if the command is a definition, a general question, a concept, or anything not tied to a specific visible element, append [POINT:none].

    when you do point, append a coordinate tag at the very end, AFTER your spoken text. screenshot images are labeled with their pixel dimensions. use those dimensions as the coordinate space. origin (0,0) is top-left. x increases rightward, y increases downward.

    format: [POINT:x,y:label] where x,y are integer pixel coordinates and label is a short 1-3 word description. if the element is on a different screen than the cursor, append :screenN.

    if pointing wouldn't help, append [POINT:none].

    examples:
    - user explicitly asks "what does that button do": "that opens the color inspector. [POINT:1100,42:color inspector]"
    - user asks "what is html": "html stands for hypertext markup language, it's the skeleton of a web page. [POINT:none]"
    - user asks "how do i commit in xcode": "open the source control menu and hit commit, or use command option c. [POINT:285,11:source control]"
    """

    private func runtimeStorageContextForVoicePrompt() -> String {
        let logs = BlinkMessageLogStore.shared
        return """
        Blink runtime storage:
        ...
        - widget snapshot: \(BlinkWidgetStateStore.snapshotURL.path)
        """
    }
    ...
    you're blink in tutor mode. the user gave you a command and wants help with the app or workflow on screen. you can see their focused window.

    your job:
    - answer ONLY the command the user just gave you. do not narrate. do not observe. do not proactively guide. do not check in when they pause. silence is the default.
    - if their command names a step, give that step. point at the relevant button, menu, field, or area if it's visible.
    - if their command is a question, answer it. nothing more.
    - never suggest a next step they didn't ask for. never say "now you should" or "next, try". the user will tell you when they want the next step.
    - teach a concept only if the user explicitly asked for the concept.

    style:
    - short spoken response, lowercase, casual, no markdown, no emojis.
    - one or two sentences. stop on the answer.
    - do not claim you clicked or controlled anything. you can only describe and point.
    ...
    private func sendTranscriptToClaudeWithScreenshot(transcript: String) {
        ...
        let completionState = BlinkRequestCompletionState()
        ...
        currentResponseTask = Task {
            ...
            do {
                ...
                let ragContext = await BlinkAppKnowledgeStore.promptContextAsync(
                    for: transcript,
                    focusedAppBundleID: focusedApp?.bundleIdentifier,
                ...
                // carry Blink's screenshot payload into the response model,
                ...
                let shouldUseFiller = Self.shouldUsePreResponseFiller(
                    transcript: transcript,
                    screenContextNeeded: shouldAttachScreenContext,
                    modelProvider: BlinkModelCatalog.voiceResponseModel(withID: visualAnalysisModelID).provider
                )
                ...
                if let pointCoordinate = parseResult.coordinate,
                   let targetScreenCapture {
                    ...
                    BlinkAnalytics.trackElementPointed(elementLabel: parseResult.elementLabel)
                ...
                BlinkAnalytics.trackResponseError(error: error.localizedDescription)
    ...
    private func bindTutorIdleObservation() {
        // Blink only speaks in response to a user command. Idle-driven tutor
        // observation would speak without a command, so the binding is disabled.
        tutorIdleCancellable?.cancel()
        tutorIdleCancellable = nil
    }
    ...
    private func analyzeVoiceResponse(
        ...
        let selectedVoiceResponseModel = BlinkModelCatalog.voiceResponseModel(withID: modelID ?? selectedModel)
        applyVoiceResponseModelSettings(selectedVoiceResponseModel)
    ...
    private func analyzeClaudeResponse(
        ...
        let modelOption = BlinkModelCatalog.voiceResponseModel(withID: model)
    ...
    private static func shouldUsePreResponseFiller(
        ...
        modelProvider: BlinkModelProvider
    ) -> Bool {
        ...
        // sounds like Blink is inventing work: "one moment. sounds good."
    ...
    private func analyzeComputerUsePointingResponse(
        ...
        let selectedPointingModel = BlinkModelCatalog.computerUseModel(withID: selectedComputerUseModel)
    ...
    private func attemptProactiveElementPointingIfUseful(
        ...
        detectedElementDisplayFrame = displayFrame
        detectedElementScreenLocation = globalLocation
        BlinkAnalytics.trackElementPointed(elementLabel: "proactive")
    ...
    /// If the cursor is in transient mode (user toggled "Show Blink" off),
    /// waits for TTS playback and any pointing animation to finish, then
    /// fades out the overlay after a 1-second pause. Cancelled automatically
    /// if the user starts another push-to-talk interaction.
    private func scheduleTransientHideIfNeeded() {
        guard !isBlinkCursorEnabled && isOverlayVisible else { return }
        // Don't hide while the user is still holding the PTT shortcut. If
        // dictation startup briefly flips to idle (permission check, mic
        // contention, async retry), the overlay would otherwise vanish
        // about a second into the user's spoken request.
        guard !globalPushToTalkShortcutMonitor.isShortcutCurrentlyPressed else { return }
        ...
        transientHideTask = Task {
            ...
            guard !Task.isCancelled else { return }
            // Re-check the PTT state right before fading — the user may have
            // started a new press during the sleep, in which case we leave
            // the overlay up so they don't see a flash of disappearance.
            guard !globalPushToTalkShortcutMonitor.isShortcutCurrentlyPressed else { return }
    ...
    func showMemoryWindow() {
        wikiViewerPanelManager.show(
            ...
            onCreateMemory: { [weak self] title, body in
                guard let self else {
                    throw NSError(domain: "Blink.Memory", code: 3, userInfo: [
                        NSLocalizedDescriptionKey: "Blink couldn't reach the memory manager."
                    ])
                }
    ...
    func runSuggestedNextAction(_ actionTitle: String, forAgentDockItem itemID: UUID) {
        guard let item = agentDockItems.first(where: { $0.id == itemID }),
            ...
            BlinkMessageLogStore.shared.append(
                lane: "agent",
                direction: "error",
                event: "blink.agent_suggested_action.missing_session",
                fields: [
                    "itemID": itemID.uuidString,
    ...
    func debugShowResponseCard() {
        latestVoiceResponseCard = BlinkResponseCard(
            source: .voice,
            rawText: "This is a developer smoke test for Blink's compact response card. Suggested actions and dismiss behavior should remain usable from the panel and chat.",
            contextTitle: "Developer"
        )
    }
...
private final class BlinkTextModePanel: NSPanel {
...
final class BlinkTextModeWindowManager {
    ...
    func show(at cursorLocation: CGPoint, accentTheme: BlinkAccentTheme? = nil, submitText: @escaping (String) -> Void) {
    ...
    func show(origin: CGPoint, accentTheme: BlinkAccentTheme? = nil, submitText: @escaping (String) -> Void) {
        preparePanel(accentTheme: accentTheme, submitText: submitText)
    ...
    private func createPanel(accentTheme: BlinkAccentTheme?, submitText: @escaping (String) -> Void) {
        let textModePanel = BlinkTextModePanel(
...
private struct BlinkTextModeInputView: View {
    ...
    let accentThemeOverride: BlinkAccentTheme?
        ...
        VStack(alignment: .leading, spacing: droppedAttachments.isEmpty ? 6 : 7) {
            ...
            HStack(spacing: 9) {
                ...
                ZStack(alignment: .leading) {
                    if text.isEmpty {
                        Text(droppedAttachments.isEmpty ? placeholderText : "Ask Blink about the attachment…")
    ...
    private func promptWithAttachments(_ prompt: String) -> String {
        ...
        Blink instant chat attachments:
        \(attachmentLines)
        """.trimmingCharacters(in: .whitespacesAndNewlines)
    }
    ...
    private static func persistDroppedImage(_ data: Data) -> URL? {
        let directory = FileManager.default
            ...
            .appendingPathComponent("Library/Application Support/Blink/AgentMode/DroppedAttachments", isDirectory: true)
        do {
            try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    ...
    private var activeSuggestions: [BlinkTextModeSuggestion] {
        guard let trigger = currentTrigger else { return [] }
        let candidates = BlinkTextModeSuggestion.candidates(for: trigger.kind)
    ...
    private var currentTrigger: (kind: BlinkTextModeSuggestion.Kind, query: String, range: Range<String.Index>)? {
        let cursorText = text
        ...
        guard let first = token.first,
              let kind = BlinkTextModeSuggestion.Kind(trigger: first) else {
            return nil
        }
        return (kind, String(token.dropFirst()), tokenRange)
    }

    private func applySuggestion(_ suggestion: BlinkTextModeSuggestion) {
        guard let trigger = currentTrigger else { return }
        text.replaceSubrange(trigger.range, with: "\(suggestion.token) ")
        focusTextField()
    }

    private var accentTheme: BlinkAccentTheme {
        accentThemeOverride ?? BlinkAccentTheme(rawValue: selectedAccentThemeID) ?? .blue
    }

    private var placeholderText: String {
        accentThemeOverride == nil ? "Ask Blink or say Hey Blink Agent..." : "Ask this agent a follow-up..."
    }
    ...
    private var controlColor: Color {
        accentTheme.cursorColor.opacity(0.92)
    }
}

private struct BlinkTextModeSuggestion: Identifiable {
    enum Kind {
        case slashCommand
        case mention
    ...
    static func candidates(for kind: Kind) -> [BlinkTextModeSuggestion] {
        switch kind {
        case .slashCommand:
            return [
                BlinkTextModeSuggestion(token: "/agent", title: "start background work", systemImage: "shippingbox"),
                BlinkTextModeSuggestion(token: "/voice", title: "use shared voice context", systemImage: "waveform"),
                BlinkTextModeSuggestion(token: "/screen", title: "include screen context", systemImage: "rectangle.dashed"),
                BlinkTextModeSuggestion(token: "/workspace", title: "current workspace", systemImage: "folder"),
                BlinkTextModeSuggestion(token: "/skills", title: "available skills", systemImage: "sparkles"),
                BlinkTextModeSuggestion(token: "/tools", title: "available tools", systemImage: "wrench.and.screwdriver")
            ]
        case .mention:
            return [
                BlinkTextModeSuggestion(token: "@agent", title: "background agent", systemImage: "shippingbox"),
                BlinkTextModeSuggestion(token: "@voice", title: "main voice thread", systemImage: "waveform"),
                BlinkTextModeSuggestion(token: "@screen", title: "visible screen", systemImage: "rectangle.dashed"),
                BlinkTextModeSuggestion(token: "@workspace", title: "current workspace", systemImage: "folder"),
                BlinkTextModeSuggestion(token: "@skills", title: "skills", systemImage: "sparkles"),
                BlinkTextModeSuggestion(token: "@tools", title: "tools", systemImage: "wrench.and.screwdriver")
            ]
        }
    }
}
...
private struct BlinkDirectActionStoredMemory: Codable, Sendable {
    var folderShortcuts: [BlinkDirectActionStoredFolderShortcut]
}

private struct BlinkDirectActionStoredFolderShortcut: Codable, Sendable {
    var aliases: [String]
    var path: String
    var displayName: String
    var lastUsedAt: Date
...
private final class BlinkDirectActionMemoryStore: @unchecked Sendable {
    static let shared = BlinkDirectActionMemoryStore()
    ...
    private let writeQueue = DispatchQueue(label: "com.rishithc.blink.direct-action-memory-writes", qos: .utility)
    private var cachedMemory: BlinkDirectActionStoredMemory?
    ...
    func recordFolderShortcut(instruction: String, url: URL, displayName: String) {
        ...
        if let index = memory.folderShortcuts.firstIndex(where: { $0.path == path }) {
            ...
            memory.folderShortcuts.append(
                BlinkDirectActionStoredFolderShortcut(
                    aliases: aliases,
                    path: path,
    ...
    private func loadMemoryLocked() -> BlinkDirectActionStoredMemory {
        if let cachedMemory {
            return cachedMemory
        }

        var memory: BlinkDirectActionStoredMemory
        if let data = try? Data(contentsOf: memoryFile),
           let decoded = try? JSONDecoder().decode(BlinkDirectActionStoredMemory.self, from: data) {
            memory = decoded
        } else {
            memory = BlinkDirectActionStoredMemory(folderShortcuts: [])
        }
    ...
    private func saveMemoryLocked(_ memory: BlinkDirectActionStoredMemory) {
        cachedMemory = memory
    ...
    @discardableResult
    private func seedBuiltInShortcutsIfNeeded(_ memory: inout BlinkDirectActionStoredMemory) -> Bool {
        let sourcePath = "/Users/blink/Documents/GitHub/blink"
