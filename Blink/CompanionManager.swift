...
private struct BlinkNativeClickRequest {
    let targetDescription: String
...
nonisolated private struct BlinkLocalAutomationResult: Sendable {
    let output: String
...
nonisolated private enum BlinkLocalAutomationRunner {
...
final class CompanionManager: ObservableObject {
    ...
    private static func notchVoicePhase(for voiceState: CompanionVoiceState) -> BlinkNotchVoicePhase {
        switch voiceState {
    ...
    let notchCaptureWindowManager = BlinkNotchCaptureWindowManager()
    ...
    let markdownViewerWindowManager = BlinkMarkdownViewerWindowManager()
    ...
    @Published private(set) var bundledKnowledgeIndex = BlinkCore.WikiManager.Index.empty
    ...
    private func getOrBuildDeepgramTTSClient(reason: String) -> DeepgramTTSClient {
        ...
        BlinkMessageLogStore.shared.append(
            ...
            fields: [
                "provider": BlinkTTSProvider.deepgram.rawValue,
    ...
    func setHomeChatModeActive(_ isActive: Bool, source: String) {
        ...
        BlinkMessageLogStore.shared.append(
            lane: "voice",
            direction: "internal",
            event: "blink.home_chat.mode_changed",
            fields: [
                "source": source,
                "isActive": isActive
            ]
    ...
    func submitHomeChatPromptFromUI(_ prompt: String, source: String = "open_blink_panel_chat") {
    ...
    private func submitHomeChatPromptToAskAgent(_ prompt: String, source: String) {
        ...
        BlinkMessageLogStore.shared.append(
            lane: "agent",
            direction: "incoming",
            event: "blink.home_chat.ask_agent_prompt",
            fields: [
                "source": source,
    ...
    private func rememberVoiceExchange(userTranscript: String, assistantResponse: String, reason: String) {
        ...
        // `[BLINK_3D] prompt: "…"` markers. Matches dispatch a generation
        ...
        BlinkMessageLogStore.shared.appendConversationTurn(
            lane: "voice",
            direction: "incoming",
            role: "user",
    ...
    private static func restoredArchivedSessions(from archivedSessionIDs: Set<UUID>) -> [CodexAgentSession] {
        ChatWorkspaceArchiveStore.loadSnapshots().compactMap { snapshot in
            ...
            let accentTheme = BlinkAccentTheme(rawValue: snapshot.accentThemeRawValue) ?? .blue
    ...
    init(runtimeMode: BlinkCompanionRuntimeMode = .menuBar) {
        ...
        BlinkAutomationStore.shared.bind(companion: self)
        // Seed bundled built-in specialist agents on first launch.
        BlinkAgentStore.shared.seedBuiltinsFromBundleIfNeeded()
    }
    ...
    func openBlinkDocument(_ url: URL) {
    ...
    private func animateAgentSpawnProxyFromCursorToDock(accentTheme: BlinkAccentTheme, caption: String? = nil, dockItemID: UUID? = nil) {
    ...
    private func loadBundledKnowledgeIndex() {
        ...
        Task.detached(priority: .utility) {
            let bundledIndex = BlinkCore.WikiManager.Index.loadForAppBundle()
            let resolvedIndex: BlinkCore.WikiManager.Index
            ...
            do {
                ...
                let memoryIndex = try BlinkCore.WikiManager.Index.load(articleRoots: [memoriesDirectory], skillRoots: [learnedSkillsDirectory])
    ...
    private func bindShortcutTransitions() {
        ...
        shiftDoubleTapCancellable = globalPushToTalkShortcutMonitor
            ...
            .receive(on: DispatchQueue.main)
            .sink { [weak self] _ in
                self?.showMainBlinkPanelFromShortcut()
            }
    }
    ...
    private func resolvedNewAgentTaskPrompt(from prompt: String) -> String {
        let explicitInstruction = Self.agentTaskCreationInstruction(from: prompt)
            ...
            ?? Self.blinkAgentInstruction(from: prompt)
    ...
    private func handleAgentFoundOpenableFile(_ fileURL: URL, session: CodexAgentSession) {
        ...
        openBlinkDocument(standardizedURL)
    ...
    func archiveSession(_ sessionID: UUID, allowIncomplete: Bool = false) {
        ...
        guard !isActivelyRunning, !session.isTurnActiveForChatQueue else {
            BlinkMessageLogStore.shared.append(
                lane: "agent",
                direction: "incoming",
                event: "blink.agent_task.archive_blocked_running",
                fields: [
        ...
        guard allowIncomplete || session.progressStage == .completed else {
            BlinkMessageLogStore.shared.append(
                lane: "agent",
                direction: "incoming",
                event: "blink.agent_task.archive_blocked_incomplete",
                fields: [
                    "sessionID": sessionID.uuidString,
                    "title": session.title,
    ...
    /// Launch a new chat session pre-configured as a specialist Blink
    /// agent. The agent's soul/instructions/memory are layered into the
    /// session's system prompt via `prependedSystemContext`.
    @discardableResult
    func createAndSelectNewCodexAgentSession(asAgent agent: BlinkAgentDefinition) -> CodexAgentSession {
        ...
        if let clickRequest = Self.nativeClickRequest(from: transcript) {
            BlinkMessageLogStore.shared.append(
                ...
                fields: [
                    ...
                    "executionMethod": "BlinkNativeComputerUseController.click",
    ...
    private func clickUsingNativeComputerUse(_ request: BlinkNativeClickRequest) {
        interruptCurrentVoiceResponse()
        let timing = activeRequestTiming
        let executionStartedAt = markRequestExecutionStarted(
            route: "native_cua.click",
    ...
    private func performNativeClick(
        ...
        displayFrame: CGRect?,
        label: String?,
        request: BlinkNativeClickRequest,
        executionStartedAt: Date,
        timing: BlinkRequestTiming?
    ) {
        do {
    ...
    func showQuickTextInputFromMenuBar() {
        // The legacy expandTextInput surface (with the "Ask Blink / Voice
    ...
    private func showMainBlinkPanelFromShortcut() {
        guard allPermissionsGranted else { return }
        guard !buddyDictationManager.isKeyboardShortcutSessionActiveOrFinalizing else { return }
    ...
    private static func isBlinkInvocationToken(_ normalizedText: String) -> Bool {
        ...
        case "blink", "klicky", "blink", "cookie", "quick":
            return true
        default:
            return false
        }
    ...
    static func agentTaskCreationInstruction(from transcript: String) -> String? {
        ...
        let patterns = [
            ...
            #"(?i)^\s*(?:(?:blink|blink)\s+)?(?:(?:can|could|would|will)\s+you\s+)?(?:please\s+)?(?:ask|tell|have|get|task)\s+(?:an?\s+|the\s+)?(?:agent|agenty|codex)\s+(?:to|for|with)\s+(.+?)\s*$"#,
            #"(?i)^\s*(?:(?:blink|blink)\s+)?(?:agent|agenty|codex)\s+(?:with|for)\s+(.+?)\s*$"#,
    ...
    private static func isVoiceRouteCapabilityQuestion(_ transcript: String) -> Bool {
        ...
        let questionPattern = #"^(?:can|could|would|will)\s+you\b|^(?:can|could|would)\s+blink\b|^is\s+it\s+possible\b|^do\s+you\b|^does\s+blink\b"#
    ...
    private static func containsDurableWorkTarget(_ normalized: String) -> Bool {
        let targetPattern = #"\b(?:blink|blink|github|repo|repository|codebase|project|app|settings|preference|preferences|log|logs|memory|skill|skills|desktop|download|downloads|document|documents|folder|folders|file|files|code|diff|git|branch|pull\s+request|pr|issue|issues|bug|test|tests|build|swift|xcode|email|gmail|calendar|spreadsheet|sheet|doc|slides)\b"#
    ...
    private static func nativeClickRequest(from transcript: String) -> BlinkNativeClickRequest? {
        let candidate = normalizedCommandCandidate(from: transcript)
        guard !candidate.isEmpty else { return nil }
        ...
        for pattern in patterns {
            ...
            return BlinkNativeClickRequest(
                targetDescription: candidate,
    ...
    private static func shortAgentInstructionSummary(_ instruction: String) -> String {
        ...
        let directTitleRules: [(pattern: String, title: String)] = [
            (#"(?i)\b(?:see\s+(?:the\s+)?issue\s+here|look\s+at\s+this|fix\s+this)\b.*\b(?:BlinkLog|NSXPCDecoder|ViewBridge|unifiedReasons|NSXPCConnection|agent_sdk_query|_sdk_query|kDragIPC|Reentrant\s+message)\b"#, "Log Issue Review"),
            (#"(?i)\b(?:BlinkLog|NSXPCDecoder|NSXPCInterface|NSXPCConnection|ViewBridge|NSViewBridgeError|Unable to obtain a task name port right|nw_protocol_instance|nw_read_request_report|unifiedReasons|agent_sdk_query|_sdk_query|Bridge SDK Message|kDragIPC|Reentrant\s+message)\b"#, "Log Issue Review"),
    ...
    /// agent task. Keep it generic so Blink does not speak the task
    ...
    private static func nextAgentDockAccentTheme(existingCount: Int) -> BlinkAccentTheme {
        let accentThemes: [BlinkAccentTheme] = [.blue, .mint, .rose, .amber, .white]
    ...
    private func submitAgentPrompt(_ prompt: String, to session: CodexAgentSession, includeScreenContext: Bool = true) {
        ...
        Task {
            // Agent allocation should not make the main Blink panel feel
    ...
    you're blink, a friendly always-on companion that lives in the user's menu bar. the user just spoke to you via push-to-talk and you can see their screen(s). your reply will be spoken aloud via text-to-speech, so write the way you'd actually talk. this is an ongoing conversation — you remember everything they've said before.
    ...
    3. SEARCH THE WEB conversationally when the user asks. answer from your own general knowledge; if the user explicitly wants live/current data (today's weather, latest price, breaking news), give a brief handoff-style acknowledgement; Blink routes that kind of task to Agent Mode.
    4. ROUTE WORK NATURALLY — simple conversational help stays in voice, direct computer-control is handled by Blink's computer-use path, and concrete file/code/research/settings/log work is handed to Agent Mode only when the user is asking for real tool work rather than talking through an idea.
    ...
    - modify settings, config, memory, skills, logs, soul.md, or any Blink state
    ...
    if the user asks you to do anything in the "DO NOT" list and Blink has not already routed it before you see the turn, be honest that no action has started. do not say "i’ll take care of that in the background", "on it", or "starting an agent" unless the app has actually routed the turn to Agent Mode or direct computer-use before it reaches you. say briefly: "that needs Blink's agent route, but it didn't start from this voice turn."
    ...
    2. SEARCH THE WEB conversationally when the user asks. answer from your own general knowledge; if the user explicitly wants live/current data (today's weather, latest price, breaking news), give a brief handoff-style acknowledgement; Blink routes that kind of task to Agent Mode.
    3. ROUTE WORK NATURALLY — simple conversational help stays in voice, direct computer-control is handled by Blink's computer-use path, and concrete file/code/research/settings/log work is handed to Agent Mode only when the user is asking for real tool work rather than talking through an idea.
    ...
    because this path speaks audio directly, never say or output Blink's internal point-control syntax. do not say "point none", "point control", "open bracket point", coordinates, or anything resembling [POINT:none]. if a visual target would help, describe it naturally in words instead.
    """
    ...
    you're Blink in tutor mode. the user wants to learn the app or workflow currently on screen, and you can see their focused window.
    ...
    - know that Blink can open apps and use the computer through Agent Mode when the user gives a direct action request.
    - simple open, type, and key-press actions use Blink's selected direct computer-use backend instead of Agent Mode.
    ...
    - do not claim you clicked or controlled anything in tutor observations. you can guide and point; simple direct action requests use Blink's selected direct computer-use backend, and broader tool work can use Agent Mode when explicitly routed there.
    ...
    You are Blink's visual click target resolver. The user wants Blink to actually click in the visible app, not merely point or explain.
    ...
    func createMemory(title: String, body: String) throws -> BlinkCore.WikiManager.Article {
...
final class BlinkTextModeWindowManager {
    ...
    private func createPanel(accentTheme: BlinkAccentTheme?, submitText: @escaping (String) -> Void) {
        ...
        BlinkWindowLevels.applyPanelDialogLevel(to: textModePanel)
    ...
    private func preparePanel(accentTheme: BlinkAccentTheme?, submitText: @escaping (String) -> Void) {
        ...
        } else if let hostingView: NSHostingView<BlinkTextModeInputView> = BlinkLiquidGlassWindowSurface.hostingView(in: panel) {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: droppedAttachments.isEmpty ? 6 : 7) {
            ...
            HStack(spacing: 9) {
                ...
                .pointerCursor()
                .disabled(!canSubmit)
                .accessibilityLabel("Send Blink prompt")
                .help("Send Blink prompt")
                ...
                .buttonStyle(.plain)
                .pointerCursor()
                .accessibilityLabel("Close Blink prompt")
                .help("Close Blink prompt")
            }
...
nonisolated private struct BlinkDirectActionStoredMemory: Codable, Sendable {
    var folderShortcuts: [BlinkDirectActionStoredFolderShortcut]
}

nonisolated private struct BlinkDirectActionStoredFolderShortcut: Codable, Sendable {
    var aliases: [String]
    var path: String
    var displayName: String
    var lastUsedAt: Date
...
private final class BlinkDirectActionMemoryStore: @unchecked Sendable {
    ...
    private let writeQueue = DispatchQueue(label: "com.blink.blink.direct-action-memory-writes", qos: .utility)
...
extension CompanionManager: BrowserWorkspaceAgentDelegate {
    ...
    public func selectedComputerUseModelUsesAnthropic() -> Bool {
        return BlinkModelCatalog.computerUseModel(withID: selectedComputerUseModel).provider == .anthropic
    }
}
