...
struct CodexAgentModePanelSection: View {
    ...
    var activeDockItem: BlinkAgentDockItem?
    var knowledgeIndex: BlinkCore.WikiManager.Index
    var responseCard: BlinkResponseCard?
    ...
    var isBlinkCursorEnabled: Bool
    var setBlinkCursorEnabled: (Bool) -> Void
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            TextField("Ask Blink to do something...", text: $prompt, axis: .vertical)
                .lineLimit(1...3)
...
struct CodexAgentModeSettingsSheet: View {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 10) {
                    settingsSection(
                        ...
                        subtitle: "Chooses the text model behind Blink's spoken replies. Realtime speech models live in Voice settings."
                    ) {
                        modelOptionGrid(
                            options: BlinkModelCatalog.voiceResponseModels,
                    ...
                    if isAdvancedModeEnabled {
                        ...
                        settingsSection(
                            ...
                            subtitle: "Used for background Agent Mode when Blink decides a request needs deeper tools, files, research, or coding. The bundled Codex runtime uses local ChatGPT sign-in when no OpenAI key is set."
                        ) {
                            VStack(spacing: 7) {
                                modelOptionGrid(
                    ...
                    settingsSection(
                        ...
                        VStack(spacing: 8) {
                            ...
                            blinkCursorToggleRow
                            tutorModeToggleRow
                        }
                    }

                    if isAdvancedModeEnabled {
                        BlinkKnowledgeIndexSummaryView(index: knowledgeIndex, openMemory: openMemory)

                        if let responseCard {
                            BlinkResponseCardCompactView(
                                card: responseCard,
                                actionHandlers: BlinkResponseCardActionHandlers(
    ...
    private var blinkCursorToggleRow: some View {
        HStack(spacing: 8) {
            ...
            Toggle("", isOn: Binding(
                get: { isBlinkCursorEnabled },
                set: { setBlinkCursorEnabled($0) }
            ))
            .toggleStyle(.switch)
            .labelsHidden()
