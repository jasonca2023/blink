...
nonisolated enum BlinkBuddyPetAtlas {
...
nonisolated enum BlinkBuddyAnimationRow: Int, CaseIterable {
    case idle           = 0
    case runningRight   = 1
    case runningLeft    = 2
    case waving         = 3
...
nonisolated struct BlinkBuddyPetManifest: Decodable {
    let id: String
...
nonisolated final class BlinkBuddyPet: Identifiable, Equatable {
    let id: String
    let displayName: String
    ...
    let frames: [BlinkBuddyAnimationRow: [CGImage]]
    ...
    init(
        ...
        description: String,
        bundleURL: URL,
        frames: [BlinkBuddyAnimationRow: [CGImage]]
    ) {
        self.id = id
    ...
    static func == (lhs: BlinkBuddyPet, rhs: BlinkBuddyPet) -> Bool {
...
nonisolated enum BlinkBuddyPetLoaderError: Error {
...
nonisolated enum BlinkBuddyPetLoader {
    /// Loads a single pet bundle directory.
    nonisolated static func loadPet(at bundleURL: URL) throws -> BlinkBuddyPet {
        let manifestURL = bundleURL.appendingPathComponent("pet.json")
        guard FileManager.default.fileExists(atPath: manifestURL.path) else {
            throw BlinkBuddyPetLoaderError.missingManifest
        }

        let manifestData: Data
        let manifest: BlinkBuddyPetManifest
        do {
            manifestData = try Data(contentsOf: manifestURL)
            manifest = try JSONDecoder().decode(BlinkBuddyPetManifest.self, from: manifestData)
        } catch let decode as DecodingError {
            throw BlinkBuddyPetLoaderError.malformedManifest(underlying: decode)
        } catch {
            throw BlinkBuddyPetLoaderError.malformedManifest(underlying: error)
        }
    ...
    nonisolated private static func decodeAtlas(at url: URL) throws -> CGImage {
        guard let source = CGImageSourceCreateWithURL(url as CFURL, nil),
            ...
            throw BlinkBuddyPetLoaderError.spritesheetDecodeFailed
        }

        // Validate atlas size once — a wrong-sized sheet would silently
        // mis-slice every frame, so fail loudly instead.
        if image.width != BlinkBuddyPetAtlas.totalWidth ||
            image.height != BlinkBuddyPetAtlas.totalHeight {
            throw BlinkBuddyPetLoaderError.spritesheetUnexpectedSize(
    ...
    nonisolated private static func sliceFrames(from atlas: CGImage) -> [BlinkBuddyAnimationRow: [CGImage]] {
        let cellW = BlinkBuddyPetAtlas.cellWidth
        let cellH = BlinkBuddyPetAtlas.cellHeight
        var result: [BlinkBuddyAnimationRow: [CGImage]] = [:]

        for row in BlinkBuddyAnimationRow.allCases {
    ...
    nonisolated private static func resolveSpritesheetURL(
        bundleURL: URL,
        manifest: BlinkBuddyPetManifest
    ) -> URL? {
...
/// `${CODEX_HOME}/pets/`, and Blink exports `CODEX_HOME` to a sandboxed
/// app-support directory so its agent sessions don't pollute the user's
/// `~/.codex/`. To make pets hatched inside Blink visible to the rest
...
@MainActor
final class BlinkBuddyPetLibrary: ObservableObject {
    static let shared = BlinkBuddyPetLibrary()

    @Published private(set) var pets: [BlinkBuddyPet] = []
    ...
    /// Optional source directory mirrored into `petsRootURL`. When Blink's
    ...
    init(
        petsRootURL: URL = BlinkBuddyPetLibrary.defaultCanonicalPetsRootURL(),
        mirrorSourceURL: URL? = BlinkBuddyPetLibrary.defaultMirrorSourceURL()
    ) {
    ...
    /// Uses the SHELL's `CODEX_HOME`, NOT Blink's sandboxed override.
    ...
    /// `<Blink app support>/CodexHome/pets/` — where in-app hatches
    /// land. Returned even if the directory doesn't exist yet; the watcher
    /// will create it.
    nonisolated static func defaultMirrorSourceURL() -> URL? {
        let openBlinkCodexHome = CodexHomeManager
            .defaultApplicationSupportDirectory()
            .appendingPathComponent("CodexHome", isDirectory: true)
        return openBlinkCodexHome.appendingPathComponent("pets", isDirectory: true)
    }

    func pet(withID id: String) -> BlinkBuddyPet? {
        pets.first { $0.id == id }
    }
    ...
    private func reload() async {
        let url = petsRootURL
        let result: ([BlinkBuddyPet], String?) = await Task.detached(priority: .utility) {
            BlinkBuddyPetLibrary.scanPets(at: url)
        }.value
    ...
    nonisolated private static func scanPets(at root: URL) -> ([BlinkBuddyPet], String?) {
        let fm = FileManager.default
        ...
        var byID: [String: (pet: BlinkBuddyPet, mtime: Date)] = [:]
        ...
        for child in children {
            ...
            do {
                let pet = try BlinkBuddyPetLoader.loadPet(at: child)
    ...
    private func scheduleMirror(delay: TimeInterval = 0.6) {
        ...
        mirrorTask = Task { [weak self] in
            ...
            let copiedAny: Bool = await Task.detached(priority: .utility) {
                BlinkBuddyPetLibrary.mirrorPets(from: mirror, to: destination)
            }.value
            if copiedAny {
    ...
    nonisolated private static func mirrorPets(from source: URL, to destination: URL) -> Bool {
        ...
        for child in children {
            ...
            do {
                ...
                NSLog("[BlinkBuddyPetLibrary] mirror copy failed for \(child.lastPathComponent): \(error)")
            }
        }
        return copiedAny
    }
