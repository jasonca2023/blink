...
private final class BlinkHUDPanel: NSPanel {
...
final class CodexHUDWindowManager: NSObject, NSWindowDelegate {
    ...
    private func makePanel(
        ...
        hostingView.layer?.cornerRadius = BlinkHUDLayout.cornerRadius
        hostingView.layer?.cornerCurve = .continuous
        hostingView.layer?.masksToBounds = true
        hostingView.layer?.backgroundColor = NSColor.clear.cgColor
        hostingView.autoresizingMask = [.width, .height]
        // Keep the HUD as an Blink surface, not a standard app window:
        ...
        let panel = BlinkHUDPanel(
            contentRect: NSRect(x: 0, y: 0, width: BlinkHUDLayout.width, height: BlinkHUDLayout.height),
        ...
        panel.minSize = NSSize(width: BlinkHUDLayout.minimumWidth, height: BlinkHUDLayout.minimumHeight)
        panel.contentMinSize = NSSize(width: BlinkHUDLayout.minimumWidth, height: BlinkHUDLayout.minimumHeight)
        let resizeContainer = BlinkHUDResizeContainerView(frame: NSRect(x: 0, y: 0, width: BlinkHUDLayout.width, height: BlinkHUDLayout.height))
        resizeContainer.autoresizingMask = [.width, .height]
        let glassBackdrop = BlinkLiquidGlassBackdropView(cornerRadius: BlinkHUDLayout.cornerRadius)
        glassBackdrop.frame = resizeContainer.bounds
        glassBackdrop.autoresizingMask = [.width, .height]
        glassBackdrop.configure(
            cornerRadius: BlinkHUDLayout.cornerRadius,
    ...
    private func enforceMinimumSize() {
        ...
        let minimumWidth = min(BlinkHUDLayout.minimumWidth, availableWidth)
        let minimumHeight = min(BlinkHUDLayout.minimumHeight, availableHeight)
    ...
    private func positionPanel() {
        ...
        let edgePadding = BlinkHUDLayout.screenEdgePadding
        ...
        let preferredY = frame.minY + BlinkHUDLayout.preferredBottomMargin
    ...
    private func availableDimension(for visibleDimension: CGFloat) -> CGFloat {
        max(
            BlinkHUDLayout.fallbackMinimumDimension,
            visibleDimension - (BlinkHUDLayout.screenEdgePadding * 2)
        )
    }
    ...
    private func hudHostingView(in panel: NSPanel) -> NSHostingView<ChatWorkspaceView>? {
        BlinkLiquidGlassWindowSurface.hostingView(in: panel)
    }
}
...
struct CodexHUDView: View {
    ...
    private var promptAutocompleteOptions: [BlinkPromptAutocompleteOption] {
    ...
    private var activeDockItem: BlinkAgentDockItem? {
    ...
    var body: some View {
        ...
        .frame(
            minWidth: BlinkHUDLayout.minimumWidth,
            idealWidth: BlinkHUDLayout.width,
            maxWidth: .infinity,
            minHeight: BlinkHUDLayout.minimumHeight,
            idealHeight: BlinkHUDLayout.height,
            maxHeight: .infinity
        )
        .background(
            Group {
        ...
        .confirmationDialog(
            ...
            Text("Running tasks stay active until you stop them, so Blink will not archive or close this task while it is still working.")
        }
    ...
    private var closeDialogButton: some View {
        ...
        .fixedSize()
        .layoutPriority(3)
        .help("Close Blink HUD")
        .accessibilityLabel("Close Blink HUD")
    }
    ...
    private var emptyState: some View {
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                Text("Ask Blink to inspect, edit, explain, or automate something.")
    ...
    private func transcriptBubble(_ entry: CodexTranscriptEntry, isUser: Bool) -> some View {
        VStack(alignment: isUser ? .trailing : .leading, spacing: 5) {
            ...
            let openableLinks = BlinkOpenableLinkExtractor.links(in: entry.text, limit: 2)
    ...
    private var reasoningStatusRow: some View {
        HStack(alignment: .top, spacing: 8) {
            BlinkThinkingDots(tint: DS.Colors.accentText)
                .padding(.top, 2)
    ...
    private var composer: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            BlinkPromptAutocompletePanel(options: promptAutocompleteOptions) { option in
                BlinkPromptAutocomplete.apply(option, to: &prompt)
            }
            ...
            .overlay(
                VStack(spacing: 7) {
                    Image(systemName: "plus.rectangle.on.folder")
                        ...
                        .foregroundColor(DS.Colors.accentText)
                    Text("Drop images or docs into Blink")
...
private final class BlinkHUDResizeContainerView: NSView {
