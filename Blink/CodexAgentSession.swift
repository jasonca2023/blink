...
final class CodexAgentSession: ObservableObject, Identifiable {
    let id: UUID
    let accentTheme: BlinkAccentTheme
    ...
    init(
        id: UUID = UUID(),
        title: String = "Agent",
        accentTheme: BlinkAccentTheme = .blue,
    ...
    func submitPromptFromUI(_ prompt: String, screenContext: CodexAgentScreenContext? = nil) {
        ...
        // When the user has picked HuggingFace as the agent backend,
        // Codex CLI cannot serve the task (it requires the OpenAI
        // Responses API). Redirect the prompt to the in-app live agent
        // loop, which talks to HF chat completions directly.
        if BlinkAgentBackendKind.current() == .huggingFace {
            if entries.isEmpty {
                title = Self.shortTitle(from: trimmed)
            }
            lastSubmittedPrompt = trimmed
            entries.append(CodexTranscriptEntry(role: .user, text: trimmed))
            appendActivityStatusLine("Routing to Live agent (HuggingFace backend)…")
            entries.append(CodexTranscriptEntry(
                role: .assistant,
                text: "Running via HuggingFace through the Live agent loop. Watch progress in Settings → Agent Mode → Live agent control."
            ))
            status = .ready
            BlinkAgentLoopController.shared.start(prompt: trimmed)
            return
        }
    ...
    private func promptForModel(prompt: String, screenContext: CodexAgentScreenContext?, coordinationNote: String?) -> String {
        ...
        - When you find a local document, image, or other user file, include its exact local path in your final answer so Blink can show it.
    ...
    private func ensureThread() async throws {
        ...
        You are running inside Blink Agent Mode on macOS. Be direct, helpful, and careful. Prefer concrete actions over vague advice.

        When working on the Blink app repo, do not run terminal `xcodebuild`. Use Xcode for app builds and permission testing, and use `swiftc -parse <relevant Swift source files>` for lightweight Swift syntax checks.

        Blink's runtime map is at \(layout.runtimeMapFile.path). Read it when the user asks about logs, storage locations, memory, skills, widgets, settings, sessions, or where Blink keeps anything. You may view or edit those local files when asked, subject to normal safety rules for destructive changes, credentials, and permissions.

        Blink's persona is at \(layout.soulFile.path). Read it before task work. Treat it as the operating identity for voice-first behavior, autonomy, memory, archive-first changes, and plain-English progress.

        Archive-first is mandatory. When replacing, optimizing, pruning, or superseding Blink memory, skills, runtime notes, prompts, config, or log-derived artifacts, copy or move the old version into \(layout.archivesDirectory.path) first. Do not delete old artifacts unless the user explicitly asks for deletion and understands it is destructive.
    ...
    private func ensureCodexAuthentication() async throws {
        guard homeManager.modelProviderID == BlinkCodexConfigTemplate.defaultModelProviderID else { return }
    ...
    private func buildAgentCoordinationNote(for prompt: String) async -> String? {
        ...
        if mentionedPaths.isEmpty {
            ...
            let claimed = await BlinkAgentFileLeaseCoordinator.shared.claimPaths(mentionedPaths, for: id, title: title)
